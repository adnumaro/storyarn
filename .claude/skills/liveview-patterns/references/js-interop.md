...
LiveView uses morphdom for DOM patching. Third-party JS libraries that manage their own DOM state conflict with this.
This reference covers resolution patterns.
...
Translated text can change DOM structure (different word count, RTL, different element wrapping). JS hooks that rely on
DOM position break across locales.
