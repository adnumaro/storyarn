  ...
  <div class="h-screen w-screen flex flex-col bg-background">
    ...
    >
      ...
      <div class="flex items-center gap-1">
        ...
        <span class="text-xs text-muted-foreground/60 px-1">{{ versionLabel }}</span>
        <a
          v-if="nextVersionUrl"
          :href="nextVersionUrl"
          data-phx-link="patch"
    ...
    <!-- Split panes -->
    <div class="flex-1 overflow-hidden grid grid-cols-2 divide-x divide-border">
      <!-- Left: current state -->
      ...
      <div class="flex flex-col overflow-hidden">
        ...
        <iframe :src="versionUrl" class="flex-1 w-full border-0" :title="versionLabel" />
      </div>
    </div>
  </div>
</template>
