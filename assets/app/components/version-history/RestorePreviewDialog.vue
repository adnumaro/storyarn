      ...
      <template v-if="restoreData">
        <div v-if="restoreData.report.hasConflicts" class="space-y-3">
          ...
          >
            ...
            <span class="text-sm"
              >Shortcut collision — will be renamed to "{{
                restoreData.report.resolvedShortcut
              }}"</span
            >
          </div>
          ...
          <p class="text-sm text-muted-foreground">
            <template v-if="restoreData.skipPreSnapshot"
              >Missing references will be cleared.</template
            >
            <template v-else
              >Missing references will be cleared. Current state will be saved as a
              backup.</template
            >
          </p>
        </div>
