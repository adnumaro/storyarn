...
const live = useLive();
const { t } = useI18n();
const searchQuery = ref(docs.search.query);
...
const searchResultsVisible = computed(() => docs.search.results !== null);
...
function activeGuide(guide: DocsGuideNav): boolean {
  return docs.guide?.category === guide.category && docs.guide.slug === guide.slug;
}
    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      <aside
        :class="[
          ...
          docs.sidebarOpen ? 'translate-x-0' : '-translate-x-full',
        ]"
      >
        <nav class="px-4 space-y-1">
          <div class="mb-5">
            <form
              class="relative"
              @submit.prevent="live.pushEvent('search', { query: searchQuery })"
            >
              <Search
      ...
      >
        <div class="flex-1 w-full max-w-4xl mx-auto py-8 min-w-0">
          ...
          >
            ...
            <div>
              <LiveLink v-if="docs.next" :to="docs.next.url" class="group flex flex-col items-end">
                <span class="text-xs text-muted-foreground group-hover:text-muted-foreground">
                  {{ $t("docs.next") }}
