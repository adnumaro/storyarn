...
import FlowCommentComposer from "./comments/FlowCommentComposer.vue";
import type {
  CommentStatus,
  CommentStatusFilter,
...
const { state } = defineProps<{ state: FlowCommentsPanelState }>();
const live = useLive();
...
const composerEnabled = computed(
  () =>
    ...
    (state.thread ? state.thread.status === "open" : state.selectedNodeId != null),
);
...
<template>
  <Sidebar side="right" :open="state.open" @close="live.pushEvent('comments_close', {})">
    <template #header>
    ...
    <div id="flow-comments-content" class="space-y-4 pb-2">
      ...
      <template v-if="state.thread">
        <div class="space-y-2">
          ...
          <div class="flex items-start justify-between gap-2">
            <div class="min-w-0">
              <p class="truncate text-sm font-medium">{{ state.thread.source.label }}</p>
              ...
              />{{
                $t(
                  state.thread.status === "open"
                    ? "flows.comments.resolve"
                    : "flows.comments.reopen",
                )
              }}</Button
            >
          </div>
      ...
      <template v-else>
        ...
        <ol v-else class="space-y-2" :aria-label="$t('flows.comments.threads')">
          <li v-for="thread in state.threads" :key="thread.id">
            ...
            >
              <div class="flex items-center justify-between gap-2">
                <span class="truncate text-xs font-medium">{{ thread.source.label }}</span
                ><CheckCheck
      ...
      <div v-if="state.canComment" v-show="composerEnabled" class="border-t border-border pt-3">
        <div
          v-if="replyTo"
      ...
      <p
        v-else-if="!state.thread && state.selectedNodeId == null && state.threads.length"
