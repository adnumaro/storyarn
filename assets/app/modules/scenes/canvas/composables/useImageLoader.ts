 ...
 */
export function useImageLoader(urlMapRef: Ref<Map<string | number, string | null>>) {
  const images = ref<Record<string, HTMLImageElement>>({});
  const urlCache = new Map<string, HTMLImageElement | null>(); // url -> HTMLImageElement
  const pendingUrls = new Map<string | number, string>(); // id -> url (currently loading)
  let aborted = false;

  function loadUrl(id: string | number, url: string): void {
  ...
  watch(
    ...
    (urlMap) => {
      ...
      const toRemove = Object.keys(images.value).filter((id) => !currentIds.has(id));
