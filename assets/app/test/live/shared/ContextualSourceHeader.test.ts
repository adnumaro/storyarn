...
const cases = [
  { sourceType: "sheet" as const, header: { health } },
  {
...
describe("Contextual source header composition", () => {
  it.each(cases)(
    ...
    async ({ sourceType, header }) => {
      ...
      expect(wrapper.find(`#${sourceType}-comments-toggle`).exists()).toBe(false);
      expect(wrapper.find(`#${sourceType}-comments-create-mode`).exists()).toBe(false);
