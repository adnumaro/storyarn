...
import {
  useFlowCodeEditor,
  type UseFlowCodeEditorReturn,
} from "@modules/flows/editor/expression/useFlowCodeEditor";
import type { ParsedCondition } from "@modules/flows/editor/expression/language/tree-parser";
import type { Variable } from "@modules/flows/editor/expression/domain/variables";
...
function mountCodeEditor(active: Ref<boolean>) {
  ...
  let editor!: UseFlowCodeEditorReturn;
  ...
  const wrapper = mount(
    defineComponent({
      setup() {
        ...
        editor = useFlowCodeEditor(container, {
          mode: "condition",
          variables,
          disabled: false,
          active,
