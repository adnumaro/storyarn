...
describe("DialogueVoice", () => {
  ...
  it("ignores queued play and pause events from a stopped or replaced voice", async () => {
    const wrapper = mount(DialogueVoice, { props: { voice: voice() } });
    await flushPromises();
    const old = wrapper.get("audio").element;
    (wrapper.vm as unknown as { stop: () => void }).stop();
    old.dispatchEvent(new Event("play"));
    expect(wrapper.emitted("playing-change")?.at(-1)).toEqual([false]);
    await wrapper.setProps({ voice: voice({ continuityKey: "next", url: "/next.mp3" }) });
    await flushPromises();
