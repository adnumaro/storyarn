...
export function createZoneHandler(hook, i18n = {}) {
  ...
  function renderZones() {
    const zones = (hook.sceneData.zones || [])
      .slice()
  ...
  function addZoneToMap(zone) {
    ...
    polygon.on("dblclick", (e) => {
      ...
      const actionType = polygon.zoneData.action_type || "none";
      if (actionType !== "none") return;

      if (polygon.zoneData.target_type === "scene" && polygon.zoneData.target_id) {
        ...
        hook.pushEvent("create_child_scene_from_zone", {
    ...
    polygon.on("contextmenu", (e) => {
      ...
      if (!data.locked) {
        ...
        items.push({
          label: i18n.create_child_scene || "Create child scene",
    ...
    if (zone.target_type === "scene" && zone.target_id) {
      polygon.getElement &&
        polygon.on("add", () => {
          const el = polygon.getElement();
