 ...
 */
export function resolveNodeColor(nodeType, nodeData, configColor, sheetsMap, hubsMap) {
	const d = nodeData || {};
	...
	if (nodeType === "jump" && d.target_hub_id && hubsMap) {
		...
		if (hub?.color_hex) return hub.color_hex;
	}
...
export function formatAssignment(assignment) {
	...
	if (op === "set") return `${label} ${ref} to ${val}`;
	if (["add", "subtract"].includes(op)) return `${label} ${val} to ${ref}`;
