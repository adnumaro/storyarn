...
const sheetsWithVariables = computed(() =>
	groupVariablesBySheet(props.variables),
);

const variableType = computed(() => {
	const v = findVariable(
		props.variables,
		props.assignment.sheet,
		props.assignment.variable,
	);
...
const template = computed(() => {
	...
	const hasValueSlot = t.some(
		(item) => item.type === "slot" && item.key === "value",
	);
	if (
		hasValueSlot &&
	...
	const filtered =
		ct === null
			...
			: sheetsWithVariables.value.filter((s) =>
					s.vars.some((v) => ct.includes(v.block_type)),
				);
...
const variableGroups = computed(() => {
	...
	const sheet = sheetsWithVariables.value.find(
		(s) => s.shortcut === props.assignment.sheet,
	);
	if (!sheet) return [];
	const ct = typesForOperator(props.assignment.operator || "set");
	const vars =
		ct === null
			? sheet.vars
			: sheet.vars.filter((v) => ct.includes(v.block_type));
	return [
		{
			heading: sheet.name,
			items: vars.map((v) => ({
...
const valueOptions = computed(() => {
	...
	if (
		v &&
		(v.block_type === "select" || v.block_type === "multi_select") &&
		v.options
	) {
...
const isNumericValue = computed(
	() =>
		props.assignment.value_type !== "variable_ref" &&
		(props.assignment.operator === "add" ||
			props.assignment.operator === "subtract"),
);
...
function update(field, value) {
	...
	if (field === "sheet") {
		...
		if (sv) {
			...
			if (!ops.includes(updated.operator) && ops.length > 0)
...
function changeOperator(op) {
	...
	if (
		NO_VALUE_OPERATORS.has(op) !==
		NO_VALUE_OPERATORS.has(props.assignment.operator)
	) {
		updated.value = null;
		updated.value_sheet = null;
