...
import {
  getTemplate, expandTemplateForVariableRef, NO_VALUE_OPERATORS, ALL_OPERATORS,
  OPERATOR_VERBS, OPERATOR_DROPDOWN_LABELS, operatorsForType, typesForOperator,
...
const template = computed(() => {
  ...
  if (hasValueSlot && props.assignment.value_type === "variable_ref" && !NO_VALUE_OPERATORS.has(op)) {
...
const sheetOptions = computed(() => {
  ...
  const filtered = ct === null ? sheetsWithVariables.value : sheetsWithVariables.value.filter((s) => s.vars.some((v) => ct.includes(v.block_type)))
...
const variableGroups = computed(() => {
  ...
  return [{ heading: sheet.name, items: vars.map((v) => ({ value: v.variable_name, label: v.variable_name })) }]
})
...
const valueOptions = computed(() => {
  if (props.assignment.value_type === "variable_ref") {
    ...
    if (!sheet) return []
    return sheet.vars.map((v) => ({ value: v.variable_name, label: v.variable_name }))
  }
  const v = findVariable(props.variables, props.assignment.sheet, props.assignment.variable)
  if (v && (v.block_type === "select" || v.block_type === "multi_select") && v.options) {
    return v.options.map((opt) => ({ value: opt.key, label: opt.value || opt.key }))
  }
  return []
})
...
function update(field, value) {
  ...
  if (field === "sheet") {
    ...
    if (sv) { const ops = operatorsForType(sv.block_type); if (!ops.includes(updated.operator) && ops.length > 0) updated.operator = ops[0] }
    if (updated.operator !== "add" && updated.operator !== "subtract") updated.value = null
    updated.value_sheet = null
  } else if (field === "value_sheet") { updated.value = null }
...
function changeOperator(op) {
  ...
  if (NO_VALUE_OPERATORS.has(op) !== NO_VALUE_OPERATORS.has(props.assignment.operator)) {
    updated.value = null; updated.value_sheet = null; updated.value_type = "literal"
  }
  ...
  if (ct !== null && updated.variable) {
    ...
    if (cv && !ct.includes(cv.block_type)) { updated.variable = null; updated.value = null; updated.value_sheet = null; updated.value_type = "literal" }
  }
        ...
        <template v-if="item.type === 'verb'">
          <Popover v-if="!disabled" v-model:open="operatorDropdownOpen">
            ...
            <PopoverContent class="w-40 p-0" align="start" :side-offset="4">
              <div
                v-for="op in availableOperators" :key="op"
          ...
          <span v-else class="sentence-text font-medium">{{ OPERATOR_VERBS[assignment.operator] || item.value }}</span>
        </template>

        <span v-else-if="item.type === 'text'" class="sentence-text">{{ item.value }}</span>

        <VariableCombobox v-else-if="item.type === 'slot' && item.key === 'sheet'"
          :model-value="assignment.sheet || ''" :options="sheetOptions" :placeholder="item.placeholder"
          :disabled="disabled" @update:model-value="(v) => update('sheet', v)" />

        <VariableCombobox v-else-if="item.type === 'slot' && item.key === 'variable'"
          :model-value="assignment.variable || ''" :groups="variableGroups" :placeholder="item.placeholder"
          :disabled="disabled || !assignment.sheet" @update:model-value="(v) => update('variable', v)" />

        <VariableCombobox v-else-if="item.type === 'slot' && item.key === 'value_sheet'"
          :model-value="assignment.value_sheet || ''" :options="valueSheetOptions" :placeholder="item.placeholder"
          :disabled="disabled || !assignment.variable" @update:model-value="(v) => update('value_sheet', v)" />

        <template v-else-if="item.type === 'slot' && item.key === 'value'">
          <VariableCombobox v-if="assignment.value_type === 'variable_ref'"
            :model-value="assignment.value || ''" :options="valueOptions" :placeholder="item.placeholder"
            :disabled="disabled || !assignment.value_sheet" @update:model-value="(v) => update('value', v)" />
          <VariableCombobox v-else-if="!isFreeTextValue"
            :model-value="assignment.value || ''" :options="valueOptions" :placeholder="item.placeholder"
            :disabled="disabled || !assignment.variable" @update:model-value="(v) => update('value', v)" />
          <VariableCombobox v-else
            :model-value="assignment.value || ''" :placeholder="item.placeholder"
            :disabled="disabled || (!assignment.variable && assignment.operator !== 'add' && assignment.operator !== 'subtract')"
            free-text :input-type="isNumericValue ? 'number' : 'text'"
            @update:model-value="(v) => update('value', v)" />
        </template>
      </template>

      <!-- Value type toggle -->
      <button v-if="!disabled && assignment.variable && !NO_VALUE_OPERATORS.has(assignment.operator)"
        type="button" class="sp-row-action" style="opacity: 0.5" title="Switch literal / variable reference"
        @click="toggleValueType">
        <ArrowLeftRight class="size-3" />
      </button>
    </div>

    <button v-if="!disabled" type="button" class="sp-row-action sp-row-action-danger" @click="emit('remove')">
      <X class="size-3" />
    </button>
  </div>
</template>
