...
export function useZoneDrag({
	...
	const zoneDragOverride = ref(null);
	let dragStartWorld = null;
	let dragOriginalVertices = null;
	let dragZoneId = null;
	let lastDragTime = 0;
	 ...
	 * and editable.
	 */
	function onZoneMouseDown(zoneId, e) {
		if (!editMode.value || !canEdit.value) return;
		if (selectedType.value !== "zone" || selectedId.value !== zoneId) return;

		// Check lock
		const lock = entityLocks.value[String(zoneId)];
		if (lock && String(lock.userId) !== String(currentUserId.value)) return;
	 ...
	 */
	function onZoneDragMove() {
		if (!isDraggingZone.value) return;

		const world = getWorldPointer();
		if (!world || !dragStartWorld) return;
