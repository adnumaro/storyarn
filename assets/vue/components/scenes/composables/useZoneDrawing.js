...
const GHOST_FILL = "rgba(99,102,241,0.15)";
...
export function useZoneDrawing({
	 ...
	 */
	function handleZoneCreationClick(e) {
		if (!editMode.value || !canEdit.value) return false;
		const tool = activeTool.value;

		// Only handle zone tools
		if (!PRESET_TOOLS.has(tool) && tool !== "freeform") return false;
	...
	function addFreeformVertex(pos, worldX, worldY) {
	...
	const drawingOverlay = computed(() => {
		...
		// Vertex markers
		const vertexConfigs = pixelVerts.map((p, i) => ({
			x: p.x,
			y: p.y,
