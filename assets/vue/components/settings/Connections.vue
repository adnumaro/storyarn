  ...
  <div>
    <div class="space-y-4">
      ...
      >
        <div class="flex items-center gap-4">
          <!-- Provider Icon -->
          <div class="size-10 flex items-center justify-center rounded-lg bg-muted">
            <!-- GitHub -->
            <svg v-if="provider.key === 'github'" class="size-6" viewBox="0 0 24 24" fill="currentColor">
            ...
            <!-- Google -->
            <svg v-else-if="provider.key === 'google'" class="size-6" viewBox="0 0 24 24">
            ...
            <!-- Discord -->
            <svg v-else-if="provider.key === 'discord'" class="size-6" viewBox="0 0 24 24" fill="#5865F2">
            ...
            <!-- Fallback -->
            <Link v-else class="size-6" />
          </div>

          <div>
            <p class="font-medium">{{ provider.label }}</p>
            <p v-if="findIdentity(provider.key)" class="text-sm text-muted-foreground">
              {{ findIdentity(provider.key).provider_email || findIdentity(provider.key).provider_name }}
            </p>
        ...
        <div>
          <template v-if="findIdentity(provider.key)">
            <template v-if="hasPassword || identities.length > 1">
              <Button
                variant="ghost"
                size="sm"
    ...
    <div class="mt-8 p-4 rounded-lg bg-muted">
      ...
      <ul class="text-sm text-muted-foreground space-y-1">
        <li v-for="(reason, idx) in translations.reasons" :key="idx">
          &bull; {{ reason }}
        </li>
      </ul>
    </div>
