...
const totalVariableRefs = computed(() =>
	props.variableUsage.reduce(
		(sum, v) => sum + v.reads.length + v.writes.length,
		0,
	),
);
...
<template>
  ...
  <div v-else class="space-y-2">
    <!-- ── Variable Usage ── -->
    <Collapsible v-model:open="variableUsageOpen" class="rounded-xl border border-border/60 bg-card">
      <CollapsibleTrigger class="flex items-center gap-2.5 w-full px-4 py-3 cursor-pointer hover:bg-muted/40 rounded-xl transition-colors">
        <div class="size-6 rounded-md bg-indigo-500/15 text-indigo-600 dark:text-indigo-400 flex items-center justify-center shrink-0">
          <Database class="size-3.5" />
        </div>
        <span class="text-sm font-semibold flex-1 text-left">Variable Usage</span>
        <Badge v-if="totalVariableRefs > 0" variant="secondary" class="text-[10px] px-1.5 py-0 rounded-full">
          {{ totalVariableRefs }}
        </Badge>
        <ChevronDown :class="['size-4 text-muted-foreground transition-transform duration-200', !variableUsageOpen && '-rotate-90']" />
      </CollapsibleTrigger>
          ...
          <div v-else class="space-y-2.5">
            ...
            >
              ...
              <div class="flex items-center gap-2 mb-2.5 pb-2 border-b border-border/40">
                ...
                <code class="text-[11px] text-muted-foreground/70 bg-muted px-1.5 py-0.5 rounded">{{ variable.shortcut }}</code>
                <Badge variant="outline" class="text-[10px] px-1.5 py-0">{{ variable.type }}</Badge>
              </div>

              <!-- Writes -->
              <div v-if="variable.writes.length > 0" class="mb-2">
                <span class="text-[11px] font-semibold text-orange-500 dark:text-orange-400 flex items-center gap-1 mb-1.5 uppercase tracking-wider">
                  <Pencil class="size-3" />
                  Modified by
                </span>
                <div class="ml-1 space-y-0.5">
                  <a
                    ...
                    :key="'w' + i"
                    :href="ref.sourceType === 'scene_zone' ? sceneUrl(ref.sceneId) : flowUrl(ref.flowId, ref.nodeId)"
                  ...
                  >
                    <div :class="['size-5 rounded flex items-center justify-center shrink-0', ref.sourceType === 'scene_zone' ? 'bg-sky-500/15 text-sky-500' : 'bg-amber-500/15 text-amber-500']">
                      <component
                        :is="ref.sourceType === 'scene_zone' ? Map : nodeIcon(ref.nodeType)"
                        class="size-3"
                      />
                    </div>
                    <span class="font-medium">{{ ref.sourceType === 'scene_zone' ? ref.sceneName : ref.flowName }}</span>
                    ...
                    <span v-if="ref.detail" class="text-muted-foreground/70 font-mono text-[11px]">{{ ref.detail }}</span>
                    <Badge v-if="ref.stale" class="text-[10px] px-1.5 py-0 bg-orange-500/15 text-orange-500 border-orange-500/20 gap-0.5">
              ...
              <div v-if="variable.reads.length > 0">
                <span class="text-[11px] font-semibold text-blue-500 dark:text-blue-400 flex items-center gap-1 mb-1.5 uppercase tracking-wider">
                  <Eye class="size-3" />
                  Read by
                </span>
                <div class="ml-1 space-y-0.5">
    ...
    <Collapsible v-model:open="backlinksOpen" class="rounded-xl border border-border/60 bg-card">
      <CollapsibleTrigger class="flex items-center gap-2.5 w-full px-4 py-3 cursor-pointer hover:bg-muted/40 rounded-xl transition-colors">
        <div class="size-6 rounded-md bg-violet-500/15 text-violet-600 dark:text-violet-400 flex items-center justify-center shrink-0">
          <Link class="size-3.5" />
        </div>
        <span class="text-sm font-semibold flex-1 text-left">Backlinks</span>
        <Badge v-if="backlinks.length > 0" variant="secondary" class="text-[10px] px-1.5 py-0 rounded-full">
          {{ backlinks.length }}
        </Badge>
        <ChevronDown :class="['size-4 text-muted-foreground transition-transform duration-200', !backlinksOpen && '-rotate-90']" />
      </CollapsibleTrigger>
      ...
      <CollapsibleContent>
        <div class="px-4 pb-4">
          ...
          <div v-else class="space-y-0.5">
            ...
            >
              <div :class="['shrink-0 size-8 rounded-lg flex items-center justify-center', sourceColor(backlink.sourceInfo.type)]">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                  ...
                  <span v-if="backlink.sourceInfo.shortcut" class="text-[11px] text-muted-foreground/60 font-mono">
                    #{{ backlink.sourceInfo.shortcut }}
                  </span>
                </div>
                <div class="flex items-center gap-1.5 mt-0.5">
                  <Badge v-if="backlink.sourceInfo.contextType" variant="outline" class="text-[10px] px-1.5 py-0">
                    {{ backlink.sourceInfo.contextType }}
                  </Badge>
                  <span v-if="backlink.sourceInfo.contextLabel" class="text-[11px] text-muted-foreground/70">
                    {{ backlink.sourceInfo.contextLabel }}
                  </span>
                </div>
              </div>

              <span class="text-[11px] text-muted-foreground/50 tabular-nums shrink-0">{{ backlink.date }}</span>
            </a>
          </div>
        </div>
      </CollapsibleContent>
    ...
    <!-- ── Scene Appearances ── -->
    <Collapsible v-model:open="sceneAppearancesOpen" class="rounded-xl border border-border/60 bg-card">
      <CollapsibleTrigger class="flex items-center gap-2.5 w-full px-4 py-3 cursor-pointer hover:bg-muted/40 rounded-xl transition-colors">
        <div class="size-6 rounded-md bg-sky-500/15 text-sky-600 dark:text-sky-400 flex items-center justify-center shrink-0">
          <Map class="size-3.5" />
        </div>
        <span class="text-sm font-semibold flex-1 text-left">Appears on Scenes</span>
        <Badge v-if="sceneAppearances.length > 0" variant="secondary" class="text-[10px] px-1.5 py-0 rounded-full">
          {{ sceneAppearances.length }}
        </Badge>
        <ChevronDown :class="['size-4 text-muted-foreground transition-transform duration-200', !sceneAppearancesOpen && '-rotate-90']" />
      </CollapsibleTrigger>
        ...
        <div class="px-4 pb-4">
          ...
          <div v-else class="space-y-0.5">
            <a
              ...
              class="flex items-center gap-3 py-2.5 px-2 -mx-2 rounded-lg hover:bg-muted/50 group cursor-pointer transition-colors"
            >
              <div class="shrink-0 size-8 rounded-lg flex items-center justify-center bg-sky-500/15 text-sky-600 dark:text-sky-400">
                <Map class="size-4" />
              </div>
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-1.5 mt-0.5">
                  <Badge variant="outline" class="text-[10px] px-1.5 py-0">{{ appearance.elementType }}</Badge>
                  <span v-if="appearance.elementName" class="text-[11px] text-muted-foreground/70">{{ appearance.elementName }}</span>
                </div>
              </div>

              <ArrowUpRight class="size-4 text-muted-foreground/20 group-hover:text-muted-foreground/50 transition-colors shrink-0" />
            </a>
          </div>
        </div>
      </CollapsibleContent>
