    ...
    <div v-if="editable && editor" class="flex flex-wrap items-center gap-0.5 px-1.5 py-1 border-b border-border bg-muted/30">
      <button
        v-for="btn in [
          ...
          { action: toggleStrike, icon: Strikethrough, active: isActive('strike'), title: 'Strikethrough' },
        ]"
        :key="btn.title"
        type="button"
        :class="['size-7 rounded flex items-center justify-center transition-colors', btn.active ? 'bg-accent text-accent-foreground' : 'text-muted-foreground hover:bg-accent hover:text-accent-foreground']"
        :title="btn.title"
        @click="btn.action"
      >
      ...
      <button
        v-for="btn in [
          { action: toggleH1, icon: Heading1, active: isActive('heading', { level: 1 }), title: 'Heading 1' },
          { action: toggleH2, icon: Heading2, active: isActive('heading', { level: 2 }), title: 'Heading 2' },
          { action: toggleH3, icon: Heading3, active: isActive('heading', { level: 3 }), title: 'Heading 3' },
        ]"
        :key="btn.title"
        type="button"
        :class="['size-7 rounded flex items-center justify-center transition-colors', btn.active ? 'bg-accent text-accent-foreground' : 'text-muted-foreground hover:bg-accent hover:text-accent-foreground']"
      ...
      <button
        v-for="btn in [
          { action: toggleBulletList, icon: List, active: isActive('bulletList'), title: 'Bullet list' },
          { action: toggleOrderedList, icon: ListOrdered, active: isActive('orderedList'), title: 'Ordered list' },
          { action: toggleBlockquote, icon: Quote, active: isActive('blockquote'), title: 'Blockquote' },
