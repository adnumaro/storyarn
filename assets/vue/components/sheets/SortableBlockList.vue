...
watch(() => props.layoutItems, (v) => { localItems.value = [...v] })
...
function extractFromColumnGroup(items, block, hoveredItem, hoveredPlacement) {
  ...
  if (remainingBlocks.length <= 1) {
    const dissolved = remainingBlocks.map((b) => ({ type: "full_width", block: b }))
  ...
  if (hoveredItem) {
    const hoveredIdx = nextItems.findIndex((item) =>
          ...
          <template #menu>
            <DropdownMenu v-if="canEdit">
              ...
              <DropdownMenuContent align="end" class="z-[1030]">
                <DropdownMenuItem class="text-destructive gap-2 text-xs" @select="deleteBlock(item.block.id)">
