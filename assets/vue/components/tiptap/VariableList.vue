...
<template>
  <div v-if="items.length > 0" class="bg-popover border border-border rounded-lg shadow-lg p-1 max-h-60 overflow-y-auto z-50 min-w-[200px] max-w-[300px]">
    <button
    ...
    >
      <component
        :is="TYPE_ICONS[item.block_type] || Hash"
        class="size-3.5 shrink-0 opacity-60"
      />
      <span class="truncate">{{ item.label || item.ref }}</span>
      <span v-if="item.sheet_name" class="text-muted-foreground text-xs ml-auto">{{ item.sheet_name }}</span>
    </button>
  </div>
  <div v-else class="bg-popover border border-border rounded-lg shadow-lg p-1 z-50">
