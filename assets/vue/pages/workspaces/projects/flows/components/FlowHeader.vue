...
<template>
  <div class="flex items-stretch gap-2">
    ...
    <div class="hidden lg:flex items-center gap-1 px-1 py-1 v2-surface-panel text-xs">
        ...
        <Popover v-model:open="sceneOpen">
          <PopoverTrigger as-child>
            ...
            >
              ...
              <span v-if="sceneInherited" class="text-muted-foreground text-[10px]">(inherited)</span>
      ...
      <!-- Word count -->
      <div class="v2-toolbar-btn gap-1.5 text-muted-foreground" :title="`${flowWordCount} words in this flow`">
        <Text class="size-3.5" />
        ...
        <Popover v-model:open="healthOpen">
          ...
          <PopoverContent side="bottom" :side-offset="4" class="w-max max-h-60 overflow-y-auto p-1">
            <div v-if="flowErrorNodes.length > 0">
              <div v-if="flowInfoNodes.length > 0" class="px-2 py-1 text-[10px] text-muted-foreground font-medium uppercase">
                Errors
              </div>
              <button
            ...
            <div v-if="flowInfoNodes.length > 0">
              <div v-if="flowErrorNodes.length > 0" class="px-2 py-1 text-[10px] text-muted-foreground font-medium uppercase mt-1">
                Info
              </div>
              <button
              ...
              >
                <span class="truncate">{{ node.label }}</span>
                <span v-if="node.reason" class="text-[11px] text-muted-foreground">{{ node.reason }}</span>
              </button>
            </div>
          </PopoverContent>
        </Popover>
    ...
    <div v-if="canEdit" class="flex items-center v2-surface-panel px-2">
      <div v-if="saveStatus === 'saving'" class="flex items-center gap-1 text-xs text-muted-foreground">
        <div class="size-3 border-2 border-muted-foreground/20 border-t-muted-foreground/60 rounded-full animate-spin" />
        <span>Saving</span>
      </div>
      <div v-else-if="saveStatus === 'saved'" class="flex items-center gap-1 text-xs text-green-500/70">
        <Check class="size-3" />
        <span>Saved</span>
      </div>
    </div>
