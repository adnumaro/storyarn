...
import {
	...
	ContextMenuItem,
	ContextMenuTrigger,
} from "@/vue/components/ui/context-menu/index.js";
...
const isSelected = computed(
	() =>
		props.selectedFlowId != null &&
		String(props.node.id) === String(props.selectedFlowId),
);
...
const shouldAutoExpand = computed(
	() =>
		hasChildren.value &&
		hasSelectedDescendant(props.node, props.selectedFlowId),
);
  ...
  <div :class="{ 'opacity-30': isDragging }">
    ...
    <ContextMenu>
      ...
      <ContextMenuContent v-if="canEdit">
        <ContextMenuItem v-if="!node.is_main" class="gap-2 text-xs" @select="emit('setMain', node.id)">
          <Star class="size-3.5" />
          Set as main
        </ContextMenuItem>
