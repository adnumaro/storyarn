...
export function useFlowEditor({ pushEvent, handleEvent }) {
	...
	async function init(containerEl, flowData, opts = {}) {
		...
		containerEl.addEventListener(
			...
			(e) => {
				...
				if (!nodeEl) {
					if (hookProxy._flowContext?.editingNodeId) {
						exitInlineEdit();
					}
					clearToolbar();
	...
	function setupServerEvents() {
		...
		handleEvent("flow_updated", (data) =>
			_editorHandlers.handleFlowUpdated(data),
		);
		...
		handleEvent("node_updated", (data) => {
			...
			_nodeUpdateQueue = _nodeUpdateQueue
				.then(async () => {
					...
					if (
						toolbarState.nodeId &&
						String(data.id) === String(toolbarState.nodeId)
					) {
		...
		handleEvent("node_data_changed", (data) =>
			_editorHandlers.handleNodeDataChanged(data),
		);
		handleEvent("flow_meta_changed", (data) =>
			_editorHandlers.handleFlowMetaChanged(data),
		);
		handleEvent("connection_added", (data) =>
			_editorHandlers.handleConnectionAdded(data),
		);
		handleEvent("connection_removed", (data) =>
			_editorHandlers.handleConnectionRemoved(data),
		);
		handleEvent("connection_updated", (data) =>
			_editorHandlers.handleConnectionUpdated(data),
		);

		if (_navigationHandler) {
			handleEvent("navigate_to_hub", (data) =>
				_navigationHandler.navigateToHub(data.jump_db_id),
			);
			handleEvent("navigate_to_node", (data) =>
				_navigationHandler.navigateToNode(data.node_db_id),
			);
			handleEvent("navigate_to_jumps", (data) =>
				_navigationHandler.navigateToJumps(data.hub_db_id),
			);
		}
	}
