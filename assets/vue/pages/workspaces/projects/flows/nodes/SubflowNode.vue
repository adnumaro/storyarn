...
<template>
  <NodeShell :color="color" :selected="data.selected">
    ...
    <div v-if="refFlowName" class="text-[11px] text-muted-foreground px-3 py-2 max-w-[200px] border-b border-border/10 break-words">
    ...
    <div v-else-if="!hasRef" class="text-[11px] text-muted-foreground px-3 py-2 max-w-[200px] border-b border-border/10 break-words">
      ...
      <div v-for="[key, output] in outputs" :key="'o-' + key" class="flex items-center py-1 text-[11px] text-muted-foreground justify-end">
        <span class="px-2 max-w-[220px] break-words text-right inline-flex items-center gap-1">
          <template v-if="getExitInfo(key)">
            <CornerDownLeft v-if="getExitInfo(key).exit_mode === 'caller_return'" class="size-2.5 shrink-0" />
