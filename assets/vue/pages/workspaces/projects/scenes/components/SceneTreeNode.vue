...
const isSelected = computed(
	() =>
		props.selectedSceneId != null &&
		String(props.node.id) === String(props.selectedSceneId),
);
...
const shouldAutoExpand = computed(
	() =>
		hasChildren.value &&
		hasSelectedDescendant(props.node, props.selectedSceneId),
);
...
// ── Droppable: the children container ──
  ...
  <div :class="{ 'opacity-30': isDragging }">
    ...
    >
      <SceneTreeNode
        ...
        @request-delete="(scene) => emit('requestDelete', scene)"
        @drop="(e) => emit('drop', e)"
      />
    </div>
  </div>
