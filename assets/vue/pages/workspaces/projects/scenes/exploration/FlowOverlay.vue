...
const visibleResponses = computed(() =>
	(props.slide?.responses || []).filter((r) => r.valid),
);
...
<template>
  <div class="absolute bottom-0 inset-x-0 z-30 border-t border-border bg-background/95 backdrop-blur-sm">
    <div class="px-5 py-4 max-w-4xl mx-auto">
      ...
      <template v-else-if="slideType === 'slug_line'">
        ...
        <div v-if="slide.description" class="mt-1.5 text-sm text-foreground/70" v-html="slide.description" />
      </template>
