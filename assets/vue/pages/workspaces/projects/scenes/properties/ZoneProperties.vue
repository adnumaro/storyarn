...
import ExpressionEditor from "@/vue/components/ExpressionEditor.vue";
import {
	EntityCombobox,
	TextField,
	ToggleField,
  ...
  <div class="space-y-3">
    ...
    <div v-if="element.actionType === 'display'" class="pt-3 border-t border-border">
      <EntityCombobox
        ...
        :options="projectVariables.map(v => ({ id: v.ref || `${v.sheet_shortcut}.${v.variable_name}`, name: v.label || `${v.sheet_shortcut}.${v.variable_name}` }))"
    ...
    <div class="pt-3 border-t border-border space-y-2">
      <div class="flex items-center justify-between">
        ...
        <div class="flex gap-0.5">
          <button
            ...
            :disabled="!canEdit"
            @click="live.pushEvent('update_zone_condition_effect', { id: String(element.id), effect: opt.id })"
          >
            {{ opt.name }}
          </button>
        </div>
      ...
      <ConditionBuilder
        ...
        :variables="projectVariables"
        :disabled="!canEdit"
        @update:condition="(c) => live.pushEvent('update_zone_condition', { 'zone-id': String(element.id), condition: c })"
      />
    </div>
  </div>
</template>
