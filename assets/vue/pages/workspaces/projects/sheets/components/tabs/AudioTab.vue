...
const totalLines = computed(() =>
	props.groupedLines.reduce((sum, g) => sum + g.lines.length, 0),
);
...
<template>
  ...
  <!-- Empty state -->
  <div v-else-if="groupedLines.length === 0" class="rounded-xl border border-border/60 bg-card p-8 text-center">
  ...
  <div v-else class="space-y-2">
    ...
    >
      ...
      <div class="flex items-center">
        <CollapsibleTrigger class="flex items-center gap-2.5 flex-1 px-4 py-3 cursor-pointer hover:bg-muted/40 rounded-xl transition-colors">
          <div class="size-6 rounded-md bg-amber-500/15 text-amber-600 dark:text-amber-400 flex items-center justify-center shrink-0">
          ...
          <ChevronDown class="size-4 text-muted-foreground transition-transform duration-200 [[data-state=closed]_&]:[-rotate-90]" />
        </CollapsibleTrigger>
      </div>
      ...
      <div class="px-4 pb-3 space-y-2">
        ...
        >
          ...
          <div v-if="line.audioAsset" class="space-y-1.5">
            ...
            <audio controls class="w-full h-8 [&::-webkit-media-controls-panel]:bg-muted/50 rounded">
          ...
          <div v-else-if="canEdit" class="flex items-center gap-2 mt-1">
            <Popover :open="openPopoverNodeId === line.nodeId" @update:open="(v) => v ? openPopover(line.nodeId) : (openPopoverNodeId = null)">
