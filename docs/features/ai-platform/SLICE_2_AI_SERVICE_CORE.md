...
- Migrations: `ai_usage_events` (user/workspace/project, feature, provider, model, input/cached/output tokens, provider_cost_usd, credits_charged, latency_ms, succeeded) — **including the canonical acceptance-outcome contract consumed by every tool slice: a constrained terminal `acceptance_outcome` (`accepted | dismissed | abandoned | failed`, NULL until terminalized) plus an optional structured `acceptance_detail` (accepted variant index, image index, per-item counts), exactly ONE aggregate terminal outcome per user-facing generation — finer-grained per-variant/per-item/per-image decisions are usage-event-keyed product events through the `Storyarn.Analytics` allowlist, never extra outcome values. Terminalization is write-once and enforced, not hoped for: a single facade function performs a conditional `UPDATE … WHERE acceptance_outcome IS NULL` (first terminal write wins; later/concurrent writes are no-ops — concurrency-tested); `failed` is stamped by the execute pipeline itself when the call fails (consumers only ever record `accepted | dismissed | abandoned`); rows still NULL after a defined window (24h initial, owner-tunable) are finalized as `abandoned` by the same sweeper cadence that settles orphaned reservations, so the acceptance-rate denominator converges. Slices 6–10 write outcomes through this boundary, none redefine it** · `ai_credit_ledger` — append-only entries (`monthly_grant | reservation | settlement | refund | purchase`) with:
...
Telemetry: exactly three events — `[:ai, :execute, :start]`, `[:ai, :execute, :stop]`, `[:ai, :execute, :exception]` — with **low-cardinality, content-free metadata only** (task name, quality tier, lane, provider, classified error code; NEVER prompts/responses and NEVER raw user/workspace/project ids — those live in metering rows, not telemetry) · metering rows are the usage audit (counts/costs, never content) · **budget rejection is an explicit `:insufficient_credits` error — NEVER auto-degrade to a cheaper model or partial run (owner no-fallback rule; the ChatGPT-plan suggestion to "degrade the operation" is explicitly NOT adopted)** · **operation `reason` is an allowlisted code atom mapped through `dgettext` — raw exception text/URLs/params are redacted diagnostics kept in server logs, never on the user-visible operation** · orphaned-reservation sweeper emits an alert metric when it settles anything (a settle by sweeper means a crash happened) · user docs: AI credits/balance section added to the flag-hidden AI docs.

## Verification / Definition of Done

- ExUnit: TaskRegistry, Router, Internal provider (Req.Test), Credits (grant idempotency per period, expiry, consumption order, reserve/settle/refund, insufficient balance, **concurrent reservations cannot overdraw**), Budget gates (reservation precedes call, output caps enforced, orphaned-reservation sweeper), `ai_operations` states + retry idempotency (no double charge) + **owner-scope enforcement on poll/PubSub (cross-scope access rejected)** + **same-transaction enqueue (no orphaned queued rows after a simulated crash)**, Metering rows, execute inline + async paths, **acceptance-outcome boundary (constrained values; write-once under concurrent terminal writes; `failed` stamped by the pipeline on failed calls; NULL-window sweep finalizes to `abandoned`)**.
