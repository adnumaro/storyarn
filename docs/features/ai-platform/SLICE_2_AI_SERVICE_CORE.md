...
- Migrations: `ai_usage_events` (user/workspace/project, feature, provider, model, input/cached/output tokens, provider_cost_usd, credits_charged, latency_ms, succeeded) — **including the canonical acceptance-outcome contract consumed by every tool slice: a constrained terminal `acceptance_outcome` (`accepted | dismissed | abandoned | failed`, NULL until the consumer records it) plus an optional `acceptance_detail` (e.g. variant index), exactly one terminal outcome per user-facing generation; Slices 6–10 write it, none redefine it** · `ai_credit_ledger` — append-only entries (`monthly_grant | reservation | settlement | refund | purchase`) with:
