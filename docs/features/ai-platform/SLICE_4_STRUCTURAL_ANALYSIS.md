...
First flagship AI action: **"Analyze structure"** on a flow (or project) detecting dead branches, unreachable nodes/endings, orphan hubs, and characters absent for long stretches — via **deterministic graph algorithms**, with the LLM only writing the human-readable report. Fixed credit price, verifiable findings, palette command + report panel with per-finding navigation.
