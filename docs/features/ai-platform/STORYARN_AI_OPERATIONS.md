...
1. The configured endpoint performs inference in the contracted EU region.
2. Zero data retention is enabled for the account/endpoint and covers the selected model.
3. The model supports the structured-output contract used by the diagnostic.
4. Current provider prices have been copied into an immutable price snapshot.
5. Conservative per-operation and global/workspace cost ceilings have been approved.

Together documents zero-data-retention controls separately from EU/dedicated deployment availability. A generic serverless endpoint must therefore never be assumed to be an EU route. See [Together privacy](https://www.together.ai/privacy), [EU data centers and dedicated deployment](https://support.together.ai/articles/8079447813-eu-data-centers-and-dedicated-model-deployment), [structured outputs](https://docs.together.ai/docs/inference/chat/structured-outputs), and [inference pricing](https://docs.together.ai/docs/inference/pricing).

## Runtime configuration

All variables below are required when the managed lane is enabled:

| Variable                                   | Purpose                                                        |
| ------------------------------------------ | -------------------------------------------------------------- |
| `STORYARN_AI_MANAGED_ENABLED`              | Manual global circuit breaker                                  |
| `STORYARN_AI_MANAGED_PROVIDER`             | Must be `together` in Slice 3                                  |
| `STORYARN_AI_MANAGED_EU_VERIFIED`          | Operator attestation; must be `true`                           |
| `STORYARN_AI_MANAGED_ZDR_VERIFIED`         | Operator attestation; must be `true`                           |
| `STORYARN_AI_TOGETHER_API_KEY`             | Ephemeral managed credential; never persisted                  |
| `STORYARN_AI_MANAGED_ENDPOINT`             | Full verified HTTPS chat-completions endpoint                  |
| `STORYARN_AI_MANAGED_REGION`               | Content-free provenance label for the contracted region        |
...
- Run one operator diagnostic only in the verified EU/ZDR environment.
