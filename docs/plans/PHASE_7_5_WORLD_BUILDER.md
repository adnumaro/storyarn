...
const toLatLng = (x, y) => L.latLng((y / 100) * imageHeight, (x / 100) * imageWidth);
...
_This phase depends on the existing Asset system for map backgrounds and the Variable system (sheets/blocks) for layer triggers. No new flow node types are required._
