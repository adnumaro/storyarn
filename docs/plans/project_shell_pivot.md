...
5. **`layout: false` interaction with root.html.heex**: sticky children with `layout: false` still get wrapped in `root.html.heex`? Actually no — `layout: false` means no Phoenix *LiveView* layout (skips app.html.heex). `root.html.heex` is the Plug-level layout, always applied. Since sticky children are rendered as separate LV processes, they technically get their own full HTML scaffolding... hmm, need to verify. If it breaks we fix by investigating `root_layout` config.
