...
Ideation owns brainstorming sessions, configuration history, authored ideas,
recoverable edits and explicit publication. Its Ideas capability enters Sessions
through a capability facade for current Project authorization and session locking.
Sessions enters Projects through current read authorization and transaction-aware
mutation authorization. Its empty architecture baseline is sealed. See the
...
| Workspaces, Projects and AI      | Commercial                                                    | Whether the owner's account is over its plan's limits, so the workspace is read-only, through `Storyarn.Commercial`     | Reviewed public contract; ENG-241                                |
...
- [ENG-108](https://linear.app/sunset/issue/ENG-108/add-transactional-owner-transfer-workflows-for-workspaces-and-projects): transactional Project ownership transfer is complete, with one canonical owner transition, locked reauthorization for owner-only writers, rollback guarantees and concurrency coverage. Persistent database enforcement remains a separate reviewed decision.
- [ENG-244](https://linear.app/sunset/issue/ENG-244/remove-workspace-ownership-transfer): Workspace ownership transfer, delivered with ENG-108, was removed. A Workspace keeps the owner it was created with; owner-only writers still reauthorize under the Workspace lock.
