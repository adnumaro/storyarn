defmodule Storyarn.Accounts.Workers.DeliverResetPasswordInstructionsWorker do
  @moduledoc """
  ...
  def perform(%Oban.Job{args: %{"email" => email, "encrypted_reset_url" => encrypted_reset_url}}) do
    with {:ok, reset_url} <- Accounts.decrypt_reset_password_url(encrypted_reset_url),
         {:ok, _email} <- Accounts.deliver_reset_password_instructions(email, reset_url) do
      :ok
    else
      {:error, reason} ->
