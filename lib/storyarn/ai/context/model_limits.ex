defmodule Storyarn.AI.Context.ModelLimits do
  ...
  def validate_context(%Task{} = task, %ExecutionRoute{provider: "fake"}, _input, %{package: %Package{} = package}) do
    if valid_package?(task, package), do: :ok, else: {:error, :model_context_limits_unavailable}
  end
  ...
  def validate_context(%Task{} = task, %ExecutionRoute{} = route, input, %{package: %Package{} = package}) do
    ...
    with true <- valid_package?(task, package) || {:error, :model_context_limits_unavailable},
         {:ok, entry} <- executable_entry(route.provider, route.model),
  ...
  @spec validate_provider_request(String.t(), map(), map()) ::
          ...
          | {:error,
             ...
             | :model_context_window_exceeded
             | :model_output_limit_exceeded}
  def validate_provider_request(provider, %{contextual?: true, input: input, context_policy: policy} = request, body)
      when is_binary(provider) and is_map(body) do
    with true <- contextual_input?(input, policy) || {:error, :model_context_limits_unavailable},
  ...
  @spec contextual_input?(term(), Task.t() | Policy.t()) :: boolean()

  def contextual_input?(%{"request" => request, "context" => context} = input, %Task{} = task)
      when map_size(input) == 2 and (is_map(request) or is_list(request)) do
    case Task.context_policy(task) do
      {:ok, policy} -> valid_context_payload?(context, policy)
      {:error, :invalid_context_policy} -> false
    end
  end

  def contextual_input?(%{"request" => request, "context" => context} = input, %Policy{} = policy)
      when map_size(input) == 2 and (is_map(request) or is_list(request)) do
    case Policy.new(policy, policy.contract) do
      {:ok, policy} -> valid_context_payload?(context, policy)
      {:error, :invalid_context_policy} -> false
    end
  end

  def contextual_input?(_input, _task), do: false

  defp valid_package?(%Task{} = task, %Package{scope: scope, payload: payload}) do
    case Task.context_policy(task) do
      {:ok, %{scope: ^scope} = policy} -> valid_context_payload?(payload, policy)
      _invalid -> false
    end
  end

  defp valid_context_payload?(
         %{"version" => "storyarn-context-v1", "scope" => scope, "entities" => entities} = context,
         policy
       )
       when map_size(context) == 3 and is_list(entities) do
    scope == Atom.to_string(policy.scope) and
      Enum.all?(entities, &valid_context_entity?(&1, policy.contract))
  end

  defp valid_context_payload?(_context, _policy), do: false

  defp valid_context_entity?(%{"type" => type, "id" => id, "content" => content} = entity, contract)
       when map_size(entity) == 3 and is_binary(type) and (is_integer(id) or is_binary(id)) and is_map(content) do
    contract.source_type?(type, :included)
  end

  defp valid_context_entity?(_entity, _contract), do: false
