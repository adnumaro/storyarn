defmodule Storyarn.AI.IntegrationAssignments do
  ...
  @spec assign(scope(), pos_integer(), pos_integer()) ::
  ...
  @spec unassign(scope(), pos_integer(), pos_integer()) ::
          {:ok, IntegrationWorkspaceAssignment.t()} | {:error, mutation_error()}
  ...
  defp eligible(role, %WorkspacePolicy{} = policy) do
    if Governance.personal_lane_allowed?(policy, role),
      do: :ok,
      else: {:error, :member_personal_ai_disabled}
  end

  defp eligibility(nil, %WorkspacePolicy{}), do: :workspace_membership_required

  defp eligibility(role, %WorkspacePolicy{} = policy) do
    cond do
      role == "owner" -> :owner
      Governance.personal_lane_allowed?(policy, role) -> :member
      true -> :blocked
    end
  end
  ...
  defp policies_by_workspace(workspace_ids), do: Governance.effective_policies(workspace_ids)
