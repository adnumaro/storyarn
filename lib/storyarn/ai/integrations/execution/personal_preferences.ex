defmodule Storyarn.AI.PersonalPreferences do
  ...
  @spec overview(scope()) :: {:ok, map()} | {:error, mutation_error()}
  def overview(%{user: %{id: user_id}} = scope) do
    with :ok <- feature_enabled(scope) do
      workspace_entries = Governance.list_workspaces(scope)
  ...
  @spec summary(scope(), pos_integer()) :: {:ok, map()} | {:error, mutation_error()}
  def summary(%{user: %{id: user_id}} = scope, workspace_id) when is_integer(workspace_id) and workspace_id > 0 do
    with :ok <- feature_enabled(scope),
      ...
      policy = Governance.get_effective_policy(workspace.id)
  ...
  """
  @spec impacts(scope(), pos_integer()) :: {:ok, [map()]} | {:error, mutation_error()}
    ...
    with :ok <- feature_enabled(scope),
      ...
      impacts =
        ...
        |> Enum.map(fn {preference, workspace, role} ->
          policy_allowed? = eligible?(role, Governance.get_effective_policy(workspace.id))
  ...
  @spec put(scope(), pos_integer(), atom() | String.t(), pos_integer(), String.t()) ::
  ...
  @spec delete(scope(), pos_integer(), atom() | String.t()) ::
          {:ok, PersonalPreference.t()} | {:error, mutation_error()}
  ...
  defp put_locked(scope, user_id, workspace_id, slot, integration_id, model) do
    with {:ok, workspace, membership} <- WorkspaceAccessLocks.lock(scope, workspace_id),
         ...
         {:ok, entry} <- Routing.fetch_model(integration.provider, String.trim(model)),
  ...
  defp resolve_preference(preference, task, assignment_source) do
    ...
    with %Integration{} = integration <-
         ...
         {:ok, entry} <- Routing.fetch_model(preference.provider, preference.model),
         true <- not entry.deprecated? || {:error, :model_deprecated},
         :ok <- normalize_model_authorization(Routing.authorize_model(entry, integration)),
  ...
  defp overview_data(user_id, workspace_ids) do
    policies = Governance.effective_policies(workspace_ids)
  ...
  defp options_for_assignment({integration, assignment}) do
    ...
    |> Routing.models_for_provider_entries(include_deprecated: false)
    |> Enum.flat_map(&option_for_entry(integration, assignment, &1))
  end

  defp option_for_entry(integration, assignment, entry) do
    with :ok <- normalize_model_authorization(Routing.authorize_model(entry, integration)),
  ...
  defp authorize_model(%Entry{} = entry, %Integration{} = integration) do
    normalize_model_authorization(Routing.authorize_model(entry, integration))
  end
  ...
  defp implementation_status(provider, model) do
    case Routing.fetch_model(provider, model) do
  ...
  defp role_available?(slot) do
    ...
    |> Enum.flat_map(&Routing.models_for_capability/1)
  ...
  defp eligible?(role, %WorkspacePolicy{} = policy) do
    Governance.workspace_can?(role, :use_ai) and Governance.personal_lane_allowed?(policy, role)
  end
