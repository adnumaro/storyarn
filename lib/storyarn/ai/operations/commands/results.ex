defmodule Storyarn.AI.Results do
  ...
  @spec get_operation(scope(), pos_integer()) :: Operation.t() | nil
  ...
  @spec get(scope(), pos_integer()) :: {:ok, map() | list(), Operation.t()} | {:error, atom()}
  ...
  """
  @spec get_by_idempotency_key(scope(), String.t(), String.t()) ::
  ...
  """
  @spec operations_by_idempotency_keys(scope(), String.t(), [String.t()]) ::
  ...
  """
  @spec record_view(scope(), pos_integer()) :: :ok
  ...
  def apply(%{user: %{id: actor_id}} = scope, operation_id, current_revision, apply_fun) when is_function(apply_fun, 2) do
    Repo.transaction(fn ->
      ...
      with %Operation{execution_status: "succeeded", user_disposition: nil} <- operation,
           ...
           :ok <- Integrations.authorize_operation_consent(operation, task, route, lock: true),
