defmodule Storyarn.AI.RouteResolver do
  ...
  alias Storyarn.AI.Routing.Rules.ConfigMap
  ...
  defp settlement_status(%ExecutionRoute{lane: :managed, price_units: units}, decision),
    do: ManagedSpend.preflight_status(:managed, decision.workspace_id, units)
  ...
  defp personal_resolution(%PolicyDecision{} = decision, %Task{} = task) do
    if :personal_byok in decision.allowed_lanes and task.personal_byok_allowed? and not decision.scheduled? do
      preference = Integrations.resolve_preference(decision.actor_id, decision.workspace_id, task)

      integrations =
        decision.actor_id
        |> Integrations.list_active()
        |> Map.new(&{&1.provider, &1})

      choices =
        task.capability
        |> Integrations.personal_providers_for()
        |> Enum.map(
          &personal_choice(
            &1,
  ...
  defp public_preference(resolution, choices) do
    ...
    resolution
    |> Integrations.public_preference_resolution()
  ...
  defp route_for_lane(:managed, decision, task) do
    ...
    with true <- is_list(config),
         true <- ManagedSpend.available?(:managed),
         provider when is_binary(provider) <- config[:provider],
         model when is_binary(model) <- config[:model],
         true <- config[:enabled] == true,
         true <- config[:verified_zdr] == true,
  ...
  defp personal_choice(config, integration, decision, task, preference) do
    assignment =
      Integrations.active_assignment(
        decision.actor_id,
  ...
  defp personal_choice_with_assignment(config, integration, assignment, decision, task, preference) do
    case Integrations.personal_model_status(config, integration) do
      :ready ->
  ...
  defp personal_choice_with_consent(config, integration, assignment, decision, task, preference) do
    case Integrations.active_consent(decision.actor_id, decision.workspace_id, integration.id, task) do
      nil ->
        choice(
          config,
          task,
  ...
  defp choice(config, task, status, integration_id, assignment_id, route, preference) do
    ...
    %{
      ...
      consent_policy_version: Integrations.consent_policy_text_version(),
      preferred: preferred?,
