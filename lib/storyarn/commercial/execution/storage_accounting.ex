defmodule Storyarn.Commercial.Billing.StorageAccounting do
  @moduledoc """
  ...
  alias Storyarn.Commercial.Billing.Persistence.AssetRecord, as: Asset
  alias Storyarn.Commercial.Billing.Persistence.ProjectRecord, as: Project
  alias Storyarn.Commercial.Billing.Persistence.ProjectSnapshotRecord, as: ProjectSnapshot
  alias Storyarn.Commercial.Billing.Persistence.ProjectSnapshotRestoreRecord, as: ProjectSnapshotRestore

  alias Storyarn.Commercial.Billing.Persistence.SnapshotObjectPublicationClaimRecord,
    as: SnapshotObjectPublicationClaim

  alias Storyarn.Commercial.Billing.Persistence.StorageCleanupOwnershipReceiptRecord,
  ...
  @snapshot_export_event_buffer_key {__MODULE__, :snapshot_export_event_buffer}
  ...
  def with_workspace_lock(workspace_id, fun, opts \\ [])
    ...
    with_snapshot_export_event_buffer(fn ->
      run_workspace_lock_transaction(workspace_id, fun, transaction_opts)
    end)
  end
  ...
  def transact_with_workspace_lock(workspace_id, fun, opts \\ [])
    ...
    with_snapshot_export_event_buffer(fn ->
      run_transact_with_workspace_lock(workspace_id, fun, transaction_opts)
    end)
  end

  defp run_workspace_lock_transaction(workspace_id, fun, transaction_opts) do
    Repo.transaction(
      fn ->
  ...
  defp run_transact_with_workspace_lock(workspace_id, fun, transaction_opts) do
    Repo.transact(
  ...
  @doc "Subscribes the caller to committed Commercial snapshot export-lease invalidations for one Project."
  @spec subscribe_project_snapshot_export_leases(pos_integer()) :: :ok | {:error, term()}
  def subscribe_project_snapshot_export_leases(project_id) when is_integer(project_id) and project_id > 0 do
    Phoenix.PubSub.subscribe(Storyarn.PubSub, snapshot_export_lease_topic(project_id))
  end

  def subscribe_project_snapshot_export_leases(_project_id), do: {:error, :invalid_project_id}
  ...
  defp reservation_underestimated(workspace, reservation, actual_bytes) do
    ...
    limit = workspace |> Subscriptions.plan_for() |> Plan.limit(:storage_bytes_per_workspace)
  ...
  defp emit_after_mutation({:ok, result} = ok, workspace_id, action) do
    maybe_publish_snapshot_export_lease_invalidation(result, action)
    usage = workspace_usage(workspace_id)
  ...
  # Standalone storage mutations have committed before this hook runs. When a
  # caller deliberately composes the mutation inside Commercial's outer
  # workspace-lock transaction, the process-local buffer publishes only after
  # that transaction commits and discards the event on rollback.
  defp maybe_publish_snapshot_export_lease_invalidation(%StorageReservation{} = reservation, action)
       when action in [:reserved, :renewed, :released, :committed] do
    publish_snapshot_export_lease_invalidation(reservation)
  end

  defp maybe_publish_snapshot_export_lease_invalidation(%{reservation: %StorageReservation{} = reservation}, action)
       when action in [:reserved, :renewed, :released, :committed] do
    publish_snapshot_export_lease_invalidation(reservation)
  end

  defp maybe_publish_snapshot_export_lease_invalidation(_result, _action), do: :ok

  defp publish_snapshot_export_lease_invalidation(%StorageReservation{
    ...
    publish_or_buffer_snapshot_export_lease_invalidation(project_id, snapshot_id)
  end

  defp publish_snapshot_export_lease_invalidation(%StorageReservation{}), do: :ok

  defp snapshot_export_lease_topic(project_id), do: "commercial:snapshot_export_leases:project:#{project_id}"

  defp with_snapshot_export_event_buffer(fun) when is_function(fun, 0) do
    case Process.get(@snapshot_export_event_buffer_key) do
      %MapSet{} ->
        fun.()

      nil ->
        Process.put(@snapshot_export_event_buffer_key, MapSet.new())

        try do
          result = fun.()

          if match?({:ok, _result}, result) do
            @snapshot_export_event_buffer_key
            |> Process.get(MapSet.new())
            |> Enum.each(fn {project_id, snapshot_id} ->
              broadcast_snapshot_export_lease_invalidation(project_id, snapshot_id)
            end)
          end

          result
        after
          Process.delete(@snapshot_export_event_buffer_key)
        end
    end
  end

  defp publish_or_buffer_snapshot_export_lease_invalidation(project_id, snapshot_id) do
    case Process.get(@snapshot_export_event_buffer_key) do
      %MapSet{} = events ->
        Process.put(@snapshot_export_event_buffer_key, MapSet.put(events, {project_id, snapshot_id}))
        :ok
  ...
  defp broadcast_snapshot_export_lease_invalidation(project_id, snapshot_id) do
    Phoenix.PubSub.broadcast(
      Storyarn.PubSub,
      snapshot_export_lease_topic(project_id),
      {:commercial_snapshot_export_lease_state_invalidated, snapshot_id}
    )
  end
