defmodule Storyarn.Flows.AvatarIntegrity do
  @moduledoc false
  ...
  def lock_and_normalize_node_avatar_for_project(project_id, _node_type, data)
    ...
    case normalize_optional_id(Map.get(data, "avatar_id")) do
      ...
      {:ok, avatar_id} ->
        with {:ok, speaker_sheet_id} <- normalize_optional_id(Map.get(data, "speaker_sheet_id")),
             {:ok, avatar_sheet_id} <- lock_project_avatar(avatar_id, project_id),
             :ok <- validate_avatar_speaker(avatar_id, avatar_sheet_id, speaker_sheet_id) do
  ...
  @spec lock_and_normalize_node_avatar_batch_for_project(integer(), [map()]) ::
          [{:ok, map()} | {:error, validation_error()} | :error]
  def lock_and_normalize_node_avatar_batch_for_project(project_id, data_items)
    ...
    prepared = Enum.map(data_items, &{&1, normalize_batch_avatar_id(&1)})
    ...
    Enum.map(prepared, fn {data, avatar_result} ->
      normalize_batch_avatar(data, avatar_result, project_avatar_sheets, existing_avatar_ids)
    end)
  end
  ...
  @spec lock_avatar_for_delete(integer(), keyword()) ::
          {:ok, SheetAvatarRecord.t()} | {:error, term()}
  def lock_avatar_for_delete(avatar_id, opts \\ []) when is_integer(avatar_id) do
    ...
    case Repo.one(
      ...
      {%SheetAvatarRecord{} = avatar, project_id} ->
  ...
  defp lock_project_avatar(avatar_id, project_id) do
    case Repo.one(
      ...
      nil ->
        if Repo.exists?(from(avatar in SheetAvatarRecord, where: avatar.id == ^avatar_id)) do
  ...
  defp existing_avatar_ids(avatar_ids) do
    from(avatar in SheetAvatarRecord, where: avatar.id in ^avatar_ids, select: avatar.id)
    |> Repo.all()
    |> MapSet.new()
  end

  defp normalize_batch_avatar_id(data) when is_map(data), do: normalize_optional_id(Map.get(data, "avatar_id"))

  defp normalize_batch_avatar_id(_data), do: :error

  defp normalize_batch_avatar(data, {:ok, nil}, _project_avatar_sheets, _existing_avatar_ids), do: {:ok, data}

  defp normalize_batch_avatar(data, {:ok, avatar_id}, project_avatar_sheets, existing_avatar_ids) do
    with {:ok, speaker_sheet_id} <- normalize_optional_id(Map.get(data, "speaker_sheet_id")),
         {:ok, avatar_sheet_id} <-
           batch_avatar_sheet(avatar_id, project_avatar_sheets, existing_avatar_ids),
         :ok <- validate_avatar_speaker(avatar_id, avatar_sheet_id, speaker_sheet_id) do
      {:ok, Map.put(data, "avatar_id", avatar_id)}
    end
  end
  ...
  defp validate_delete_ownership(avatar, project_id, opts) do
    ...
    cond do
      is_integer(expected_sheet_id) and avatar.sheet_id != expected_sheet_id -> {:error, :not_found}
      is_integer(expected_project_id) and project_id != expected_project_id -> {:error, :not_found}
      true -> {:ok, avatar}
    end
  end
  ...
  defp ensure_no_node_references(avatar_id) do
    ...
    if count == 0,
      do: :ok,
      else: {:error, {:avatar_in_use, avatar_id, {:referenced_by_flow_nodes, count}}}
  end
  ...
  defp ensure_transaction! do
    if not Repo.in_transaction?() do
      raise ArgumentError,
            "flow avatar reference integrity checks require an explicit database transaction"
    end
  end
end
