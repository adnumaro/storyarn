defmodule Storyarn.Flows.ConnectionCrud do
  ...
  defp create_connection_result(%Flow{} = flow, attrs) do
    ...
    Repo.transaction(fn ->
      with {:ok, %{flow: locked_flow}} <-
             References.lock_active_flow_for_write(flow, :key_share),
  ...
  def update_connection(%FlowConnection{} = connection, attrs) do
    ...
    fn ->
      with {:ok, %{flow: flow}} <-
             References.lock_active_flow_for_write(connection.flow_id, :key_share),
  ...
  defp delete_connections_in_locked_flow(flow_id, delete_fn) do
    case References.lock_active_flow_for_write(flow_id, :key_share) do
  ...
  defp validate_connection_rules(project_id, source_node, target_node, attrs) do
    ...
    with {:ok, output_pins} <-
           References.lock_effective_output_pins(project_id, source_node) do
      cond do
