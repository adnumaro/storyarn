defmodule Storyarn.Flows.SequenceCompositionHistory do
  ...
  defp restore_in_transaction(owner_id, snapshot, expected_current) do
    with {:ok, %{node: owner, flow: flow, project_id: project_id}} <-
         ...
         snapshot = preserve_legacy_tracks(snapshot, owner),
  ...
  defp ensure_expected_current(owner, expected_current) do
    ...
    current = if expected_current["tracks"] == :preserve, do: Map.put(current, "tracks", :preserve), else: current
  ...
  # Version 1 history did not capture audio. Restore its visual state without
  # clearing tracks added meanwhile; the owner lock protects this projection.
  defp preserve_legacy_tracks(%{"tracks" => :preserve} = captured, owner),
    do: Map.put(captured, "tracks", Enum.map(tracks(owner.id), &serialize_track/1))

  defp preserve_legacy_tracks(captured, _owner), do: captured
  ...
  defp normalize_snapshot(snapshot) do
    ...
    with true <- snapshot["version"] in [1, "1", @version, Integer.to_string(@version)],
         ...
         {:ok, tracks} <- normalize_snapshot_tracks(snapshot) do
      {:ok,
       %{
         "version" => @version,
         "owner_id" => owner_id,
  ...
  defp normalize_snapshot_tracks(%{"version" => version} = captured) when version in [1, "1"] do
    if Map.has_key?(captured, "tracks"),
      do: normalize_tracks(captured["tracks"]),
      else: {:ok, :preserve}
  end

  defp normalize_snapshot_tracks(captured), do: normalize_tracks(captured["tracks"])
  ...
  defp normalize_tracks(rows) when is_list(rows) do
  ...
  defp normalize_tracks(_rows), do: {:error, :invalid_composition_snapshot}
