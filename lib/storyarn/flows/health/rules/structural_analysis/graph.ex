defmodule Storyarn.Flows.StructuralAnalysis.Graph do
  ...
  defp classify_connections(nodes, connections) do
    ...
    |> Enum.reduce({[], %{}, %{}}, fn connection, {valid, invalid_outputs, invalid_inputs} ->
      ...
      valid_output? =
        Editor.valid_output_pin?(source.type, source.data, connection.source_pin)

      valid_input? = Editor.valid_input_pin?(target.type, connection.target_pin)
  ...
  defp compute_dead_end_ids(nodes, connections) do
    ...
    for n <- nodes,
        Editor.needs_outgoing_connection?(n.type),
  ...
  defp compute_isolated_ids(nodes, connections) do
    ...
    for n <- nodes,
        not Editor.connection_optional_type?(n.type),
  ...
  defp node_missing_output_pins(node, connected_output_pins) do
    if Editor.needs_outgoing_connection?(node.type) do
      connected = Map.get(connected_output_pins, node.id, MapSet.new())

      node.type
      |> Editor.output_pins(node.data)
  ...
  defp connected_output_pins(nodes, connections) do
    ...
    Enum.reduce(connections, %{}, fn connection, acc ->
      ...
      pin =
        Editor.canonical_output_pin(source.type, source.data, connection.source_pin)
