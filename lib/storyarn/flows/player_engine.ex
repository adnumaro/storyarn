defmodule Storyarn.Flows.PlayerEngine do
  @moduledoc """
  Flow-owned runtime coordinator that auto-advances the evaluator through
  non-interactive nodes (entry, hub, condition, instruction, jump)
  until it reaches a dialogue, exit, or error.

  The evaluator is a pure functional state machine. This module owns the
  player-specific policy for deciding which evaluator states are interactive.
  """
  ...
  defp do_step(%State{status: :finished} = state, _nodes, _connections, _max, _count, skipped, _advance?) do
  ...
  defp stop_at_dialogue(%{id: node_id, data: data} = node, state, skipped) do
    ...
    if length(valid_responses) > 1 do
      case Engine.step(state, %{node.id => node}, []) do
