defmodule Storyarn.Flows.ProjectReferenceIntegrity do
  @moduledoc false
  ...
  alias Storyarn.Flows.References.Data.ProjectRecord
  alias Storyarn.Flows.References.Data.SceneRecord
  ...
  defp lock_reference_ids(:flow, project_id, ids), do: lock_active_hierarchical_ids(Flow, project_id, ids)

  defp lock_reference_ids(:scene, project_id, ids), do: lock_active_hierarchical_ids(SceneRecord, project_id, ids)
  ...
  defp ensure_transaction! do
    if not Repo.in_transaction?() do
      raise ArgumentError,
            "flow project reference integrity checks require an explicit database transaction"
    end
  end
