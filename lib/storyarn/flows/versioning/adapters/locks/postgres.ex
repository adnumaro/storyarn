defmodule Storyarn.Flows.Versioning.VersionNumberLock do
  @moduledoc false
  ...
  @spec run(integer(), (-> {:ok, term()} | {:error, term()})) ::
          {:ok, term()} | {:error, term()}
  def run(flow_id, fun) when is_integer(flow_id) and flow_id > 0 and is_function(fun, 0) do
    Repo.transaction(fn ->
      Repo.query!("SELECT pg_advisory_xact_lock($1, $2)", [
        @entity_version_namespace,
        lock_key({"flow", flow_id})
      ])
