defmodule Storyarn.Flows.Versioning.SnapshotStorage do
  @moduledoc """
  ...
  defguardp valid_identity(project_id, flow_id, version_number)
  ...
  def store_snapshot(project_id, flow_id, version_number, snapshot, opts \\ []) when is_list(opts) do
    key = build_key(project_id, flow_id, version_number, unique_key_suffix())
  ...
  def build_key(project_id, flow_id, version_number, suffix)
      when is_integer(project_id) and project_id > 0 and is_integer(flow_id) and flow_id > 0 and
             is_integer(version_number) and version_number > 0 do
    if Regex.match?(~r/\A[0-9a-f]{16}\z/, suffix) do
      "projects/#{project_id}/snapshots/flow/#{flow_id}/#{version_number}-#{suffix}.json.gz"
    else
  ...
  def entity_key?(storage_key, project_id, flow_id, version_number)
      when is_binary(storage_key) and valid_identity(project_id, flow_id, version_number) do
    base = "projects/#{project_id}/snapshots/flow/#{flow_id}/#{version_number}"
    Regex.match?(~r/\A#{Regex.escape(base)}-[0-9a-f]{16}\.json\.gz\z/, storage_key)
  end

  def entity_key?(_storage_key, _project_id, _flow_id, _version_number), do: false
