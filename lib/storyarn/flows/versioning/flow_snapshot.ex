defmodule Storyarn.Flows.Versioning.FlowSnapshot do
  ...
  defp lock_localization_inventory(project_id) do
    Repo.query!(
      ...
      ["storyarn:localization:inventory", Integer.to_string(project_id)]
    )
  ...
  defp build_locked(flow) do
    ...
    with {:ok, normalized_nodes} <-
         ...
         :ok <- VariableReferenceTracker.validate_flow_node_variable_targets(normalized_nodes, flow.project_id),
  ...
  defp validate_and_normalize_node_references(nodes, project_id, scene_id) do
    ...
    with {:ok, _ids} <- ProjectReferenceIntegrity.lock_active_references(project_id, specs) do
  ...
  defp restore_transaction(flow, snapshot, opts) do
    with {:ok, _project} <- ProjectReferenceIntegrity.lock_active_project(flow.project_id, :update),
  ...
  defp validate_normalized_restore_references(snapshot, flow) do
    ...
    with {:ok, _ids} <-
      ...
      with :ok <- VariableReferenceTracker.validate_flow_node_variable_targets(normalized_nodes, project_id),
           :ok <- validate_flow_reference_cycles(flow.id, normalized_nodes),
  ...
  defp restore_node(flow_id, current_by_id, data, snapshot, project_id, opts) do
    ...
    with {:ok, node_data} <- resolve_node_asset_refs(data["data"], snapshot, project_id, opts) do
      attrs = %{
        ...
        data: node_data,
        word_count: WordCount.for_node_data(data["type"], node_data),
        parent_id: nil
      }
  ...
  defp extract_restored_localization(restored) do
    Enum.reduce_while(Map.values(restored), :ok, fn node, :ok ->
      case LocalizationProjection.extract_flow_node(node) do
        :ok -> {:cont, :ok}
        {:error, reason} -> {:halt, {:error, reason}}
      end
    end)
  ...
  defp rebuild_references(deleted_ids, restored, project_id) do
    Enum.each(deleted_ids, fn node_id ->
      EntityReferenceTracker.delete_references(node_id)
      VariableReferenceTracker.delete_references(node_id)
    end)

    Enum.reduce_while(Map.values(restored), :ok, fn node, :ok ->
      with :ok <- EntityReferenceTracker.update_references(node, project_id: project_id),
           :ok <- normalize_reference_result(VariableReferenceTracker.update_references(node)) do
        {:cont, :ok}
      else
        {:error, reason} -> {:halt, {:error, reason}}
      end
