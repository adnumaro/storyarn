defmodule Storyarn.Ideation.Ideas.Rules.Canvas do
  @moduledoc false
  # "none" keeps the default look: a yellow card, or plain text on a text-only note.
  @colors ~w(none yellow coral mint blue violet paper)
  @shapes ~w(plain rectangle ellipse diamond)

  def normalize(attrs) when is_map(attrs) do
    attrs |> Map.put_new("width", 280) |> Map.put_new("color", "none") |> validate()
  end
