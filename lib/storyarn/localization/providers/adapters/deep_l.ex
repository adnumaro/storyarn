defmodule Storyarn.Localization.Providers.DeepL do
  ...
  defp handling_mode(text) do
    if Texts.html?(text) or Texts.placeholders(text) != [], do: :html, else: :plain
  end

  defp preprocess_text(text, :html), do: Texts.pre_translate(text)
  ...
  defp validate_translations(entries, translations, mode) when length(entries) == length(translations) do
    ...
    |> Enum.reduce_while({:ok, []}, fn {entry, translation}, {:ok, acc} ->
      ...
      case Texts.validate_placeholders(entry.original, translated_text) do
        :ok ->
  ...
  defp postprocess_text(text, :html), do: Texts.post_translate(text)
