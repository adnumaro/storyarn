defmodule Storyarn.Localization.Translation.Execution.BatchTranslationJob do
  @moduledoc false
  ...
  def perform(run_id, attempt, max_attempts)
    ...
    case RunQueries.get(run_id) do
      nil -> {:discard, :translation_run_not_found}
      %{status: "cancelled"} -> :ok
      run -> execute(run, attempt >= max_attempts)
    end
  end
  ...
  defp execute(run, final_attempt?) do
    with true <- active_target?(run),
         {:ok, running} <-
           RunCommands.transition_active(run.id, %{
             status: "running",
  ...
  defp execute_running(running, final_attempt?) do
    ...
    Broadcaster.broadcast(running)

    opts = [
      status: running.text_status,
      source_type: running.source_type,
      translator: translation_provider(),
      cancelled?: fn -> RunQueries.cancelled?(running.id) end,
      progress_callback: &persist_progress(running, base_counts, &1)
    ]

    case translate_batch(running, opts) do
      {:returned, {:ok, result}} -> complete(running, base_counts, result)
      {:returned, {:error, :cancelled}} -> mark_cancelled(running)
      {:returned, {:error, reason}} -> fail(running, reason, final_attempt?)
      {:raised, exception, stacktrace} -> fail_and_reraise(running, exception, stacktrace, final_attempt?)
      {:caught, kind, reason, stacktrace} -> fail_and_raise(running, kind, reason, stacktrace, final_attempt?)
    end
  end
  ...
  defp persist_progress(run, base_counts, result) do
    case RunCommands.transition_active(run.id, merged_counts(base_counts, result)) do
      {:ok, updated} -> Broadcaster.broadcast(updated)
      {:error, :inactive} -> :ok
    end
  end

  defp complete(run, base_counts, result) do
    case RunCommands.transition_terminal(
           run.id,
      ...
      {:ok, {completed, notification_outcome}} ->
        Broadcaster.broadcast(completed)
  ...
  defp mark_cancelled(run) do
    case RunQueries.get(run.id) do
      %{status: "cancelled"} -> :ok
      current -> current |> RunCommands.cancel() |> then(fn {:ok, _run} -> :ok end)
    end
  end
  ...
  defp fail(run, reason, final_attempt?) do
    ...
    result =
      if final_attempt? do
        RunCommands.transition_terminal(run.id, attrs)
      else
        RunCommands.transition_active(run.id, attrs)
      end
  ...
  defp active_target?(run) do
    case Languages.get_language_by_locale(run.project_id, run.target_locale) do
  ...
  defp translation_provider do
    Application.get_env(:storyarn, :localization_translation_provider, Providers.default_adapter())
  end
end
