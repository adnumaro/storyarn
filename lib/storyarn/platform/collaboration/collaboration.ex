defmodule Storyarn.Platform.Collaboration do
  @moduledoc """
  The realtime Collaboration capability within Platform.
  ...
  alias Storyarn.Platform.Collaboration.Colors
  alias Storyarn.Platform.Collaboration.CursorTracker
  ...
  @doc """
  Unsubscribes the current process from presence updates for an editor.
  """
  def unsubscribe_presence(scope) do
    topic = Presence.topic(normalize_scope(scope))
    PubSub.unsubscribe(Storyarn.PubSub, "proxy:#{topic}")
  end

  @doc """
  Stops tracking a user's presence without exposing the Phoenix.Presence
  adapter or its topic format to presentation code.
  """
  def untrack_presence(pid, scope, user_id) do
    topic = Presence.topic(normalize_scope(scope))
  ...
  """
  defdelegate broadcast_dashboard_change(project_id, scope), to: Cache

  @doc """
  Broadcasts a dashboard change after a successful operation and returns the
  operation result unchanged.
  """
  defdelegate broadcast_dashboard_result(result, project_id, scope), to: Cache

  @doc false
  defdelegate dashboard_topic(project_id), to: Cache
end
