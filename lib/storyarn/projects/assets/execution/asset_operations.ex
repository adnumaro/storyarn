defmodule Storyarn.Projects.Assets.AssetOperations do
  @moduledoc """
  Transactional execution kernel for Project assets.

  It keeps the lock ordering, database transactions, storage compensations and
  post-commit work that must move together. Public callers enter through
  `Storyarn.Projects.Assets`, whose use-case delegates keep this module private.
  """
  ...
  alias Storyarn.Projects.Assets.Persistence.EntityTrashRefRecord, as: EntityTrashRef
  ...
  Delegates to `Storyarn.Projects.Assets.Asset.display_url/1`.
  """
  ...
  Delegates to `Storyarn.Projects.Assets.Asset.image?/1`.
  """
  ...
  Delegates to `Storyarn.Projects.Assets.Asset.audio?/1`.
  """
  ...
  Delegates to `Storyarn.Projects.Assets.Asset.allowed_content_type?/1`.
  """
  ...
  Delegates to `Storyarn.Projects.Assets.Storage.delete/1`.
  """
  ...
  Delegates to `Storyarn.Projects.Assets.Storage.download/1`.
  """
  ...
  Delegates to `Storyarn.Projects.Assets.ImageProcessor.available?/0`.
  """
  ...
  Delegates to `Storyarn.Projects.Assets.ImageProcessor.get_dimensions/1`.
  """
  ...
  # Project-facing capability entry points. They keep the root facade from
  # coupling to storage policy, blob storage, or Versioning internals.
  ...
  def upload_asset(path, entry, project_id, uploaded_by_id, opts \\ []) do
    upload_and_create_asset(path, entry, project_id, uploaded_by_id, opts)
  end
  ...
  def create_binary_asset(project_id, binary, attrs, uploaded_by_id \\ nil) do
    upload_binary_and_create_asset(binary, attrs, project_id, uploaded_by_id)
  end

  def create_sanitized_svg_asset(project_id, binary, attrs, uploaded_by_id \\ nil) do
    upload_sanitized_svg_and_create_asset(binary, attrs, project_id, uploaded_by_id)
  end
