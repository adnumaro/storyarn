defmodule Storyarn.Projects.Persistence.SequenceConfigRecord do
  @moduledoc """
  Per-sequence configuration. 1:1 with the Project-owned Flow node record where
  `type='sequence'`.
  ...
  `Storyarn.Projects.Persistence.SequenceVisualLayerRecord`).

  Audio tracks live in the separate `flow_node_sequence_tracks` table
  (schema `Storyarn.Projects.Persistence.SequenceTrackRecord`). See its moduledoc for
  rationale.
  """
  use Ecto.Schema
  ...
  schema "flow_node_sequence_configs" do
    belongs_to :flow_node, FlowNodeRecord, primary_key: true
