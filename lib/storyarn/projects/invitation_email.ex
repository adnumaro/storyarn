defmodule Storyarn.Projects.InvitationEmail do
  @moduledoc """
  Project-owned invitation email content.

  Projects owns the invitation intent, copy, and localization. Platform only
  ...
  @doc "Renders a project invitation email."
  def render(project_name, inviter_name, role, url, days) do
    subject =
    ...
    <mj-text>
      #{dgettext("emails", "<strong>%{inviter}</strong> has invited you to join <strong>%{project}</strong> on Storyarn as <strong>%{role}</strong>.", inviter: Layout.escape(inviter_name), project: Layout.escape(project_name), role: Layout.escape(role))}
    </mj-text>
    <mj-text>
