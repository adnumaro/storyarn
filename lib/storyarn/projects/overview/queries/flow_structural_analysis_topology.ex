defmodule Storyarn.Projects.FlowStructuralAnalysis.Topology do
  @moduledoc """
  ...
  Unlike the editor serializer it loads none of the editorial
  ...
  alias Storyarn.Projects.FlowReferenceResolver
  ...
  """
  @spec from_loaded(FlowRecord.t()) :: t()
  def from_loaded(%FlowRecord{} = flow) do
  ...
  """
  @spec from_loaded_many([FlowRecord.t()]) :: [t()]
  ...
  def from_loaded_many(flows) when is_list(flows) do
    ...
    |> Enum.flat_map(fn {project_id, project_flows} ->
      ...
      subflow_cache = FlowReferenceResolver.batch_resolve_subflow_data(all_nodes, project_id)
      exit_cache = FlowReferenceResolver.batch_resolve_exit_data(all_nodes, project_id)
  ...
  @doc """
  Builds the topology from serialized Flow output — node data
  ...
  def load_project(project_id, opts \\ []) do
    ...
    connections_by_flow =
      from(c in FlowConnectionRecord,
  ...
  defp resolve_nodes(nodes, project_id) do
    resolve_nodes(
      nodes,
      project_id,
      FlowReferenceResolver.batch_resolve_subflow_data(nodes, project_id),
      FlowReferenceResolver.batch_resolve_exit_data(nodes, project_id)
    )
  end
  ...
  defp resolve_nodes(nodes, project_id, subflow_cache, exit_cache) do
    Enum.map(nodes, fn
      %{type: "subflow"} = node ->
        %{node | data: FlowReferenceResolver.resolve_subflow_data(node.data, subflow_cache)}

      %{type: "exit"} = node ->
        %{node | data: FlowReferenceResolver.resolve_exit_data(node.data, project_id, exit_cache)}

      # A sequence node is a visual container. The editor serializer
