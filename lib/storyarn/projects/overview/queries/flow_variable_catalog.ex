defmodule Storyarn.Projects.FlowVariableCatalog do
  @moduledoc """
  Flow-owned read model for every variable addressable by conditions and
  instructions.

  It reads the shared persistence tables through records owned by Project and
  emits the runtime descriptor shape consumed by the editor and evaluator.
  """

  import Ecto.Query

  alias Storyarn.Projects.FlowVariableConstraints
  ...
  defp decorate_variable(variable) do
    ...
    variable
    |> Map.put(:constraints, FlowVariableConstraints.extract(variable.block_type, variable.config))
