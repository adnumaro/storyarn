defmodule Storyarn.Projects.SceneVariableCatalog do
  @moduledoc """
  Scene-owned read model for every variable addressable by conditions and
  instructions.

  It reads the shared persistence tables through Project-owned records and
  ...
  alias Storyarn.Projects.SceneVariableConstraints
  alias Storyarn.Projects.SceneVariableNamespaceResolver
  alias Storyarn.Repo

  require SceneVariableNamespaceResolver
    ...
    from(block in BlockRecord,
      ...
      where:
        sheet.project_id == ^project_id and is_nil(sheet.deleted_at) and
          ...
          SceneVariableNamespaceResolver.authoritative_namespace_owner?(sheet),
  ...
  defp decorate_variable(variable) do
    ...
    variable
    |> Map.put(:constraints, SceneVariableConstraints.extract(variable.block_type, variable.config))
