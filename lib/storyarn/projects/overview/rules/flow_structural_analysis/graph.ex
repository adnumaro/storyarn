defmodule Storyarn.Projects.FlowStructuralAnalysis.Graph do
  @moduledoc """
  ...
  `Storyarn.Projects.FlowStructuralAnalysis.Topology`.
  """
  ...
  defp classify_connections(nodes, connections) do
    ...
    |> Enum.reduce({[], %{}, %{}}, fn connection, {valid, invalid_outputs, invalid_inputs} ->
      ...
      valid_output? =
        FlowNodeConnectionRules.valid_output_pin?(source.type, source.data, connection.source_pin)

      valid_input? = FlowNodeConnectionRules.valid_input_pin?(target.type, connection.target_pin)
  ...
  defp compute_dead_end_ids(nodes, connections) do
    ...
    for n <- nodes,
        FlowNodeConnectionRules.needs_outgoing_connection?(n.type),
  ...
  defp compute_isolated_ids(nodes, connections) do
    ...
    for n <- nodes,
        not FlowNodeConnectionRules.connection_optional_type?(n.type),
  ...
  defp node_missing_output_pins(node, connected_output_pins) do
    if FlowNodeConnectionRules.needs_outgoing_connection?(node.type) do
      connected = Map.get(connected_output_pins, node.id, MapSet.new())

      node.type
      |> FlowNodeConnectionRules.output_pins(node.data)
  ...
  defp connected_output_pins(nodes, connections) do
    ...
    Enum.reduce(connections, %{}, fn connection, acc ->
      ...
      pin =
        FlowNodeConnectionRules.canonical_output_pin(source.type, source.data, connection.source_pin)
