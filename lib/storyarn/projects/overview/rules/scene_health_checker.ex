defmodule Storyarn.Projects.SceneHealthChecker do
  @moduledoc """
  ...
  alias Storyarn.Projects.SceneCondition
  ...
  defp zone_action_findings(scene, zone, snapshot) do
    ...
    |> maybe_add(
      configuration_valid? and action_type == "action" and
        ...
        not SceneInstruction.has_assignments?(Map.get(action_data, "assignments")),
  ...
  defp invalid_connection_route?(connection, from_id, to_id, waypoints) do
    not is_list(waypoints) or length(waypoints) > @max_waypoints or
      (is_list(waypoints) and not Enum.all?(waypoints, &SceneRoutePoints.valid_waypoint?/1)) or
      (is_list(waypoints) and not SceneRoutePoints.enough_points?(from_id, to_id, waypoints)) or
  ...
  defp assignment_findings(scene, entity, entity_type, assignments, snapshot, extra_details \\ %{}) do
    ...
    incomplete? = assignments != [] and Enum.any?(assignments, &(not SceneInstruction.complete_assignment?(&1)))

    stale_refs =
      if references_loaded?(snapshot), do: stale_assignment_refs(assignments, variable_types(snapshot)), else: []

    # The PREPARED map, not the variable list: `SceneInstruction.has_type_warnings?/2`
    ...
    type_warning? = SceneInstruction.has_type_warnings?(assignments, variable_types(snapshot))
  ...
  defp condition_findings(scene, entity, entity_type, condition, snapshot, extra_details \\ %{}) do
    rules = SceneCondition.extract_all_rules(condition)

    cond do
      is_nil(condition) ->
        []

      match?({:error, _}, SceneCondition.validate(condition)) ->
  ...
  defp complete_condition_rule?(rule) do
    ...
    not is_nil(ref) and operator in @valid_condition_operators and
      (not SceneCondition.operator_requires_value?(operator) or not blank?(field(rule, :value)))
  end
  ...
  defp condition_type_mismatch?(rules, types) do
    Enum.any?(rules, fn rule ->
      ...
      case Map.get(types, ref) do
        nil -> false
        type -> operator in @valid_condition_operators and operator not in SceneCondition.operators_for_type(type)
      end
    end)
  end
  ...
  defp variable_types(snapshot) do
    Map.get_lazy(snapshot, :variable_types, fn ->
      snapshot |> Map.get(:project_variables, []) |> SceneInstruction.variable_type_map()
    end)
  end
