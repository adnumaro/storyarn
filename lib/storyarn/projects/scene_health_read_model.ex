defmodule Storyarn.Projects.SceneHealthReadModel do
  @moduledoc """
  Builds the enriched snapshots `Storyarn.Projects.SceneHealthChecker` consumes, and is
  ...
  alias Storyarn.Projects.SceneHealthChecker
  alias Storyarn.Projects.SceneInstruction
  alias Storyarn.Projects.SceneReadModel
  alias Storyarn.Projects.SceneVariableCatalog
  alias Storyarn.Repo
  ...
  """
  @spec findings(map(), collections(), references()) :: [SceneHealthChecker.finding()]
  def findings(scene, collections, references) do
    scene |> build(collections, references) |> SceneHealthChecker.check()
  end
  ...
  defp build(scene, collections, references) do
    ...
    %{
      ...
      # `SceneInstruction.has_type_warnings?/2` now takes the prepared map for the
      # same reason. `SceneInstruction.variable_type_map/1` is the one builder — the
      # key format decides whether a reference is stale, so both scene surfaces
      # and the flow checkers must spell it identically.
      variable_types: SceneInstruction.variable_type_map(references.variables)
    }
  end
  ...
  def load_project(project_id) do
    scenes = SceneReadModel.list_active(project_id)
  ...
  def list_dashboard_findings(project_id) do
    project_id
    |> load_project()
    |> Enum.flat_map(fn entry ->
      entry.scene
      |> findings(entry.collections, entry.references)
  ...
  defp load_references(project_id, scenes, flow_names) do
    references(%{
      ...
      asset_ids: ProjectAssets.list_image_asset_ids(project_id),
      ...
      variables: SceneVariableCatalog.list_referenceable(project_id)
    })
  end
  ...
  defp load_collections(scene_ids) do
    %{
      layers: grouped(SceneLayerRecord, scene_ids, asc: :position, asc: :id),
      zones: grouped(SceneZoneRecord, scene_ids, asc: :position, asc: :id),
      pins: grouped(ScenePinRecord, scene_ids, asc: :position, asc: :id),
      connections: grouped(SceneConnectionRecord, scene_ids, asc: :id),
      annotations: grouped(SceneAnnotationRecord, scene_ids, asc: :position, asc: :id),
      ambient_flows: grouped(SceneAmbientFlowRecord, scene_ids, asc: :position, desc: :priority, asc: :id)
    }
  end
