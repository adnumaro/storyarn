defmodule Storyarn.Projects.ProjectTemplates.PublicationRunner do
  @moduledoc false
  ...
  def create_template_from_project(%{user: _} = scope, %Project{} = source_project, attrs) do
    with :ok <- Authorization.ensure_private_visibility(attrs),
         {:ok, source_project} <- Authorization.authorize_source_project(scope, source_project),
         :ok <- Commercial.can_create_project_template?(source_project),
         {:ok, publication} <-
           source_project
  ...
  def publish_new_version(%{user: _} = scope, %ProjectTemplate{} = template, %Project{} = source_project) do
    with :ok <- Authorization.authorize_template_manager(scope, template),
         {:ok, source_project} <- Authorization.authorize_source_project(scope, source_project),
         :ok <- Authorization.ensure_template_source(template, source_project),
         :ok <- Commercial.can_create_project_template_version?(template),
         {:ok, publication} <-
           template
  ...
  defp authorize_publication_template_for_worker(scope, %ProjectTemplatePublication{mode: "new"}, source_project) do
    case Commercial.can_create_project_template?(source_project) do
      :ok ->
  ...
  defp cleanup_publication_artifacts!(keys) do
    case cleanup_publication_artifacts(keys) do
      ...
      {:error, reason} ->
        raise Storyarn.Projects.Assets.StorageCleanupPersistenceError,
