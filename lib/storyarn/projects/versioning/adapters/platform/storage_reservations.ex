defmodule Storyarn.Projects.PlatformStorageReservations do
  @moduledoc """
  Anti-corruption layer between Project snapshot workflows and Platform's
  authoritative storage-reservation writer.

  Platform exposes transport-neutral receipts through its public facade. This
  module translates those receipts into the Project-owned read model used by
  snapshot and restore code; a Platform Ecto schema never crosses the boundary.
  """
  ...
  def reserve(attrs), do: attrs |> Platform.reserve_project_storage() |> map_reservation_result()

  @spec acquire_snapshot_export_lease(map()) ::
          reservation_result()
  def acquire_snapshot_export_lease(attrs) do
    attrs
    |> Platform.acquire_project_snapshot_export_lease()
  ...
  def renew_live(reservation_id, lease_token, expected_generation) do
    reservation_id
    |> Platform.renew_project_storage_reservation(
      lease_token,
      expected_generation
    )
    |> map_reservation_result()
  ...
  def extend(reservation_id, lease_token, expected_generation, target_bytes) do
    reservation_id
    |> Platform.extend_project_storage_reservation(
      lease_token,
      expected_generation,
      target_bytes
    )
  ...
  def mark_started(reservation_id, lease_token, expected_generation, cleanup_plan) do
    reservation_id
    |> Platform.mark_project_storage_reservation_started(
      lease_token,
      expected_generation,
      cleanup_plan
    )
  ...
  def commit(reservation_id, lease_token, expected_generation, actual_bytes, owner_fun) when is_function(owner_fun, 1) do
    reservation_id
    |> Platform.commit_project_storage_reservation(
      lease_token,
      expected_generation,
      actual_bytes,
  ...
  def commit_restore(reservation_id, lease_token, expected_generation, actual_bytes, prelock_fun, owner_fun)
    ...
    reservation_id
    |> Platform.commit_project_snapshot_restore_storage_reservation(
      lease_token,
      expected_generation,
      actual_bytes,
      prelock_fun,
  ...
  def release(reservation_id, lease_token, expected_generation, attrs) do
    reservation_id
    |> Platform.release_project_storage_reservation(
      lease_token,
      expected_generation,
      attrs
    )
  ...
  def object_prefixes(%StorageReservationRecord{} = reservation) do
    reservation
    |> to_receipt()
    |> Platform.project_storage_reservation_object_prefixes()
  end
