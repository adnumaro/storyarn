defmodule Storyarn.Projects.Versioning do
  ...
  # Root-facade entry points retain the exact ID validation, authorization and
  # fallback semantics that Storyarn.Projects exposed before this reorganization.
  @doc false
  def request_authorized_workspace_snapshot_import(current_scope, workspace_id, uploaded_path, attrs)
      when is_integer(workspace_id) and is_binary(uploaded_path) and is_map(attrs) do
    with_authorized_workspace(current_scope, workspace_id, :access_workspace_settings, fn workspace ->
      WorkspaceSnapshotImports.request(current_scope, workspace, uploaded_path, attrs)
    end)
  end

  def request_authorized_workspace_snapshot_import(_scope, _workspace_id, _path, _attrs),
    do: {:error, :invalid_snapshot_import_request}

  @doc false
  def prepare_authorized_external_workspace_snapshot_import(current_scope, workspace_id, attrs)
      when is_integer(workspace_id) and is_map(attrs) do
    with_authorized_workspace(current_scope, workspace_id, :access_workspace_settings, fn workspace ->
      WorkspaceSnapshotImports.prepare_external_upload(current_scope, workspace, attrs)
    end)
  end

  def prepare_authorized_external_workspace_snapshot_import(_scope, _workspace_id, _attrs),
    do: {:error, :invalid_snapshot_import_request}

  @doc false
  def request_authorized_stored_workspace_snapshot_import(current_scope, workspace_id, import_id)
      when is_integer(workspace_id) and is_integer(import_id) do
    with_authorized_workspace(current_scope, workspace_id, :access_workspace_settings, fn workspace ->
      WorkspaceSnapshotImports.request_stored(current_scope, workspace, import_id)
    end)
  end

  def request_authorized_stored_workspace_snapshot_import(_scope, _workspace_id, _import_id),
    do: {:error, :invalid_snapshot_import_request}

  @doc false
  def update_authorized_workspace_snapshot_upload_progress(current_scope, workspace_id, import_id, percent)
      when is_integer(workspace_id) and is_integer(import_id) and is_integer(percent) do
    with_authorized_workspace(current_scope, workspace_id, :access_workspace_settings, fn workspace ->
      WorkspaceSnapshotImports.upload_progress(current_scope, workspace, import_id, percent)
    end)
  end

  def update_authorized_workspace_snapshot_upload_progress(_scope, _workspace_id, _import_id, _percent),
    do: {:error, :workspace_snapshot_upload_not_found}

  @doc false
  def cancel_authorized_workspace_snapshot_upload(current_scope, workspace_id, import_id)
      when is_integer(workspace_id) and is_integer(import_id) do
    with_authorized_workspace(current_scope, workspace_id, :access_workspace_settings, fn workspace ->
      WorkspaceSnapshotImports.cancel_upload(current_scope, workspace, import_id)
    end)
  end

  def cancel_authorized_workspace_snapshot_upload(_scope, _workspace_id, _import_id),
    do: {:error, :workspace_snapshot_upload_not_found}

  @doc false
  def list_authorized_workspace_snapshot_imports(current_scope, workspace_id) when is_integer(workspace_id) do
    case with_authorized_workspace(current_scope, workspace_id, :access_workspace_settings, fn workspace ->
           WorkspaceSnapshotImports.list(current_scope, workspace)
         end) do
  ...
  def list_authorized_workspace_snapshot_imports(_scope, _workspace_id), do: []
  ...
  defdelegate run_asset_materialization_scope(opts, fun), to: AssetMaterializationScope, as: :run
  defdelegate with_asset_copy_tracker(opts, fun), to: MaterializationHelpers
  defdelegate with_project_storage_lock(project_id, fun), to: MaterializationHelpers

  defp with_authorized_workspace(scope, workspace_id, action, callback) when is_function(callback, 1) do
    with {:ok, workspace, _membership} <- Access.authorize_workspace(scope, workspace_id, action) do
      callback.(workspace)
    end
  end
end
