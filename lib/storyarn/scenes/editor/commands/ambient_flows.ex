defmodule Storyarn.Scenes.Editor.Commands.AmbientFlows do
  @moduledoc false
    ...
    |> ReferenceIntegrity.with_active_scene_lock(fn scene ->
      with {:ok, flow_id} <- requested_flow_id(attrs, nil),
           ...
           {:ok, ambient_flow} <-
             ...
             |> Repo.insert(),
           :ok <-
             References.update_ambient_flow_variable_references(
               ambient_flow,
  ...
  def delete_ambient_flow(%SceneAmbientFlow{} = ambient_flow) do
    ...
    |> ReferenceIntegrity.with_active_scene_lock(fn scene ->
      with {:ok, locked_ambient_flow} <-
           ...
           :ok <-
             References.delete_ambient_flow_variable_references(locked_ambient_flow.id) do
  ...
  def reorder_ambient_flows(scene_id, ordered_ids) when is_list(ordered_ids) do
    ...
    |> ReferenceIntegrity.with_active_scene_lock(fn scene ->
      with {:ok, normalized_ids} <- normalize_reorder_ids(ordered_ids),
        ...
        {:ok, {AmbientFlowQueries.list_ambient_flows(scene.id), scene.project_id, changed?}}
      end
    end)
    |> broadcast_ambient_flow_result()
  end
