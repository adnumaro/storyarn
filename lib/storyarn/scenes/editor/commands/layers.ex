defmodule Storyarn.Scenes.Editor.Commands.Layers do
  @moduledoc false
  ...
  def create_layer(scene_id, attrs) do
    ...
    |> ReferenceIntegrity.with_active_scene_lock(fn scene ->
      position = Positions.next_position(SceneLayer, scene.id)
  ...
  def update_layer(%SceneLayer{} = layer, attrs) do
    ...
    |> Scenes.broadcast_scene_dashboard_result(layer.scene_id)
  end
  ...
  def toggle_layer_visibility(%SceneLayer{} = layer) do
    ReferenceIntegrity.with_active_scene_lock(layer.scene_id, fn scene ->
      with {:ok, locked_layer} <- lock_layer_for_scene(layer.id, scene.id) do
        locked_layer
  ...
  def reorder_layers(scene_id, layer_ids) when is_list(layer_ids) do
    ReferenceIntegrity.with_active_scene_lock(scene_id, fn scene ->
      with {:ok, normalized_ids} <-
             normalize_reorder_ids(layer_ids),
           :ok <- lock_requested_layers(scene.id, normalized_ids) do
        PositionPersistence.set_layer_positions(Enum.with_index(normalized_ids), scene.id)

        {:ok, LayerQueries.list_layers(scene.id)}
      end
    end)
  end
