defmodule Storyarn.Scenes.Editor.Commands.ReferenceIntegrity do
  @moduledoc """
  ...
  """
  @spec lock_active_project(term(), project_lock()) :: {:ok, map()} | {:error, term()}
  defdelegate lock_active_project(project_id, lock_mode \\ :share), to: References
  ...
  def lock_scene_root_references(%Scene{} = scene, attrs) when is_map(attrs) do
    ...
    with {:ok, [parent_id, background_asset_id]} <-
           References.lock_active_references(scene.project_id, specs),
         :ok <-
