defmodule Storyarn.Scenes.FlowRuntime.InstructionExec do
  @moduledoc """
  Executes instruction assignments against the current variable state.

  The "write" side of the evaluator — mirrors `Storyarn.Scenes.Instruction`
  ...
  def execute(assignments, variables) when is_list(assignments) do
    assignments
    |> Enum.filter(&Expressions.instruction_complete?/1)
  ...
  def execute_string(json_string, variables) when is_binary(json_string) do
    case Jason.decode(json_string) do
      {:ok, assignments} when is_list(assignments) ->
        execute(Expressions.sanitize_instructions(assignments), variables)
