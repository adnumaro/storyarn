defmodule Storyarn.Scenes.Health.Queries.Snapshots do
  @moduledoc """
  Builds the enriched snapshots `Storyarn.Scenes.Health.Rules.Checker` consumes, and is
  ...
  """
  @spec findings(map(), collections(), references()) :: [Checker.finding()]
  def findings(scene, collections, references) do
    scene |> build(collections, references) |> Checker.check()
  end
  ...
  def load_project(project_id) do
    scenes = Editor.list_scenes(project_id)
