defmodule Storyarn.Scenes.References.Queries.VariableNamespaces do
  @moduledoc false
    ...
    quote do
      fragment(
        ...
        (? IS NOT NULL OR NOT EXISTS (
          SELECT 1
          FROM sheets AS scene_variable_namespace_owner
          WHERE scene_variable_namespace_owner.project_id = ?
            AND scene_variable_namespace_owner.deleted_at IS NULL
            AND scene_variable_namespace_owner.shortcut = CAST(? AS TEXT)
        ))
        """,
