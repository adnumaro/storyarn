defmodule Storyarn.Scenes.Versioning.AssetHashResolver do
  @moduledoc """
  ...
  alias Storyarn.Scenes.Versioning.AssetBlobKeys, as: BlobStore
  alias Storyarn.Scenes.Versioning.AssetCopyError
  alias Storyarn.Scenes.Versioning.AssetMaterializationCache
  alias Storyarn.Scenes.Versioning.AssetPolicy
  alias Storyarn.Scenes.Versioning.MaterializationHelpers
  alias Storyarn.Scenes.Versioning.SnapshotMetadata, as: SnapshotObjectFormat

  @sha256_regex ~r/\A[0-9a-f]{64}\z/
  @max_asset_size 52_428_800
  @relationship_metadata_keys ~w(original_asset_id web_asset_id variant_asset_ids)
  @exact_restore_contract_key "asset_restore_contract_version"
  @exact_restore_contract_version 1

  @doc false
  def exact_restore_contract_version, do: @exact_restore_contract_version
  ...
  defp preload_materialized_asset(snapshot, source_asset_id, destination_asset_id, project_id, cache)
    ...
    if exact_restore_contract?(snapshot) do
      AssetMaterializationCache.put(
        ...
        project_id,
        source_asset_id,
        exact_pre_materialized_fingerprint(source_asset_id),
        :copy,
        destination_asset_id
      )
    else
  ...
  defp resolve_pre_materialized_asset(asset_id, snapshot, project_id, opts) do
    ...
    case result do
      ...
      {:error, :limit_reached, details} ->
        raise AssetCopyError, asset_id: asset_id, reason: {:limit_reached, details}
    end
  end

  defp pre_materialized_asset_entry_for_restore(asset_id, snapshot, opts) do
    if MaterializationHelpers.exact_materialization?(opts) do
      {:ok,
       %{
         metadata: %{},
         fingerprint: exact_pre_materialized_fingerprint(asset_id)
       }}
    else
      fetch_pre_materialized_asset_entry(asset_id, snapshot)
    end
  end
  ...
  defp validate_expected_content_type(asset_id, entry, opts) do
    case Keyword.get(opts, :expected_content_type_prefix) do
      ...
      prefix when is_binary(prefix) ->
        if MaterializationHelpers.exact_materialization?(opts) or
             String.starts_with?(entry.metadata["content_type"], prefix) do
          :ok
        else
          {:error,
  ...
  defp validate_asset_filename(filename) when is_binary(filename) do
    if String.valid?(filename) and String.trim(filename) != "" and
         valid_sanitized_filename_segment?(AssetPolicy.sanitize_filename(filename)) do
      :ok
    else
  ...
  defp validate_asset_content_type(%{"content_type" => content_type}, _opts) do
    if AssetPolicy.allowed_content_type?(content_type),
      do: :ok,
