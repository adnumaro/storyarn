defmodule Storyarn.Scenes.Versioning.AssetStorageCompensation do
  @moduledoc false
  ...
  alias Storyarn.Scenes.Persistence.StorageCleanupRequestRecord
  alias Storyarn.Scenes.Persistence.StorageReservationRecord
  ...
  def cleanup_after_rollback!(reference, opts \\ []) when is_reference(reference) and is_list(opts) do
    case cleanup_after_rollback(reference, opts) do
      :ok -> :ok
      {:error, reason} -> raise "Scene asset rollback cleanup failed: #{inspect(reason)}"
    end
  end
  ...
  defp persist_cleanup_request(storage_keys) do
    ...
    |> StorageCleanupRequestRecord.scene_restore_changeset(storage_keys)
    |> Repo.insert()
  end
