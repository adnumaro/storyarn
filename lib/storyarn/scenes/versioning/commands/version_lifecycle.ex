defmodule Storyarn.Scenes.Versioning.Commands.VersionLifecycle do
  @moduledoc false
  ...
  alias Storyarn.Scenes.Versioning.Adapters.Locks.Postgres, as: VersionNumberLock
  ...
  defp validate_scene_scope(%Scene{id: id, project_id: project_id}, project_id)
    ...
    case Repo.get(Scene, id) do
      ...
      _scene -> {:error, :entity_scope_mismatch}
    end
  end

  defp validate_scene_scope(_scene, _project_id), do: {:error, :entity_scope_mismatch}
  ...
  defp maybe_create_version_if_due(scene, project_id, user_id, opts, min_interval) do
    if version_due?(History.get_latest_version(scene.id), min_interval) do
      snapshot = SceneSnapshot.build(scene)
      ...
      |> VersionNumberLock.run(fn ->
        create_version_if_due_locked(
          ...
          opts,
          min_interval,
          History.get_latest_version(scene.id)
        )
      end)
  ...
  defp change_data(scene_id, snapshot, opts) do
    if Keyword.get(opts, :skip_diff, false) do
      ...
      case History.get_latest_version(scene_id) do
        nil ->
