defmodule Storyarn.Scenes.Versioning.SceneSnapshot do
  @moduledoc """
  ...
  alias Ecto.Multi
  alias Storyarn.Repo
  alias Storyarn.Scenes.Persistence.AssetRecord, as: Asset
  alias Storyarn.Scenes.Persistence.FlowRecord, as: Flow
  alias Storyarn.Scenes.Persistence.ProjectRecord, as: Project
  alias Storyarn.Scenes.Persistence.SheetRecord, as: Sheet
  alias Storyarn.Scenes.RoutePoints, as: RoutePoints
  alias Storyarn.Scenes.Scene, as: Scene
  alias Storyarn.Scenes.SceneAmbientFlow, as: SceneAmbientFlow
  alias Storyarn.Scenes.SceneAnnotation, as: SceneAnnotation
  alias Storyarn.Scenes.SceneConnection, as: SceneConnection
  alias Storyarn.Scenes.SceneLayer, as: SceneLayer
  alias Storyarn.Scenes.ScenePin, as: ScenePin
  alias Storyarn.Scenes.SceneZone, as: SceneZone
  alias Storyarn.Scenes.Versioning.AssetHashResolver
  alias Storyarn.Scenes.Versioning.AssetMaterializationScope
  alias Storyarn.Scenes.Versioning.DiffHelpers
  alias Storyarn.Scenes.Versioning.EntityRestoreSafety
  alias Storyarn.Scenes.Versioning.MaterializationHelpers
  alias Storyarn.Scenes.Versioning.ReferenceProjection, as: References
  ...
  def restore_snapshot(scene, snapshot, opts \\ [])
  ...
  @doc false
  # Collects only the collections variable validation reads, so a snapshot
  # missing restore-only keys (connections, annotations) still validates —
  # restore previews must accept every snapshot the pre-migration validator
  # accepted.
  def validate_variable_references(snapshot, project_id)
      when is_map(snapshot) and is_integer(project_id) and project_id > 0 do
    with {:ok, layers} <- variable_reference_collection(snapshot, "scene", "layers"),
         {:ok, layer_pins} <- layer_variable_sources(layers, "pins", "scene_pin"),
         {:ok, layer_zones} <- layer_variable_sources(layers, "zones", "scene_zone"),
         {:ok, orphan_pins} <- variable_reference_collection(snapshot, "scene", "orphan_pins"),
         {:ok, orphan_zones} <- variable_reference_collection(snapshot, "scene", "orphan_zones"),
         {:ok, ambient_flows} <- variable_reference_collection(snapshot, "scene", "ambient_flows") do
      sources =
        layer_pins ++
          layer_zones ++
          Enum.map(orphan_pins, &scene_snapshot_variable_source("scene_pin", {&1, nil})) ++
          Enum.map(orphan_zones, &scene_snapshot_variable_source("scene_zone", {&1, nil})) ++
  ...
  def validate_variable_references(_snapshot, _project_id), do: {:error, :invalid_scene_snapshot}

  defp variable_reference_collection(snapshot, entity_type, key) do
    case Map.fetch(snapshot, key) do
  ...
  defp layer_variable_sources(layers, child_key, source_type) do
    Enum.reduce_while(layers, {:ok, []}, fn layer, {:ok, sources} ->
      case variable_reference_collection(layer, "scene_layer", child_key) do
        {:ok, children} ->
          {:cont, {:ok, sources ++ Enum.map(children, &scene_snapshot_variable_source(source_type, {&1, nil}))}}
  ...
  @doc false
  def build(scene), do: build_snapshot(scene)

  @doc false
  def restore(scene, snapshot, opts \\ []), do: restore_snapshot(scene, snapshot, opts)

  @doc false
  def diff(old_snapshot, new_snapshot), do: diff_snapshots(old_snapshot, new_snapshot)
end
