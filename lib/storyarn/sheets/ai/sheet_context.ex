defmodule Storyarn.Sheets.AI.SheetContext do
  ...
  def build(project, subject_ref, policy, entity_builder) do
    case ContextQueries.get_sheet_brief(project.id, subject_ref.subject_id) do
      nil ->
  ...
  defp block_entities(project_id, sheet_id, block_ids, policy, entity_builder) do
    blocks = ContextQueries.list_blocks(project_id, sheet_id, block_ids, policy.max_entities + 1)
  ...
  defp load_reference_targets(project_id, targets, limit) do
    ...
    %{
      "sheet" =>
        project_id
        |> ContextQueries.list_sheet_briefs(sheet_ids, limit)
        |> Map.new(&{&1.id, &1}),
      "flow" =>
        project_id
