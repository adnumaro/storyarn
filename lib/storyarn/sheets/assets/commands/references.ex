defmodule Storyarn.Sheets.Assets.Commands.References do
  @moduledoc false
  ...
  alias Storyarn.Sheets.References, as: SheetReferences
  ...
  def lock_active_for_restore(project_id, owner_ids)
    ...
    with {:ok, normalized} <- normalize_owner_ids(owner_ids),
         ...
         {:ok, _asset_ids} <- SheetReferences.lock_active_references(project_id, specs) do
      :ok
    end
  end
