defmodule Storyarn.Sheets.Editor.Commands.Blocks do
  @moduledoc false
  ...
  def create_block(%Sheet{} = sheet, attrs) do
    ...
    word_count = Localization.word_count_for_block(block_type, value)
  ...
  defp put_block_word_count(changeset) do
    ...
    Ecto.Changeset.put_change(changeset, :word_count, Localization.word_count_for_block(type, value))
  end
  ...
  defp validate_and_normalize_block_references!(changeset, project_id) do
    ...
    case VariableUsage.lock_and_normalize_block_value(project_id, type, value) do
      {:ok, normalized_value} ->
  ...
  defp lock_active_project!(project_id) do
    case VariableUsage.lock_active_project(project_id, :update) do
      {:ok, _project} -> :ok
      {:error, reason} -> Repo.rollback(reason)
    end
  end
  ...
  def update_block_value(%Block{} = block, value) do
    fn ->
      ...
      value =
        case VariableUsage.lock_and_normalize_block_value(
               sheet.project_id,
               block.type,
               value
             ) do
      ...
      word_count = Localization.word_count_for_block(block.type, value)
  ...
  defp maybe_update_block_references(%Block{} = block, project_id) do
    VariableUsage.update_block_references(block, project_id: project_id)
  end
  ...
  defp insert_block_from_snapshot(sheet, snapshot, attrs) do
    ...
    changeset =
      ...
      |> Ecto.Changeset.put_change(
        :word_count,
        Localization.word_count_for_block(snapshot.type, attrs.value)
      )
      |> ensure_unique_variable_name(sheet.id, nil)
  ...
  def duplicate_block(%Block{} = block) do
    fn ->
      ...
      word_count = Localization.word_count_for_block(block.type, block.value)
  ...
  def reorder_blocks(sheet_id, block_ids) when is_integer(sheet_id) and sheet_id > 0 and is_list(block_ids) do
    fn ->
      ...
      Positions.batch_set("blocks", pairs,
