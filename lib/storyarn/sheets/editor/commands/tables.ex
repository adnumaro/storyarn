defmodule Storyarn.Sheets.Editor.Commands.Tables do
  @moduledoc false
  ...
  def reorder_columns(block_id, column_ids) when is_list(column_ids) do
    fn ->
      ...
      Positions.batch_set("table_columns", pairs, scope: {"block_id", block_id})
  ...
  def reorder_rows(block_id, row_ids) when is_list(row_ids) do
    fn ->
      ...
      Positions.batch_set("table_rows", pairs, scope: {"block_id", block_id})
  ...
  defp sync_row_to_children(%Block{} = parent_block, %TableRow{} = row, :create) do
    if FormulaEngine.has_formula_variable_bindings?(row.cells) do
    ...
    if instances == [] do
      ...
      mappings =
        Map.new(child_sheet_ids, fn sheet_id ->
          {sheet_id, FormulaEngine.build_var_name_mapping(parent_block.sheet_id, sheet_id)}
        end)
