defmodule Storyarn.Sheets.Expressions.Queries.VariableNamespaces do
  @moduledoc """
  Resolves the canonical namespace used by Sheet variables.

  An explicit shortcut is authoritative. A Sheet without a shortcut falls
  back to its decimal ID, but only when no active Sheet owns that string as an
  explicit shortcut. One query combines indexable shortcut and primary-key
  predicates, avoiding both ambiguous `COALESCE` matches and read skew.
  """

  import Ecto.Query

  alias Storyarn.Repo
  alias Storyarn.Sheets.Expressions.Projections.SheetRecord
      ...
      from(sheet in SheetRecord,
        where:
          sheet.project_id == ^project_id and is_nil(sheet.deleted_at) and
            (sheet.shortcut in ^namespaces or (is_nil(sheet.shortcut) and sheet.id in ^numeric_ids)),
