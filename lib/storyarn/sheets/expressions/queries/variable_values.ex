defmodule Storyarn.Sheets.Expressions.Queries.VariableValues do
  @moduledoc """
  ...
  alias Storyarn.Repo
  alias Storyarn.Sheets.Expressions.Projections.BlockRecord, as: Block
  alias Storyarn.Sheets.Expressions.Projections.SheetRecord, as: Sheet
  alias Storyarn.Sheets.Expressions.Projections.TableColumnRecord, as: TableColumn
  alias Storyarn.Sheets.Expressions.Projections.TableRowRecord, as: TableRow
  ...
  defp compute_formula_cell(expression, bindings, row_cells) do
    values =
      Map.new(bindings || %{}, fn {symbol, binding} ->
        value =
          case binding do
            %{"type" => "same_row", "column_slug" => slug} -> NumericValues.parse(row_cells[slug])
