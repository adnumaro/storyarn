defmodule Storyarn.Sheets.Health.Queries.Stats do
  @moduledoc """
  Project-level Sheet statistics and health projections.

  The queries remain inside the Health capability because their shape is
  tailored to dashboard health and usage reporting, not to Sheet editing.
  """
  ...
  def sheet_word_counts(project_id) do
    localizable_block_types = Localization.localizable_block_types()
    ...
    block_counts =
      ...
      |> Repo.all()
      |> Enum.filter(&Localization.localizable_block?/1)
    ...
    sheet_counts =
      ...
      |> Repo.all()
      |> Map.new(fn {sheet_id, name} -> {sheet_id, Localization.word_count_for_name(name)} end)
  ...
  def list_dashboard_health_findings(project_id, referenced_ids \\ nil) do
    project_id
    |> Snapshots.load_project(referenced_ids)
