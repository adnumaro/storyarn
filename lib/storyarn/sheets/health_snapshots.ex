defmodule Storyarn.Sheets.HealthSnapshots do
  ...
  `SheetStats.referenced_block_ids_for_project/1` cached — the dashboard does, for
  its "variables in use" stat.
  """
  @spec load_project(integer(), MapSet.t() | nil) :: [map()]
  def load_project(project_id, referenced_ids \\ nil) do
    sheets = SheetQueries.list_sheets_unpreloaded(project_id)
  ...
  defp build_snapshots(project_id, sheets, referenced_ids) do
    blocks_by_sheet =
      sheets
      |> SheetQueries.list_project_blocks_grouped()
  ...
  defp gallery_data(all_blocks) do
    case block_ids_of_type(all_blocks, "gallery") do
      [] -> %{}
      gallery_ids -> GalleryCrud.batch_load_gallery_data(gallery_ids)
    end
  end
  ...
  defp reference_targets(blocks, project_id) do
    ...
    targets =
      ...
      |> ReferenceTracker.get_reference_targets(project_id)
