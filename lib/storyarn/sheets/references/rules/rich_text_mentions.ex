defmodule Storyarn.Sheets.References.Rules.RichTextMentions do
  @moduledoc false
  ...
  defp collect_html_candidates(value, acc) when is_binary(value) do
    if mention_markup?(value), do: [value | acc], else: acc
  end

  defp collect_html_candidates(value, acc) when is_list(value), do: Enum.reduce(value, acc, &collect_html_candidates/2)
  ...
  defp accumulate_mention(element, {:ok, mentions}) do
    ...
    case {type_attributes, id_attributes} do
      {[type], [id]}
      when type in ["sheet", "flow"] and is_binary(id) and byte_size(id) > 0 ->
