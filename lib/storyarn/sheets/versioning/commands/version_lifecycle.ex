defmodule Storyarn.Sheets.Versioning.Commands.VersionLifecycle do
  @moduledoc false
  ...
  defp maybe_create_version_if_due(sheet, project_id, user_id, opts, min_interval) do
    if version_due?(History.get_latest_version(sheet.id), min_interval) do
      snapshot = SheetSnapshot.build(sheet)
      ...
      |> VersionNumberLock.run(fn ->
        create_version_if_due_locked(
          ...
          opts,
          min_interval,
          History.get_latest_version(sheet.id)
        )
      end)
  ...
  defp change_data(sheet_id, snapshot, opts) do
    if Keyword.get(opts, :skip_diff, false) do
      ...
      case History.get_latest_version(sheet_id) do
        nil ->
