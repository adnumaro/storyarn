defmodule Storyarn.Sheets.Versioning.ConflictDetector do
  @moduledoc """
  ...
  @doc "Returns the conflict report consumed by the Sheet version-history UI."
  @spec detect(map(), Sheet.t()) :: map()
  def detect(snapshot, %Sheet{} = sheet) when is_map(snapshot) do
    references = Enum.map(SheetSnapshot.scan_references(snapshot), &normalize_reference/1)

    missing = find_missing_references(references, sheet.project_id, snapshot)

    conflicts = group_conflicts(missing)
    {shortcut_collision, resolved_shortcut} = shortcut_collision(sheet, snapshot["shortcut"])
  ...
  defp shortcut_collision(_sheet, nil), do: {false, nil}

  defp shortcut_collision(sheet, shortcut) when is_binary(shortcut) do
    collision? =
      Repo.exists?(
  ...
  defp shortcut_collision(_sheet, shortcut), do: {false, shortcut}
