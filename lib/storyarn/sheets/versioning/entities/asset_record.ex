defmodule Storyarn.Sheets.Versioning.Entities.AssetRecord do
  @moduledoc "Versioning-owned persistence model used to capture and restore Sheet assets without importing another context's schema."
  ...
  @doc "Checks if a content type is allowed for direct uploads."
  ...
  @doc false
  def update_metadata_changeset(asset, metadata) when is_map(metadata) do
    cast(asset, %{metadata: metadata}, [:metadata])
  end

  defp create_changeset(asset, attrs, content_types) do
    asset
    |> cast(attrs, [:filename, :content_type, :size, :key, :url, :metadata, :blob_hash])
    |> validate_required([:filename, :content_type, :size, :key, :blob_hash])
