defmodule Storyarn.Sheets.Versioning.SheetSnapshot do
  @moduledoc """
  ...
  import Storyarn.Sheets.Versioning.Commands.MaterializationHelpers, only: [exact_materialization?: 1]
    ...
    {:ok, snapshot} =
      Repo.transaction(
        fn ->
          ...
          :ok = Localization.lock_inventory!(locked_sheet.project_id)
  ...
  defp delete_extra_children_source(%{source: source, active_instance_ids: active_instance_ids}) do
    ...
    Localization.archive_restore_texts_for_sources(
      "block",
    ...
    source
    |> Editor.delete_inherited_instances()
  ...
  defp sync_inherited_definitions(source_ids) do
    validate_each(source_ids, fn source_id ->
      Block
      |> Repo.get!(source_id)
      |> Editor.sync_inheritance_definition(active_owner_sheets_only: true)
      |> normalize_property_inheritance_result(
        source_id,
  ...
  defp block_restore_attrs(block_data) do
    %{
      ...
      word_count: Localization.word_count_for_block(block_data["type"], block_data["value"]),
  ...
  defp ensure_avatar_deletion_safe(%SheetAvatar{id: avatar_id}) do
    case References.ensure_avatar_deletable(avatar_id) do
      :ok ->
        :ok
  ...
  defp restore_sheet_localization_in_place(sheet, snapshot, block_data) do
    ...
    Localization.archive_texts_for_active_target_locales(
      sheet.project_id,
      "block",
      target_ids,
      "version_replaced"
    ...
    Localization.archive_restore_texts_for_sources("block", block_data.soft_deleted_ids, "source_deleted")
  ...
  defp add_block_value_refs(refs, block, block_index) do
    case References.extract_block_value_references(block["type"], block["value"]) do
      {:ok, references} ->
  ...
  @doc false
  def build(sheet), do: build_snapshot(sheet)

  @doc false
  def restore(sheet, snapshot, opts \\ []), do: restore_snapshot(sheet, snapshot, opts)
