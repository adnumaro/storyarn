defmodule Storyarn.Sheets.Versioning.SheetSnapshot do
  ...
  alias Storyarn.Sheets.LocalizationProjection
  ...
  alias Storyarn.Sheets.Versioning.LocalizationSnapshotValidator, as: SheetLocalizationSnapshotValidator
  ...
  defp rebuild_instantiated_block_references(block, project_id) do
    case ReferenceTracker.update_block_references(block, project_id: project_id) do
      :ok -> :ok
  ...
  defp delete_extra_children_source(%{source: source, active_instance_ids: active_instance_ids}) do
    Enum.each([source.id | active_instance_ids], &ReferenceTracker.delete_block_references/1)

    LocalizationProjection.archive_restore_texts_for_sources(
      "block",
  ...
  defp restore_sheet_localization_in_place(sheet, snapshot, block_data) do
    ...
    LocalizationProjection.archive_restore_texts_for_sources("block", block_data.soft_deleted_ids, "source_deleted")
  ...
  defp rebuild_restored_block_references(block_data, project_id, opts) do
    ...
    Enum.each(Enum.uniq(target_ids ++ block_data.soft_deleted_ids), fn block_id ->
      ReferenceTracker.delete_block_references(block_id)
    end)
  ...
  defp reconcile_block_references(block, project_id, opts) do
    update_references =
      Keyword.get(
        ...
        &ReferenceTracker.update_block_references/2
      )
