defmodule Storyarn.Sheets.Versioning do
  ...
  alias Storyarn.Sheets.Versioning.Adapters.Locks.Postgres, as: VersionNumberLock
  ...
  defdelegate restore_tracked_version(scope, sheet, version, opts), to: Tracked, as: :restore_version
  defdelegate serialize_version_snapshot(snapshot), to: SnapshotViewer, as: :serialize_sheet

  def prepare_version_restore(%Sheet{} = sheet, target_version), do: prepare_restore(sheet, target_version)

  def prepare_version_restore_conflicts(%Sheet{} = sheet, target_version),
    do: prepare_restore_conflicts(sheet, target_version)

  def detect_version_restore_conflicts(snapshot, %Sheet{} = sheet), do: detect_restore_conflicts(snapshot, sheet)

  def ensure_version_restore_enabled, do: ensure_restore_enabled()
  ...
  def create_version(%Sheet{} = sheet, user_id, opts \\ []) do
    create_version(@entity_type, sheet, sheet.project_id, user_id, opts)
  end

  @doc false
  def create_version(entity_type, entity, project_id, user_id, opts \\ [])

  def create_version(@entity_type, %Sheet{} = sheet, project_id, user_id, opts) do
    with :ok <- validate_sheet_scope(sheet, project_id),
         :ok <- validate_actor_id(user_id) do
      snapshot = SheetSnapshot.build(sheet)
      create_from_snapshot(sheet, project_id, user_id, snapshot, opts)
    end
  end

  def create_version(_entity_type, _entity, _project_id, _user_id, _opts), do: {:error, :entity_scope_mismatch}

  defp create_from_snapshot(sheet, project_id, user_id, snapshot, opts) do
    sheet.id
    |> VersionNumberLock.run(fn ->
      create_from_snapshot_locked(sheet, project_id, user_id, snapshot, opts)
    end)
    |> normalize_limit_error()
  end

  defp create_from_snapshot_locked(sheet, project_id, user_id, snapshot, opts) do
    case ensure_named_version_capacity(project_id, opts) do
      :ok ->
        {change_summary, change_details} = change_data(sheet.id, snapshot, opts)
  ...
  def maybe_create_version(%Sheet{} = sheet, user_id, opts \\ []) do
    ...
    if Keyword.get(opts, :is_auto) and not auto_versioning_enabled?(sheet.project_id) do
      {:skipped, :auto_versioning_disabled}
    else
      maybe_create_version(@entity_type, sheet, sheet.project_id, user_id, opts)
    end
  end
  ...
  def maybe_create_version(@entity_type, %Sheet{} = sheet, project_id, user_id, opts) do
    with :ok <- validate_sheet_scope(sheet, project_id),
         :ok <- validate_actor_id(user_id) do
      min_interval = Keyword.get(opts, :min_interval, @default_min_interval_seconds)
      maybe_create_version_if_due(sheet, project_id, user_id, opts, min_interval)
    end
  end

  def maybe_create_version(_entity_type, _entity, _project_id, _user_id, _opts), do: {:error, :entity_scope_mismatch}

  defp maybe_create_version_if_due(sheet, project_id, user_id, opts, min_interval) do
    if version_due?(get_latest_version(sheet.id), min_interval) do
      snapshot = SheetSnapshot.build(sheet)
  ...
  defp create_version_if_due_locked(sheet, project_id, user_id, snapshot, opts, min_interval, latest) do
    if version_due?(latest, min_interval),
      do: create_from_snapshot_locked(sheet, project_id, user_id, snapshot, opts),
  ...
  defp cleanup_deleted_snapshot(storage_key) do
    case SnapshotStorage.delete(storage_key) do
      :ok -> :ok
      {:error, reason} -> Logger.warning("Failed to delete Sheet version snapshot: #{inspect(reason)}")
    end
  end
  ...
  @doc false
  def build_snapshot(%Sheet{} = sheet), do: SheetSnapshot.build(sheet)

  @doc false
  def snapshot_has_changes?(previous, current), do: SheetSnapshot.diff(previous, current) != []

  @doc false
  def snapshot_has_changes?(@entity_type, previous, current), do: SheetSnapshot.diff(previous, current) != []
  ...
  defp auto_versioning_enabled?(project_id) do
    case Repo.get(ProjectRecord, project_id) do
      %ProjectRecord{deleted_at: nil, auto_version_sheets: enabled?} -> enabled? == true
      _project -> false
    end
  end
  ...
  defp validate_sheet_scope(_flow, _project_id), do: {:error, :entity_scope_mismatch}
  ...
  defp change_data(sheet_id, snapshot, opts) do
  ...
  defp previous_change_data(previous, snapshot) do
    case load_version_snapshot(previous) do
      {:ok, previous_snapshot} ->
        changes = SheetSnapshot.diff(previous_snapshot, snapshot)
        {format_summary(changes), serialize_changes(changes)}

      {:error, reason} ->
        Logger.warning("Failed to load previous Sheet snapshot: #{inspect(reason)}")
