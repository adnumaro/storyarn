defmodule Storyarn.Versioning.Builders.FlowBuilder do
  ...
  defp lock_and_validate_incoming_dynamic_pins(repo, project_id, restored_flow_id, snapshot_nodes) do
    ...
    with :ok <-
         ...
         :ok <- validate_incoming_dynamic_connections(invalid_connections) do
      {:ok,
       %{
  ...
  defp validate_incoming_dynamic_connections([]), do: :ok

  defp validate_incoming_dynamic_connections([%{reason: reason} | _invalid_connections]), do: {:error, reason}
  ...
  # Snapshot identity and canvas position do not represent content changes.
