defmodule Storyarn.Workspaces.Invitations.Adapters.Jobs.InvitationQueue do
  @moduledoc """
  Workspace-owned adapter for durable invitation delivery.
  ...
  @doc "Wakes the Workspace invitation queue after its enclosing transaction commits."
  ...
  def wake_after_commit(%Oban.Job{} = job, opts \\ []) do
    ...
    case safely_notify(notifier) do
      ...
      :error ->
        Logger.warning("Workspace invitation queue wakeup failed after commit job_id=#{job.id}")
    end
  end
  ...
  defp encrypt_token(encoded_token) do
    ...
    case encryptor.dump(encoded_token) do
      ...
      _error ->
        Logger.error("Workspace invitation token encryption failed")
        {:error, :encryption_unavailable}
    end
  rescue
    error ->
      Logger.error("Workspace invitation token encryption failed: #{Exception.message(error)}")
      {:error, :encryption_unavailable}
  end
