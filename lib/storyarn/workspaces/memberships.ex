defmodule Storyarn.Workspaces.Memberships do
  ...
  def list_workspace_members(workspace_id), do: MembershipOperations.list_members(@config, workspace_id)
  ...
  def get_membership(workspace_id, user_id) when is_integer(workspace_id) and is_integer(user_id),
    do: MembershipOperations.get_membership(@config, workspace_id, user_id)
  ...
  def authorize(scope, workspace_id, action), do: MembershipOperations.authorize(@config, scope, workspace_id, action)
end
