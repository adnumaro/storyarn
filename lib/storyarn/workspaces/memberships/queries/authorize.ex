defmodule Storyarn.Workspaces.Memberships.Queries.Authorize do
  ...
  @canonical_owner_actions [:manage_workspace, :delete_workspace, :run_bulk_ai]
  @ownership_sensitive_actions [:manage_workspace, :delete_workspace, :manage_members, :remove_members, :run_bulk_ai]
  ...
  def call(%{user: user}, workspace_id, action) do
    with %Workspace{} = workspace <- Repo.get(Workspace, workspace_id),
         ...
         :ok <- authorize_membership(workspace, membership, user.id, action),
  ...
  defp authorize_canonical_owner(action, _owner_membership, _user_id) when action in [:manage_members, :remove_members],
    do: :ok
