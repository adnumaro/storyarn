defmodule StoryarnWeb.Components.LandingPage.LandingFooter do
  ...
  def landing_footer(assigns) do
    ...
    ~H"""
    <footer>
      <div class="border-t border-border bg-muted/60 px-6 py-8 shadow-[0_24px_80px_rgba(0,0,0,0.16)] sm:px-8 lg:px-10">
        <div class="grid gap-8 lg:grid-cols-[1.35fr_0.95fr_0.85fr] lg:items-start">
          <div class="grid gap-4 max-w-[30rem]">
            ...
            <p class="max-w-[28rem] text-sm leading-relaxed text-foreground/58">
              {gettext(
            ...
            <div class="flex flex-wrap items-center gap-3 text-sm text-foreground/55">
              <span class="text-xs px-1.5 py-0.5 rounded bg-muted text-muted-foreground badge-sm">{gettext("Private beta")}</span>
              <span>{gettext("Realtime collaboration")}</span>
              <span>{gettext("Version snapshots")}</span>
            </div>
          </div>

          <nav class="grid gap-3 text-sm text-foreground/62">
            ...
            <.link navigate={~p"/docs"} class="transition-colors hover:text-foreground">
              {gettext("Docs")}
            </.link>
        ...
        <div class="mt-8 flex flex-col gap-3 border-t border-border pt-4 text-sm text-foreground/42 sm:flex-row sm:items-center sm:justify-between">
