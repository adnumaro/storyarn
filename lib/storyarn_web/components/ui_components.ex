defmodule StoryarnWeb.Components.UIComponents do
  ...
  def oauth_buttons(assigns) do
    ...
    <div class={["flex flex-col gap-2", @class]}>
      <.link
        ...
        class="inline-flex items-center justify-center px-3 py-2 text-sm rounded-md border border-input hover:bg-accent gap-2"
      >
  ...
  def empty_state(assigns) do
    ...
    <div class={["text-center py-12", @class]}>
      <.icon name={@icon} class="size-12 mx-auto mb-4 text-foreground/30" />
      <p :if={@title} class="font-medium text-muted-foreground">{@title}</p>
      <p :if={@inner_block != []} class="text-sm text-muted-foreground mt-1">
        {render_slot(@inner_block)}
      </p>
  ...
  def theme_toggle(assigns) do
    ~H"""
    <div class="card relative flex flex-row items-center border-2 border-border bg-border rounded-full">
      <div class="absolute w-1/3 h-full rounded-full border-1 border-border bg-background brightness-200 left-0 [[data-theme=light]_&]:left-1/3 [[data-theme=dark]_&]:left-2/3 transition-[left]" />
