defmodule StoryarnWeb.FlowLive.Handlers.CommentHandlers do
  @moduledoc false
  ...
  @mutations ~w(create reply set_status)
  ...
  def init(socket) do
    ...
    |> assign(:pending_comment_thread_id, nil)
  end
  ...
  def loaded(socket) do
    ...
    case socket.assigns.pending_comment_thread_id do
      nil -> socket
      thread_id -> socket |> assign(:pending_comment_thread_id, nil) |> select_thread(thread_id)
    end
  end

  def unload(socket) do
    Projects.unsubscribe_flow_comments(socket.assigns.project.id, socket.assigns.flow.id)
    socket
  end

  def close(%{assigns: %{comments: _state}} = socket), do: put_state(socket, %{open: false, error: nil})
  def close(socket), do: socket

  def handle_params(socket, %{"thread" => thread_id}) do
    if socket.assigns.loading do
      assign(socket, :pending_comment_thread_id, positive_id(thread_id))
    else
      select_thread(socket, positive_id(thread_id))
    end
  end
  ...
  def handle(_action, _params, %{assigns: %{loading: true}} = socket), do: failure(socket, :unavailable)
  ...
  def refresh(%{assigns: %{flow: nil}} = socket), do: socket

  def refresh(socket) do
    %{current_scope: scope, project: project, flow: flow, comments: state} = socket.assigns

    case Projects.flow_comment_counts(scope, project.id, flow.id) do
      {:ok, counts} ->
        can_comment = match?({:ok, _, _}, Projects.authorize(scope, project.id, :edit_content))
        socket = socket |> assign(:comment_counts, counts) |> put_state(%{canComment: can_comment})
  ...
  defp refresh_selected_source(socket, node_id) do
    case optional_node(socket, node_id) do
      {:ok, _node_id} ->
        socket
      ...
      _unavailable ->
        socket
        |> assign(:comment_focus_node_id, nil)
        |> put_state(%{selectedNodeId: nil, error: error_message(:source_unavailable)})
    end
  end
  ...
  defp mutate("reply", params, socket) do
    ...
    case current_flow_thread(socket, thread_id) do
      {:ok, _detail} ->
  ...
  defp mutate("set_status", params, socket) do
    ...
    with {:ok, _detail} <- current_flow_thread(socket, thread_id),
         {:ok, _thread} <-
  ...
  defp load_detail(socket, thread_id, cursor \\ nil) do
    case current_flow_thread(socket, thread_id, cursor: cursor) do
      {:ok, %{thread: thread, messages: messages, next_cursor: next_cursor}} ->
        ...
        node_id = if thread.source.status == "available", do: thread.source.id

        socket
        |> put_state(%{thread: thread, messages: messages, messageNextCursor: next_cursor, selectedNodeId: node_id})
        |> assign(:comment_focus_node_id, node_id)
  ...
  defp current_flow_thread(socket, thread_id, opts \\ []) do
    with {:ok, %{thread: %{source: %{flow_id: flow_id}}} = detail} <-
           Projects.get_comment_thread(socket.assigns.current_scope, socket.assigns.project.id, thread_id, opts),
         true <- flow_id == socket.assigns.flow.id do
      {:ok, detail}
    else
  ...
  defp optional_node(_socket, nil), do: {:ok, nil}

  defp optional_node(socket, raw_id) do
    with node_id when is_integer(node_id) <- positive_id(raw_id),
         %{id: ^node_id} <- Flows.get_node(socket.assigns.flow.id, node_id) do
      {:ok, node_id}
    else
      _error -> {:error, :not_found}
    end
  ...
  defp focus_node(socket, node_id) do
    socket
    |> assign(:comment_focus_node_id, node_id)
    |> push_event("focus_comment_node", %{node_db_id: node_id})
  end

  defp focus_selected_thread(socket), do: focus_node(socket, socket.assigns.comment_focus_node_id)
