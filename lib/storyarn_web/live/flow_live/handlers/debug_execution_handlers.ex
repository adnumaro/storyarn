defmodule StoryarnWeb.FlowLive.Handlers.DebugExecutionHandlers do
  ...
  defp restore_debug_state(socket, previous_state, new_state) do
    if changed_flow?(previous_state, new_state) do
      graph = Flows.load_runtime_graph(new_state.current_flow_id)

      socket =
        socket
        |> assign(:debug_state, new_state)
        |> assign_runtime_graph(graph)
  ...
  @doc "Refreshes composition inputs during an active session without restarting execution."
  def refresh_composition_graph(
        %{assigns: %{debug_panel_open: true, debug_state: %{current_flow_id: flow_id}, flow: %{id: flow_id}}} = socket
      ) do
    assign_runtime_graph(socket, Flows.load_runtime_graph(flow_id))
  end

  def refresh_composition_graph(socket), do: socket

  defp assign_runtime_graph(socket, graph) do
    presented = present_runtime_graph(graph)
    socket |> assign(:debug_nodes, presented.nodes) |> assign(:debug_connections, presented.connections)
  end
