defmodule StoryarnWeb.FlowLive.VersionEvents do
  @moduledoc """
  Flow-owned event handlers for the Flow version panel.

  Persistence and restore policy always go through `Storyarn.Flows`; the
  callback map contains only Flow-page navigation and reload behavior.
  """
  ...
  def handle_compare(%{"version_number" => version_number}, socket, config) do
    case VersionHistory.parse_version_number(version_number) do
      {:ok, number} ->
        Flows.record_version_compared(socket.assigns.current_scope, entity(socket))
  ...
  def with_authorized_restore(socket, fun) do
    case Flows.ensure_version_restore_enabled() do
      :ok ->
  ...
  defp get_version(socket, _config, version_number) do
    with {:ok, number} <- VersionHistory.parse_version_number(version_number),
         version when not is_nil(version) <-
           Flows.get_version(entity(socket).id, number) do
      {:ok, version}
    else
      _ -> :error
    end
  ...
  defp promote_version(socket, config, version, title, description) do
    case Flows.update_version(version, %{
           title: title,
           description: description
         }) do
      {:ok, _} ->
  ...
  defp delete_version(socket, config, version) do
    case Flows.delete_version(version) do
      {:ok, _} ->
        {:noreply,
         socket
         |> config.reload_history.()
  ...
  defp show_restore_preview(socket, _config, version, request_id) do
    case Flows.prepare_version_restore_conflicts(entity(socket), version) do
  ...
  defp restore_version(socket, config, version, request_id) do
    case Flows.restore_tracked_version(
