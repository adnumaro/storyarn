defmodule StoryarnWeb.FlowLive.VersionHistory do
  @moduledoc """
  Flow-owned version history loading and serialization for the Flow editor.

  This module deliberately exposes no generic entity or versioning-context
  switch: the Flow Web boundary consumes only `Storyarn.Flows`.
  """
  ...
  @doc """
  Loads version history for a Flow and assigns it to `:history_data`.

  ## Options
    * `flow` - the Flow record (must have `:id` and `:current_version_id`)
    * `project_id` - the project ID
    * `workspace_id` - the workspace ID
  """
  def load_history_data(socket, flow, project_id, workspace_id) do
    versions =
      Flows.list_versions(flow.id,
  ...
  def load_more_history(socket, flow_id, page) do
    offset = (page - 1) * @versions_per_page

    new_versions =
      Flows.list_versions(flow_id,
        limit: @versions_per_page + 1,
        offset: offset
      )
  ...
  def detect_and_show_restore_preview(socket, flow, version, request_id) do
    case Flows.prepare_version_restore(flow, version) do
      {:ok, :unsaved_changes} ->
