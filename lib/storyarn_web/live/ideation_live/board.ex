defmodule StoryarnWeb.IdeationLive.Board do
  ...
  @round_writes ~w(new_round update_round close_round set_round_privacy reveal_round move_decision_lane)
  ...
  def handle_info(
        ...
        %{assigns: %{project: %{id: project_id}, session_id: session_id}} = socket
      ), do: {:noreply, CommentHandlers.refresh_participation(socket, thread_id)}
  ...
  def handle_info({:tree_changed, _}, socket),
    do: {:noreply, socket |> ReferenceHandlers.refresh() |> ExplorationContextHandlers.refresh()}

  def handle_info({:entities_deleted, _, _}, socket),
    do: {:noreply, socket |> ReferenceHandlers.refresh() |> ExplorationContextHandlers.refresh()}
