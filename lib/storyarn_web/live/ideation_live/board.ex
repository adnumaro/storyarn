defmodule StoryarnWeb.IdeationLive.Board do
  ...
  @idea_writes ~w(create_idea derive_idea save_idea prepare_reveal reveal_ideas)
  ...
  def handle_info({:project_restored, _restore_id}, socket) do
    socket = socket |> reset_epoch("project_restored") |> assign(:board, BoardData.empty()) |> refresh()
    {:noreply, socket}
  end
  ...
  defp accept_read(socket, {:ok, {:ok, data}}) do
    ...
    case Projects.authorize(socket.assigns.current_scope, socket.assigns.project.id, :view) do
      {:ok, project, membership} ->
        ...
        assign(socket, board: data, board_error: nil, membership: membership, can_edit: can_edit)
  ...
  defp lose_access(socket) do
    socket |> reset_epoch("access_changed") |> assign(board: BoardData.empty(), board_error: "unauthorized")
  end
