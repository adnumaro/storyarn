defmodule StoryarnWeb.PageLive.Components.VariableUsageSection do
  @moduledoc """
  LiveComponent that shows which flow nodes read/write each variable on a page.
          ...
          <%= if @total_refs == 0 do %>
            <div class="bg-base-200/50 rounded-lg p-6 text-center">
              <p class="text-base-content/70 text-sm">
                {gettext("No variables on this page are used in any flow yet.")}
              </p>
            </div>
          <% else %>
            <div class="space-y-4">
  ...
  defp variable_block_usage(assigns) do
    ...
    <div :if={@has_usage} class="rounded-lg border border-base-300/50 p-3">
      <div class="flex items-center gap-2 mb-2">
        ...
        <code class="text-xs text-base-content/50">{@page.shortcut}.{@block.variable_name}</code>
  ...
  defp usage_ref_row(assigns) do
    detail = format_ref_detail(assigns.ref, assigns.page, assigns.block)
  ...
  defp format_ref_detail(ref, page, block) when ref.kind == "write" do
