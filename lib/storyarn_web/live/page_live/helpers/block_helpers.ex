defmodule StoryarnWeb.PageLive.Helpers.BlockHelpers do
  ...
  # These return {:ok, blocks} or {:error, message} instead of {:noreply, socket}
  # ===========================================================================
  ...
  def update_rich_text_value(socket, block_id, content) do
    ...
    case Pages.update_block_value(block, %{"content" => content}) do
      {:ok, _block} ->
        # For rich text, we return current blocks without forcing reload
        # to avoid disrupting the editor
        blocks = ReferenceHelpers.load_blocks_with_references(page_id, project_id)
        {:ok, blocks}
