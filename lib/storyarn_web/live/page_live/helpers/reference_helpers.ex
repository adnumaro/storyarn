defmodule StoryarnWeb.PageLive.Helpers.ReferenceHelpers do
  @moduledoc false
  ...
  def select_reference(socket, block_id, target_type, target_id) do
    ...
    with {target_id_int, ""} <- Integer.parse(target_id),
         {:ok, _target} <- Pages.validate_reference_target(target_type, target_id_int, project_id),
         {:ok, _block} <-
           Pages.update_block_value(block, %{
             "target_type" => target_type,
             "target_id" => target_id_int
           }) do
      blocks = load_blocks_with_references(socket.assigns.page.id, project_id)
  ...
  """
  def load_blocks_with_references(page_id, project_id) do
  ...
  def select_reference_value(socket, block_id, target_type, target_id) do
    ...
    with {target_id_int, ""} <- Integer.parse(target_id),
      ...
      blocks = load_blocks_with_references(page_id, project_id)
      {:ok, blocks}
    else
      :error ->
