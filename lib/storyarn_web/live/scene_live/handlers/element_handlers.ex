defmodule StoryarnWeb.SceneLive.Handlers.ElementHandlers do
  ...
  def handle_create_pin(%{"position_x" => x, "position_y" => y}, socket) do
    ...
    case Scenes.create_pin(socket.assigns.scene.id, attrs) do
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, dgettext("scenes", "Could not create pin."))}
    end
  end
  ...
  def handle_start_pin_from_sheet(%{"sheet-id" => sheet_id}, socket) do
    ...
    if sheet do
      ...
      {:noreply, put_flash(socket, :error, dgettext("scenes", "Sheet not found."))}
    end
  end
  ...
  def handle_create_zone(%{"vertices" => vertices} = params, socket) do
    name = params["name"]
    name = if name == "" or is_nil(name), do: dgettext("scenes", "New Zone"), else: name
    ...
    case Scenes.create_zone(socket.assigns.scene.id, attrs) do
      {:ok, zone} ->
        {:noreply,
         socket
  ...
  def handle_update_collection_item_condition(
        %{"zone-id" => zone_id, "item-id" => item_id, "condition" => condition},
        socket
      ) do
    update_fn = fn item -> Map.put(item, "condition", condition) end
    do_update_collection_item(socket, zone_id, item_id, update_fn)
  end

  @doc "Updates the instruction assignments on a collection item."
  def handle_update_collection_item_instruction(
        %{"zone-id" => zone_id, "item-id" => item_id, "assignments" => assignments},
        socket
      ) do
    update_fn = fn item -> Map.put(item, "instruction", %{"assignments" => assignments}) end
    do_update_collection_item(socket, zone_id, item_id, update_fn)
  end
  ...
  defp update_collection_item_in_list(items, item_id, update_fn) do
    Enum.map(items, fn item ->
      if item["id"] == item_id, do: update_fn.(item), else: item
    end)
  end
  ...
  def handle_create_connection(%{"from_pin_id" => from_pin_id, "to_pin_id" => to_pin_id}, socket) do
    ...
    case Scenes.create_connection(socket.assigns.scene.id, attrs) do
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, dgettext("scenes", "Could not create connection."))}
    end
  end
  ...
  def handle_delete_connection(%{"id" => connection_id}, socket) do
    case Scenes.get_connection(socket.assigns.scene.id, connection_id) do
      nil -> {:noreply, socket}
  ...
  def handle_create_annotation(%{"position_x" => x, "position_y" => y} = params, socket) do
    attrs = %{
      "text" => params["text"] || dgettext("scenes", "Note"),
      "position_x" => x,
      "position_y" => y,
    ...
    case Scenes.create_annotation(socket.assigns.scene.id, attrs) do
      {:ok, annotation} ->
        {:noreply,
         socket
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, dgettext("scenes", "Could not create annotation."))}
    end
  end
  ...
  def handle_update_annotation(%{"field" => field} = params, socket)
    ...
    case Scenes.get_annotation(socket.assigns.scene.id, id) do
      nil -> {:noreply, socket}
  ...
  def handle_delete_annotation(%{"id" => annotation_id}, socket) do
    case Scenes.get_annotation(socket.assigns.scene.id, annotation_id) do
      nil -> {:noreply, socket}
  ...
  defp do_move_pin(socket, pin, x, y) do
    ...
    case Scenes.move_pin(pin, x, y) do
      {:ok, updated} ->
        {:noreply,
         socket
  ...
  defp do_delete_pin(socket, %{locked: true}) do
    {:noreply, put_flash(socket, :error, dgettext("scenes", "Cannot delete a locked element."))}
  end
  ...
  defp do_delete_pin(socket, pin) do
    ...
    case Scenes.delete_pin(pin) do
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, dgettext("scenes", "Could not delete pin."))}
    end
  end
  ...
  defp do_update_zone_vertices(socket, zone, vertices) do
    ...
    case Scenes.update_zone_vertices(zone, %{"vertices" => vertices}) do
      {:ok, updated} ->
        {:noreply,
         socket
    ...
    case Scenes.update_zone(zone, attrs) do
      {:ok, updated} ->
        {:noreply,
         ...
         |> assign(:selected_element, updated)
         |> assign(:zones, replace_in_list(socket.assigns.zones, updated))
         |> assign(:_broadcast, {:zone_updated, %{id: updated.id}})
  ...
  defp do_duplicate_zone(socket, zone) do
    ...
    case Scenes.create_zone(socket.assigns.scene.id, attrs) do
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, dgettext("scenes", "Could not duplicate zone."))}
    end
  end
  ...
  defp do_delete_zone(socket, zone) do
    case Scenes.delete_zone(zone) do
      {:ok, _} ->
        {:noreply,
         socket
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, dgettext("scenes", "Could not delete zone."))}
    end
  end
  ...
  defp do_update_connection(socket, conn, field, value) do
    ...
    case Scenes.update_connection(conn, %{field => value}) do
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, dgettext("scenes", "Could not update connection."))}
    end
  end
  ...
  defp do_update_connection_waypoints(socket, conn, waypoints) do
    ...
    case Scenes.update_connection_waypoints(conn, %{"waypoints" => waypoints}) do
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, dgettext("scenes", "Could not update waypoints."))}
    end
  end
  ...
  defp do_delete_connection(socket, connection) do
    case Scenes.delete_connection(connection) do
      {:ok, _} ->
        {:noreply,
         socket
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, dgettext("scenes", "Could not delete connection."))}
    end
  end
  ...
  defp do_update_annotation(socket, annotation, field, value) do
    ...
    case Scenes.update_annotation(annotation, %{field => value}) do
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, dgettext("scenes", "Could not update annotation."))}
    end
  end
  ...
  defp do_move_annotation(socket, annotation, x, y) do
    ...
    case Scenes.move_annotation(annotation, x, y) do
      {:ok, updated} ->
        {:noreply,
         socket
  ...
  defp do_delete_annotation(socket, annotation) do
    case Scenes.delete_annotation(annotation) do
      {:ok, _} ->
        {:noreply,
         socket
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, dgettext("scenes", "Could not delete annotation."))}
    end
  end
  ...
  defp do_duplicate_pin(socket, pin) do
    ...
    case Scenes.create_pin(socket.assigns.scene.id, attrs) do
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, dgettext("scenes", "Could not duplicate pin."))}
    end
  end
  ...
  defp do_create_pin_from_clipboard(socket, attrs) do
    pin_attrs =
      Map.merge(attrs, %{
        ...
        "label" => attrs["label"] || dgettext("scenes", "New Pin"),
    ...
    case Scenes.create_pin(socket.assigns.scene.id, pin_attrs) do
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, dgettext("scenes", "Could not paste pin."))}
    end
  end
  ...
  defp do_create_zone_from_clipboard(socket, attrs) do
    zone_attrs =
      Map.merge(attrs, %{
        "name" => (attrs["name"] || dgettext("scenes", "New Zone")) <> " (paste)",
        "vertices" => attrs["vertices"] || [],
        "layer_id" => socket.assigns.active_layer_id
      })

    case Scenes.create_zone(socket.assigns.scene.id, zone_attrs) do
      {:ok, zone} ->
        {:noreply,
         socket
         |> push_undo({:create_zone, zone})
         ...
         |> push_event("zone_created", serialize_zone(zone))
         |> push_event("element_selected", %{type: "zone", id: zone.id})
         |> put_flash(:info, dgettext("scenes", "Zone pasted."))}

      {:error, _} ->
        {:noreply, put_flash(socket, :error, dgettext("scenes", "Could not paste zone."))}
    end
  end

  defp do_create_annotation_from_clipboard(socket, attrs) do
    ann_attrs = %{
      "text" => attrs["text"] || dgettext("scenes", "Note"),
      "position_x" => attrs["position_x"] || 50.0,
      "position_y" => attrs["position_y"] || 50.0,
    ...
    case Scenes.create_annotation(socket.assigns.scene.id, ann_attrs) do
      {:ok, annotation} ->
        {:noreply,
         socket
         |> push_undo({:create_annotation, annotation})
         ...
         |> push_event("annotation_created", serialize_annotation(annotation))
         |> push_event("element_selected", %{type: "annotation", id: annotation.id})
         |> put_flash(:info, dgettext("scenes", "Annotation pasted."))}

      {:error, _} ->
        {:noreply, put_flash(socket, :error, dgettext("scenes", "Could not paste annotation."))}
    end
  end
