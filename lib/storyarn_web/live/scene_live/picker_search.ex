defmodule StoryarnWeb.SceneLive.PickerSearch do
  @moduledoc """
  Scene editor bounded search contract for Vue picker components.
  ...
  def asset_options(project_id, kind, opts \\ []) do
    ...
    {assets, has_more} = Scenes.search_asset_options(project_id, kind, opts)
  ...
  def sheet_options(project_id, opts \\ []) do
    ...
    sheets = Scenes.search_sheets(project_id, query, limit: limit + 1)
    {page, has_more} = split_page(sheets, limit)

    selected =
      case parse_integer(selected_id) do
        nil -> nil
        id -> Scenes.get_sheet(project_id, id)
      end
  ...
  def flow_options(project_id, opts \\ []) do
    ...
    flows = Scenes.search_flows(project_id, query, limit: limit + 1)
    {page, has_more} = split_page(flows, limit)

    selected =
      case parse_integer(selected_id) do
        nil -> nil
        id -> Scenes.get_flow(project_id, id)
      end
  ...
  def initial_asset_options(project_id, kind, selected_ids) do
    project_id
    |> Scenes.initial_asset_options(kind, normalize_ids(selected_ids))
  ...
  def initial_sheet_options(project_id, selected_ids) do
    ...
    initial_entity_options(
      ...
      selected_ids,
      fn id -> Scenes.get_sheet(project_id, id) end
    )
  end
  ...
  def initial_flow_options(project_id, selected_ids) do
    ...
    initial_entity_options(
      ...
      selected_ids,
      fn id -> Scenes.get_flow(project_id, id) end
    )
  end
