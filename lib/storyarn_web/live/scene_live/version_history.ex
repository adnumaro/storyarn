defmodule StoryarnWeb.SceneLive.VersionHistory do
  @moduledoc """
  Scene-owned version history loading and serialization for the Scene editor.

  This module deliberately exposes no generic entity or versioning-context
  switch: the Scene Web boundary consumes only `Storyarn.Scenes`.
  """
  ...
  @doc """
  Loads version history for a Scene and assigns it to `:history_data`.

  ## Options
    * `scene` - the Scene record (must have `:id` and `:current_version_id`)
    * `project_id` - the project ID
    * `workspace_id` - the workspace ID
  """
  def load_history_data(socket, scene, project_id, workspace_id) do
    versions =
      Scenes.list_versions(scene.id,
    ...
    can_name =
      Scenes.can_create_named_version?(project_id, workspace_id) == :ok
  ...
  def load_more_history(socket, scene_id, page) do
    offset = (page - 1) * @versions_per_page

    new_versions =
      Scenes.list_versions(scene_id,
        limit: @versions_per_page + 1,
        offset: offset
      )
  ...
  def detect_and_show_restore_preview(socket, scene, version, request_id) do
    case Scenes.prepare_version_restore(scene, version) do
      {:ok, :unsaved_changes} ->
