defmodule StoryarnWeb.SheetLive.Components.AudioTab do
    ...
    if Assets.allowed_content_type?(content_type) do
      ...
      case Billing.can_upload_asset_for_project?(project, byte_size(binary_data)) do
        ...
        {:error, :limit_reached, _details} ->
          send(
            self(),
            {:audio_tab, :error, dgettext("sheets", "Storage limit reached. Upgrade your plan.")}
          )
  ...
  defp do_process_upload(socket, project, node_id, filename, content_type, binary_data) do
