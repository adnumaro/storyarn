      ...
      <div class="max-w-lg mx-auto text-center py-12">
        <%= if @invitation do %>
          ...
          <.header>
            ...
            <:subtitle>
              {gettext("You've been invited to join a workspace on Storyarn")}
            </:subtitle>
          </.header>
  ...
  def handle_event("accept", _params, socket) do
    ...
    case Workspaces.accept_invitation(invitation, user) do
      {:ok, _membership} ->
        socket =
          socket
          |> put_flash(:info, gettext("Welcome to the workspace!"))
      ...
      {:error, :already_member} ->
        socket =
          socket
          |> put_flash(:info, gettext("You're already a member of this workspace."))
          |> push_navigate(to: ~p"/workspaces/#{invitation.workspace.slug}")
