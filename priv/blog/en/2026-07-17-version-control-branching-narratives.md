...
Storyarn's history did not begin with this research, but these sources help us judge it. We version units a designer recognizes: Sheets, Flows, and Scenes. Revisions can be named and compared as structured changes without reading JSON. When entity restore is enabled, the Sheet, Flow, and Scene paths all persist, reload, and verify the current state as a safety version immediately before changing anything. Required references are validated against their active project-owned targets; missing, malformed, or ambiguous dependencies block the restore instead of being silently cleared.
...
There are limits. Entity restore is independently contained for Sheets, Flows, and Scenes while each path completes operational verification, and automatic history capture is not triggered uniformly by every edit. Project snapshots do not yet reproduce an exact moment in time: they do not recreate everything deleted, remove everything created later, or include every area of the platform. We do not present them as a complete rollback.
...
Statements about Storyarn were checked against its [entity history implementation](https://github.com/adnumaro/storyarn/blob/fedc69771168ae8732afd08013153659bb0efbfb/lib/storyarn/versioning/version_crud.ex), [dependency analysis](https://github.com/adnumaro/storyarn/blob/fedc69771168ae8732afd08013153659bb0efbfb/lib/storyarn/versioning/conflict_detector.ex), [project snapshots](https://github.com/adnumaro/storyarn/blob/fedc69771168ae8732afd08013153659bb0efbfb/lib/storyarn/versioning/builders/project_snapshot_builder.ex), and current tests.
