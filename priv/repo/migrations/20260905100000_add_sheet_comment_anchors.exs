defmodule Storyarn.Repo.Migrations.AddSheetCommentAnchors do
  use Ecto.Migration

  def up do
    alter table(:comment_threads) do
      add :sheet_block_id, references(:blocks, on_delete: :nilify_all)
    end

    create index(:comment_threads, [:sheet_block_id])
    ...
    create constraint(:comment_threads, :comment_threads_source_type,
             check: "source_type IN ('flow_node', 'flow_canvas', 'scene_canvas', 'sheet_block')"
           )

    create constraint(:comment_threads, :comment_threads_anchor_shape,
             check:
               "(source_type = 'flow_node' AND flow_canvas_id IS NULL AND scene_canvas_id IS NULL AND sheet_block_id IS NULL) OR " <>
                 "(source_type = 'flow_canvas' AND flow_node_id IS NULL AND scene_canvas_id IS NULL AND sheet_block_id IS NULL AND source_id = container_id) OR " <>
                 "(source_type = 'scene_canvas' AND flow_node_id IS NULL AND flow_canvas_id IS NULL AND sheet_block_id IS NULL AND source_id = container_id) OR " <>
                 "(source_type = 'sheet_block' AND flow_node_id IS NULL AND flow_canvas_id IS NULL AND scene_canvas_id IS NULL)"
           )
    ...
    create constraint(:comment_threads, :comment_threads_position,
             check:
               "(source_type = 'flow_node' AND position_x IS NULL AND position_y IS NULL) OR " <>
                 ...
                 "(source_type IN ('scene_canvas', 'sheet_block') AND " <>
                 "position_x IS NOT NULL AND position_y IS NOT NULL AND " <>
    ...
    create constraint(:comment_threads, :comment_threads_anchor_identity,
             check:
               "(source_type = 'flow_node' AND (flow_node_id IS NULL OR flow_node_id = source_id)) OR " <>
                 ...
                 "(source_type = 'sheet_block' AND (sheet_block_id IS NULL OR sheet_block_id = source_id))"
           )
  end

  def down do
    raise Ecto.MigrationError,
          "AddSheetCommentAnchors is irreversible: rollback would remove Sheet block anchors and spatial metadata. " <>
