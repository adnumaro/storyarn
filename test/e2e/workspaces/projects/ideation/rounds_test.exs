defmodule StoryarnWeb.E2E.IdeationRoundsTest do
  ...
  test "a session starts quiet and the next round opens a new band below the notes without moving them",
    ...
    browser =
      ...
      |> assert_has("#brainstorming-round-prompt-#{first.id}", text: "Add a question")
      |> refute_has("#brainstorming-round-#{first.id}", text: "Round 1")
    ...
    browser =
      ...
      |> evaluate(@diagnostic, &IO.inspect(&1, label: "E2E_DIAG_BEFORE_N"))
      |> press("#brainstorming-canvas", "n")
      |> evaluate(@diagnostic, &IO.inspect(&1, label: "E2E_DIAG_AFTER_N"))
      |> evaluate("new Promise((resolve) => setTimeout(resolve, 600))")
      |> evaluate(@diagnostic, &IO.inspect(&1, label: "E2E_DIAG_AFTER_N_600MS"))
  ...
  test "the session tree opens a band and the parked list from deep links", %{conn: conn} do
    ...
    |> assert_has("#brainstorming-tree-round-#{second.id}", text: "R2")
    |> assert_has("#brainstorming-tree-round-#{second.id}", text: "Second question")
