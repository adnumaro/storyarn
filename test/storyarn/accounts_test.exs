defmodule Storyarn.AccountsTest do
  ...
  describe "find_or_create_user_from_oauth/2" do
    test "creates a new user when no user exists with the email" do
      auth = mock_oauth_auth("github", email: unique_user_email())
    ...
    test "returns existing user when identity already exists" do
      user = user_fixture()
      auth = mock_oauth_auth("github", email: user.email)
      {:ok, _identity} = Accounts.link_oauth_identity(user, "github", auth)

      assert {:ok, returned_user} = Accounts.find_or_create_user_from_oauth("github", auth)
      assert returned_user.id == user.id
    end

    test "links identity to existing user with same email" do
      user = user_fixture()
      auth = mock_oauth_auth("github", email: user.email)
  ...
  describe "get_identity_by_provider/2" do
    test "returns identity when it exists" do
      user = user_fixture()
      identity = user_identity_fixture(user, "github")

      found = Accounts.get_identity_by_provider("github", identity.provider_id)
      assert found.id == identity.id
    end

    test "returns nil when identity does not exist" do
      assert Accounts.get_identity_by_provider("github", "nonexistent") == nil
    end
  end

  describe "list_user_identities/1" do
    test "returns all identities for a user" do
      user = user_fixture()
      _github = user_identity_fixture(user, "github")
      _google = user_identity_fixture(user, "google")
  ...
  describe "link_oauth_identity/3" do
    test "creates a new identity for the user" do
      user = user_fixture()
      auth = mock_oauth_auth("github")

      assert {:ok, identity} = Accounts.link_oauth_identity(user, "github", auth)
      assert identity.user_id == user.id
      assert identity.provider == "github"
      assert identity.provider_id == to_string(auth.uid)
    end

    test "fails when trying to link same provider twice" do
      user = user_fixture()
      auth = mock_oauth_auth("github")

      assert {:ok, _identity} = Accounts.link_oauth_identity(user, "github", auth)

      # Try to link same provider with different uid - should fail due to user_id+provider constraint
      auth2 = mock_oauth_auth("github")
      assert {:error, changeset} = Accounts.link_oauth_identity(user, "github", auth2)
      assert "already linked" in errors_on(changeset).user_id
    end
  end
  ...
  describe "unlink_oauth_identity/2" do
    ...
    test "returns error when identity does not exist" do
      user = user_fixture()
      assert {:error, :not_found} = Accounts.unlink_oauth_identity(user, "github")
    end
    ...
    test "returns error when trying to unlink only auth method" do
      ...
      assert {:error, :cannot_unlink_only_auth_method} =
               Accounts.unlink_oauth_identity(user, "github")
    end
  end
