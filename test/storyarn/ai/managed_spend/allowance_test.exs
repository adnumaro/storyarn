defmodule Storyarn.AI.ManagedSpendTest do
  ...
  test "grant issuance is idempotent and conflicting reuse is rejected", ctx do
    ...
    assert {:ok, first} = ManagedSpend.grant(ctx.workspace.id, ctx.owner.id, attrs)
    assert {:ok, replayed} = ManagedSpend.grant(ctx.workspace.id, ctx.owner.id, attrs)
    assert replayed.id == first.id

    assert {:error, :grant_conflict} =
             ManagedSpend.grant(ctx.workspace.id, ctx.owner.id, %{attrs | units: 11})
  ...
  test "the expiration sweep advances through ordered account batches" do
    ...
    grants =
      for index <- 1..3 do
        ...
        assert {:ok, grant} =
                 ManagedSpend.grant(workspace.id, owner.id, %{
    ...
    assert %{
           ...
           } = ManagedSpend.expire_due(now, batch_size: 2)
    ...
    assert %{
           ...
           } = ManagedSpend.expire_due(now, batch_size: 2, after_account_id: next_account_id)
  ...
  describe "projection/1 (preflight)" do
    test "reports spendable units without touching the ledger", ctx do
      ...
      assert %{status: "active", available_units: 3} = ManagedSpend.projection(ctx.workspace.id)
    ...
    test "surfaces a paused account and an unknown workspace", ctx do
      ...
      assert {:ok, _account} = ManagedSpend.set_status(ctx.workspace.id, "paused")

      assert %{status: "paused", available_units: 2} = ManagedSpend.projection(ctx.workspace.id)
      assert %{status: "unavailable", available_units: 0} = ManagedSpend.projection(ctx.workspace.id + 100_000)
    end
