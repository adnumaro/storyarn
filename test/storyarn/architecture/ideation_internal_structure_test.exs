defmodule Storyarn.Architecture.IdeationInternalStructureTest do
  ...
  test "Decisions owns its write workflow and immutable agreement history" do
    ...
    assert "#{@root}/decisions/entities/*.ex" |> Path.wildcard() |> Enum.map(&Path.basename/1) ==
             ~w(application.ex decision.ex revision.ex task_link.ex)
  end
             ...
             Enum.sort(
               expected ++
                 ...
                 [
                   ...
                   restore_recovery: 2,
                   restore_recovery: 3,
                   get_contextual_brainstorming: 4,
                   get_contextual_brainstorming: 5,
                   create_contextual_session: 3,
                   link_contextual_session: 4,
                   resume_contextual_session: 5,
                   get_reference: 5,
                   list_references: 4,
                   list_references: 5,
             ...
             Enum.sort(
               expected ++
                 [
                   ...
                   lock_contextual_creation: 2,
                   notify_contextual_session: 2,
  ...
  test "the recovery inventory cannot silently omit newly persisted fields" do
    schemas = %{
      ...
      "decision_task_links" => Storyarn.Ideation.Decisions.TaskLink
    }
