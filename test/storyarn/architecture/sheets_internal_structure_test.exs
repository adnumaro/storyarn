defmodule Storyarn.Architecture.SheetsInternalStructureTest do
  ...
  test "Sheets has exactly the agreed capabilities and no loose implementation files" do
  ...
  test "the bounded-context facade composes only capability facades" do
    dependencies =
      ~r/^\s*alias\s+(Storyarn\.Sheets\.[A-Za-z0-9_.]+)/m
      |> Regex.scan(File.read!("lib/storyarn/sheets.ex"), capture: :all_but_first)
      |> List.flatten()
      |> Enum.sort()
  ...
  test "raw SQL and Platform object-storage contracts live only in adapters" do
    ...
    assert violations == [],
           "Sheet SQL and storage integrations must live under adapters/: #{inspect(violations)}"
  end
