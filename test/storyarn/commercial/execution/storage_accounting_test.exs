defmodule Storyarn.Commercial.Billing.StorageAccountingTest do
  ...
  describe "zero-byte snapshot export leases" do
    test "publishes only after an outer Commercial transaction commits and discards rollbacks", context do
      committed_snapshot =
        insert_full_snapshot!(context.project, 1, %{project: 10, metadata: 10, assets: 10})
      ...
      assert :ok = Commercial.subscribe_project_snapshot_export_leases(context.project.id)

      assert {:ok, %StorageReservation{} = committed_lease} =
               Commercial.transact_with_workspace_lock(context.workspace.id, fn _workspace ->
                 assert {:ok, lease} =
                          context
                          |> reservation_attrs(
                 ...
                 refute_received {:commercial_snapshot_export_lease_state_invalidated, _snapshot_id}
                 {:ok, lease}
               end)

      assert committed_lease.project_snapshot_id_snapshot == committed_snapshot.id

      assert_receive {:commercial_snapshot_export_lease_state_invalidated, committed_snapshot_id}
      assert committed_snapshot_id == committed_snapshot.id

      assert {:error, :rollback_requested} =
               Commercial.transact_with_workspace_lock(context.workspace.id, fn _workspace ->
                 assert {:ok, _lease} =
                          context
                          |> reservation_attrs(
                 ...
                 refute_received {:commercial_snapshot_export_lease_state_invalidated, _snapshot_id}
                 {:error, :rollback_requested}
               end)

      refute_receive {:commercial_snapshot_export_lease_state_invalidated, _snapshot_id}

      refute Repo.get_by(StorageReservation,
               project_snapshot_id_snapshot: rolled_back_snapshot.id,
               kind: "snapshot_export",
               status: "active"
             )
    end

    test "publishes one project-scoped invalidation per successful acquire, release, or replay", context do
      snapshot = insert_full_snapshot!(context.project, 1, %{project: 10, metadata: 10, assets: 10})
      snapshot_id = snapshot.id
      attrs = reservation_attrs(context, "event-export", "snapshot_export", 0, snapshot)
      ...
      assert {:ok, renewed} = Billing.acquire_snapshot_export_lease(attrs)
    ...
    test "rejects invalid subscriptions and does not publish unrelated or failed mutations", context do
      assert {:error, :invalid_project_id} = Commercial.subscribe_project_snapshot_export_leases(0)
      assert {:error, :invalid_project_id} = Commercial.subscribe_project_snapshot_export_leases("invalid")
      assert :ok = Commercial.subscribe_project_snapshot_export_leases(context.project.id)

      other_project = project_fixture(context.user, workspace: context.workspace)

      other_snapshot =
        insert_full_snapshot!(other_project, 1, %{project: 10, metadata: 10, assets: 10})
      ...
      assert {:ok, _lease} = Billing.acquire_snapshot_export_lease(other_attrs)

      own_snapshot =
        insert_full_snapshot!(context.project, 2, %{project: 10, metadata: 10, assets: 10})

      assert {:ok, _reservation} =
               reserve(context, "non-export", "restore_staging", 1, own_snapshot)

      assert {:error, _reason} = Billing.acquire_snapshot_export_lease(%{})
      refute_receive {:commercial_snapshot_export_lease_state_invalidated, _snapshot_id}
    end
    ...
    test "retention recovery releases only expired zero-byte export leases", context do
      ...
      Commercial.subscribe_project_snapshot_export_leases(context.project.id)
