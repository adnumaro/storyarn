defmodule Storyarn.Flows.AI.SourceLocksTest do
  ...
  # These Flow-owned locks cover `dialogue`, which reaches flow, flow_node,
  # sheet and sheet_block, and `flow_neighborhood`, which reaches
  # flow_connection.
  #
  ...
  defp dialogue_operation(%{scope: scope, project: project, dialogue_node: node}) do
    {:ok, ref} = ContextContract.dialogue(project.workspace_id, project.id, node.id)
    operation_for(scope, project, :dialogue, ref)
  end

  defp neighborhood_operation(%{scope: scope, project: project, dialogue_node: node}) do
    {:ok, ref} = ContextContract.flow_neighborhood(project.workspace_id, project.id, node.id)
