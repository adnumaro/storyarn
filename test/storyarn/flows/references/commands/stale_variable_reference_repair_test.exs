defmodule Storyarn.Flows.StaleVariableReferenceRepairTest do
  ...
  test "rejects invalid PostgreSQL bigint identities without reaching Ecto" do
    ...
    for project_id <- @invalid_project_ids do
      assert {:error, :not_found} = Flows.repair_stale_variable_references(scope, project_id)
      assert {:error, :not_found} = Projects.repair_stale_project_variable_references(scope, project_id)
    end
  end
end
