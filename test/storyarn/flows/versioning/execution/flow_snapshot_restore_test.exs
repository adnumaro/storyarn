defmodule Storyarn.Flows.Versioning.FlowSnapshotRestoreTest do
  # Table-locking rollback coverage lives in FlowSnapshotRestoreRollbackTest.
  ...
  alias Storyarn.Flows.Versioning.EntityVersionRecord, as: EntityVersion
  ...
  describe "restore_snapshot/3" do
    test "rejects a restore when its verified safety-version record is no longer durable", %{
      ...
      safety_version =
        %EntityVersion{}
        |> EntityVersion.create_changeset(%{
          entity_type: "flow",
          entity_id: flow.id,
