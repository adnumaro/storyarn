defmodule Storyarn.Localization.Access.Commands.ReferenceIntegrityTest do
  ...
  test "requires an explicit transaction for project and reference locks" do
    ...
    assert_raise ArgumentError, ~r/explicit database transaction/, fn ->
      Access.lock_active_project(project.id)
    end
  end
    ...
    assert {:ok, :verified} =
             Repo.transaction(fn ->
               assert {:ok, %ProjectRecord{id: project_id}} =
                        Access.lock_active_project(Integer.to_string(project.id), :update)

               assert project_id == project.id

               assert {:ok, [asset_id, sheet_id, nil]} =
                        Access.lock_active_references(project.id, [
               ...
               assert :ok =
                        Access.ensure_locked_asset_content_type(
                          project.id,
                          asset.id,
                          :voiceover,
                          "audio/%"
