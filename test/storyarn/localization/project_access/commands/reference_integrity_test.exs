defmodule Storyarn.Localization.ProjectAccess.Commands.ReferenceIntegrityTest do
  ...
  alias Storyarn.Localization.ProjectAccess.Projections.ProjectRecord
  alias Storyarn.Repo
  ...
  test "requires an explicit transaction for project and reference locks" do
    ...
    assert_raise ArgumentError, ~r/explicit database transaction/, fn ->
      ProjectAccess.lock_active_project(project.id)
    end
  end
    ...
    assert {:ok, :verified} =
             Repo.transaction(fn ->
               assert {:ok, %ProjectRecord{id: project_id}} =
                        ProjectAccess.lock_active_project(Integer.to_string(project.id), :update)

               assert project_id == project.id

               assert {:ok, [asset_id, sheet_id, nil]} =
                        ProjectAccess.lock_active_references(project.id, [
               ...
               assert :ok =
                        ProjectAccess.ensure_locked_asset_content_type(
                          project.id,
                          asset.id,
                          :voiceover,
                          "audio/%"
