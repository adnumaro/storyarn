defmodule Storyarn.Localization.Translation.Execution.BatchTranslatorTest do
  ...
  describe "translate_batch/3" do
    ...
    test "translates pending texts in bounded cursor pages without skipping rows" do
      ...
      assert Texts.count_texts(project.id, locale_code: "es", status: "pending") == 0
      assert Texts.count_texts(project.id, locale_code: "es", status: "draft") == 5
    end
    ...
    test "honors total limit while loading only bounded pages" do
      ...
      assert Texts.count_texts(project.id, locale_code: "es", status: "pending") == 2
      assert Texts.count_texts(project.id, locale_code: "es", status: "draft") == 3
    end
    ...
    test "stops before loading another page when cancellation is requested" do
      ...
      assert_received {:progress, 2}
      assert Texts.count_texts(project.id, locale_code: "es", status: "draft") == 2
      assert Texts.count_texts(project.id, locale_code: "es", status: "pending") == 1
    end
  end
