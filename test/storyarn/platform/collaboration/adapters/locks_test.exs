defmodule Storyarn.Platform.Collaboration.LocksTest do
  use Storyarn.DataCase, async: false

  alias Storyarn.Platform.Collaboration.Locks
  ...
  describe "acquire/3" do
    test "acquires a lock on an unlocked node" do
      ...
      assert {:ok, lock_info} = Locks.acquire(@scope, @node_id, user)
    ...
    test "same user can re-acquire (refresh) their own lock" do
      ...
      {:ok, lock1} = Locks.acquire(@scope, @node_id, user)
      {:ok, lock2} = Locks.acquire(@scope, @node_id, user)
    ...
    test "different user cannot acquire a lock held by another user" do
      ...
      {:ok, _lock} = Locks.acquire(@scope, @node_id, user1)

      assert {:error, :already_locked, lock_info} = Locks.acquire(@scope, @node_id, user2)
      assert lock_info.user_id == 1
    end
    ...
    test "can acquire locks on different nodes in the same flow" do
      ...
      assert {:ok, _} = Locks.acquire(@scope, 10, user)
      assert {:ok, _} = Locks.acquire(@scope, 20, user)
    end
    ...
    test "different users can lock different nodes" do
      ...
      assert {:ok, _} = Locks.acquire(@scope, 10, user1)
      assert {:ok, _} = Locks.acquire(@scope, 20, user2)
    end
  end
  ...
  describe "release/3" do
    test "lock holder can release their lock" do
      ...
      assert :ok = Locks.release(@scope, @node_id, user.id)
    ...
    test "non-holder cannot release a lock" do
      ...
      {:ok, _} = Locks.acquire(@scope, @node_id, user1)

      assert {:error, :not_lock_holder} = Locks.release(@scope, @node_id, 2)
    end

    test "releasing a non-existent lock returns :ok" do
      assert :ok = Locks.release(@scope, 999, 1)
    end
  end
  ...
  describe "release_all/2" do
    test "releases all locks for a user in a flow" do
      ...
      {:ok, _} = Locks.acquire(@scope, 20, user)
      {:ok, _} = Locks.acquire(@scope, 30, user)

      assert :ok = Locks.release_all(@scope, user.id)

      assert {:error, :not_locked} = Locks.get_lock(@scope, 10)
      assert {:error, :not_locked} = Locks.get_lock(@scope, 20)
      assert {:error, :not_locked} = Locks.get_lock(@scope, 30)
    end
    ...
    test "does not release locks from other users in the same flow" do
      ...
      {:ok, _} = Locks.acquire(@scope, 10, user1)
      {:ok, _} = Locks.acquire(@scope, 20, user2)

      :ok = Locks.release_all(@scope, user1.id)

      assert {:error, :not_locked} = Locks.get_lock(@scope, 10)
      assert {:ok, lock} = Locks.get_lock(@scope, 20)
    ...
    test "does not release locks in other flows" do
      ...
      {:ok, _} = Locks.acquire({:flow, 1}, @node_id, user)
      {:ok, _} = Locks.acquire({:flow, 2}, @node_id, user)

      :ok = Locks.release_all({:flow, 1}, user.id)

      assert {:error, :not_locked} = Locks.get_lock({:flow, 1}, @node_id)
  ...
  describe "refresh/3" do
    test "lock holder can refresh their lock" do
      user = make_user(1)
      {:ok, original} = Locks.acquire(@scope, @node_id, user)

      # Small delay to get a different timestamp
      Process.sleep(1)

      assert :ok = Locks.refresh(@scope, @node_id, user.id)

      {:ok, refreshed} = Locks.get_lock(@scope, @node_id)
    ...
    test "non-holder cannot refresh a lock" do
      ...
      assert {:error, :not_lock_holder} = Locks.refresh(@scope, @node_id, 2)
    end

    test "cannot refresh a non-existent lock" do
      assert {:error, :not_lock_holder} = Locks.refresh(@scope, 999, 1)
    end
  end
  ...
  describe "get_lock/2" do
    test "returns lock info for a locked node" do
      ...
      assert {:ok, lock_info} = Locks.get_lock(@scope, @node_id)
      assert lock_info.user_id == 1
    end

    test "returns not_locked for an unlocked node" do
      assert {:error, :not_locked} = Locks.get_lock(@scope, 999)
    end
  end
  ...
  describe "list_locks/1" do
    ...
    test "returns empty map when no locks exist" do
      assert Locks.list_locks({:flow, 999}) == %{}
    end
  ...
  describe "locked_by_other?/3" do
    test "returns true when locked by a different user" do
      ...
      assert Locks.locked_by_other?(@scope, @node_id, 2)
    end
    ...
    test "returns false when locked by the same user" do
      ...
      refute Locks.locked_by_other?(@scope, @node_id, 1)
    end

    test "returns false when not locked" do
      refute Locks.locked_by_other?(@scope, 999, 1)
    end
  end
  ...
  describe "clear_all/0" do
    test "removes all locks" do
      user = make_user(1)
      {:ok, _} = Locks.acquire({:flow, 1}, 10, user)
      {:ok, _} = Locks.acquire({:flow, 2}, 20, user)

      assert :ok = Locks.clear_all()

      assert {:error, :not_locked} = Locks.get_lock({:flow, 1}, 10)
      assert {:error, :not_locked} = Locks.get_lock({:flow, 2}, 20)
    end
  end
end
