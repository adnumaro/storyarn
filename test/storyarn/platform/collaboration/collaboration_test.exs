defmodule Storyarn.CollaborationTest do
  ...
  describe "Collaboration facade" do
    ...
    test "presence teardown stays behind the facade" do
      scope = {:flow, System.unique_integer([:positive])}

      assert :ok = Collaboration.subscribe_presence(scope)
      assert :ok = Collaboration.unsubscribe_presence(scope)
      assert :ok = Collaboration.untrack_presence(self(), scope, 42)
    end
  end
  ...
  describe "dashboard compatibility facade" do
    test "delegates the stable topic and invalidation contract to DashboardCache" do
      project_id = System.unique_integer([:positive])

      assert Collaboration.dashboard_topic(project_id) ==
               Cache.dashboard_topic(project_id)
    ...
    test "delegates result broadcasting without changing success or error results" do
      ...
      success = {:ok, %{id: project_id}}
      assert Collaboration.broadcast_dashboard_result(success, project_id, :sheets) == success
      assert_receive {:dashboard_invalidate, :sheets}

      error = {:error, :invalid}
      assert Collaboration.broadcast_dashboard_result(error, project_id, :sheets) == error
