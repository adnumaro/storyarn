defmodule Storyarn.Projects.Versioning.SnapshotStorageTest do
  ...
  describe "store_snapshot/5" do
    test "stores compressed JSON and returns key and size" do
      ...
      assert key =~ ~r|\Aprojects/1/snapshots/sheet/42/1-[0-9a-f]{16}\.json\.gz\z|
      assert size_bytes > 0
    end
  ...
  describe "load_snapshot/1" do
    ...
    test "returns a checksum for the exact compressed bytes stored" do
      key = "project_templates/test/snapshot-storage/checksummed.json.gz"
    ...
    test "streams snapshots spanning multiple storage chunks" do
      key =
        "project_templates/test/snapshot-storage/multi-chunk-#{SnapshotStorage.unique_key_suffix()}.json.gz"
  ...
  describe "load_verified_snapshot/4" do
    test "loads only after verifying compressed metadata" do
      key = "project_templates/test/snapshot-storage/verified.json.gz"
    ...
    test "rejects a compressed-size mismatch before decompression" do
      key = "project_templates/test/snapshot-storage/size-mismatch.json.gz"
    ...
    test "rejects a checksum mismatch before decompression" do
      key = "project_templates/test/snapshot-storage/checksum-mismatch.json.gz"
      snapshot = %{"payload" => String.duplicate("compressible", 100)}
    ...
    test "rejects same-size invalid gzip bytes by checksum before inflation" do
      key = "project_templates/test/snapshot-storage/invalid-gzip-checksum.json.gz"
      snapshot = %{"payload" => String.duplicate("compressible", 100)}
    ...
    test "stops incremental inflation above the uncompressed limit" do
      key = "project_templates/test/snapshot-storage/inflate-limit.json.gz"
    ...
    test "rejects an expected compressed size above the configured limit" do
      key = "project_templates/test/snapshot-storage/expected-compressed-limit.json.gz"
      snapshot = %{"payload" => String.duplicate("compressible", 100)}
    ...
    test "rejects an oversized storage stat before accepting mismatched metadata" do
      key = "project_templates/test/snapshot-storage/stat-compressed-limit.json.gz"
      snapshot = %{"payload" => String.duplicate("compressible", 100)}
    ...
    test "applies compressed and uncompressed bounds to unverified snapshot loads" do
      key = "project_templates/test/snapshot-storage/unverified-load-limits.json.gz"
  ...
  describe "write size limits" do
    ...
    test "store_raw_with_checksum rejects oversized compressed bytes before upload" do
      key =
        "project_templates/test/snapshot-storage/raw-compressed-limit-#{SnapshotStorage.unique_key_suffix()}.json.gz"
    ...
    test "store_raw uses the same write bounds" do
      key =
        "project_templates/test/snapshot-storage/raw-limit-#{SnapshotStorage.unique_key_suffix()}.json.gz"
    ...
    test "a stored snapshot is loadable under the same limits" do
      key =
        "project_templates/test/snapshot-storage/write-read-limits-#{SnapshotStorage.unique_key_suffix()}.json.gz"
  ...
  describe "build_key/5" do
    test "builds an attempt-owned key" do
      assert SnapshotStorage.build_key(5, "flow", 10, 3, "0123456789abcdef") ==
