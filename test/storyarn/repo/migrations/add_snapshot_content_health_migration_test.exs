defmodule Storyarn.Repo.Migrations.AddSnapshotContentHealthMigrationTest do
  ...
  test "insert rejects unknown, blocked, and malformed health but accepts assessed restorable reports", %{
    ...
    unknown_id = insert_snapshot!(prefix, unknown_health())
    blocked_id = insert_snapshot!(prefix, blocked_health())

    malformed_health = put_in(healthy_health(), ["impact_counts", "restore_blocked"], "0")

    malformed_id = insert_snapshot!(prefix, malformed_health)
    legacy_id = insert_snapshot!(prefix, legacy_health())
    healthy_id = insert_snapshot!(prefix, healthy_health())
  ...
  test "backfills only finalized strict ready snapshots and keeps the evidence one-shot and immutable", %{
    ...
    assert_content_health_update_rejected(prefix, legacy_id, healthy_health())

    post_migration_id = insert_strict_ready_snapshot!(prefix)

    assert persisted_health(prefix, post_migration_id) == unknown_health()
  ...
  test "every transition into or within an active restore status revalidates content health", %{
    ...
    healthy_snapshot_id = insert_snapshot!(prefix, healthy_health())
  ...
  defp blocked_health do
    warning_health("restore_blocked", "error")
  end

  defp runtime_warning_health do
    warning_health("runtime_degraded", "warning")
  end

  defp unknown_health, do: empty_health("unknown")
  defp legacy_health, do: empty_health("legacy_strict")
  defp healthy_health, do: empty_health("healthy")

  defp empty_health(state) do
    %{
  ...
  defp warning_health(impact, severity) do
    issue = %{
    ...
    %{
      "impact_counts" => Map.put(%{"restore_blocked" => 0, "runtime_degraded" => 0}, impact, 1),
      "issue_count" => 1,
      "issue_counts_by_code" => %{"capture.unclassified_content_issue" => 1},
      "issues" => [issue],
      "issues_truncated" => false,
      "severity_counts" => Map.put(%{"error" => 0, "warning" => 0, "info" => 0}, severity, 1),
      "state" => "warnings",
      "version" => 1
    }
  end
