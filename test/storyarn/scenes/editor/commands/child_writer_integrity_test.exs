defmodule Storyarn.Scenes.Editor.Commands.ChildWriterIntegrityTest do
  ...
  alias Storyarn.Scenes.Editor.Commands.AmbientFlows
  alias Storyarn.Scenes.Editor.Commands.Annotations
  alias Storyarn.Scenes.Editor.Commands.Layers
  alias Storyarn.Scenes.Editor.Commands.Tree
  alias Storyarn.Scenes.Editor.Queries.Layers, as: LayerQueries
  ...
  describe "layer source and child integrity" do
    test "all layer writers reject a scene in trash without changing its layers" do
      ...
      layer_ids = scene.id |> LayerQueries.list_layers() |> Enum.map(& &1.id)
      soft_delete!(scene)

      assert {:error, :scene_not_active} =
               Layers.create_layer(scene.id, %{"name" => "Forbidden"})

      assert {:error, :scene_not_active} =
               Layers.update_layer(layer, %{"name" => "Forbidden"})

      assert {:error, :scene_not_active} =
               Layers.toggle_layer_visibility(layer)

      assert {:error, :scene_not_active} =
               Layers.delete_layer(layer)

      assert {:error, :scene_not_active} =
               Layers.reorder_layers(scene.id, Enum.reverse(layer_ids))

      persisted = Repo.get!(SceneLayer, layer.id)
      assert persisted.name == "Protected"
      assert persisted.visible
      assert Enum.map(LayerQueries.list_layers(scene.id), & &1.id) == layer_ids
    end
    ...
    test "reorder rejects duplicate, malformed and foreign layer IDs atomically" do
      ...
      assert {:error, {:invalid_scene_layer_reorder, _ids}} =
               Layers.reorder_layers(scene.id, [local_layer.id, local_layer.id])

      assert positions(SceneLayer, scene_id: scene.id) == original

      assert {:error, {:invalid_scene_layer_reorder, _ids}} =
               Layers.reorder_layers(scene.id, [local_layer.id, "invalid"])

      assert positions(SceneLayer, scene_id: scene.id) == original

      assert {:error, {:invalid_scene_layer_reorder, _ids}} =
               Layers.reorder_layers(scene.id, [local_layer.id, foreign_layer.id])
  ...
  describe "annotation source, child and effective-layer integrity" do
    test "all annotation writers reject a scene in trash" do
      ...
      assert {:error, :scene_not_active} =
               Annotations.create_annotation(scene.id, annotation_attrs("Forbidden"))

      assert {:error, :scene_not_active} =
               Annotations.update_annotation(annotation, %{"text" => "Forbidden"})

      assert {:error, :scene_not_active} =
               Annotations.move_annotation(annotation, 1.0, 2.0)
    ...
    test "unrelated updates re-read the row and validate its persisted effective layer" do
      ...
      assert {:ok, updated} =
               Annotations.update_annotation(stale_annotation, %{
                 "font_size" => "lg"
               })
      ...
      assert {:error, {:scene_layer_ownership_mismatch, ^foreign_layer_id, ^scene_id, ^foreign_scene_id}} =
               Annotations.move_annotation(stale_annotation, 1.0, 2.0)
    ...
    test "a forged scene owner cannot update or delete another scene's annotation" do
      ...
      assert {:error, :annotation_not_found} =
               Annotations.update_annotation(forged_annotation, %{
                 "text" => "Forbidden"
               })

      assert {:error, :annotation_not_found} =
               Annotations.delete_annotation(forged_annotation)
  ...
  describe "ambient flow source, child and reorder integrity" do
    test "delete and reorder reject a scene in trash" do
      ...
      {:ok, ambient_flow} = AmbientFlows.create_ambient_flow(scene.id, %{"flow_id" => flow.id})
      soft_delete!(scene)

      assert {:error, :scene_not_active} =
               AmbientFlows.delete_ambient_flow(ambient_flow)

      assert {:error, :scene_not_active} =
               AmbientFlows.reorder_ambient_flows(scene.id, [ambient_flow.id])
    ...
    test "all ambient-flow writers reject an active scene whose project is in trash" do
      ...
      assert {:error, :project_not_active} =
               AmbientFlows.create_ambient_flow(scene.id, %{
                 "flow_id" => replacement_flow.id
               })

      assert {:error, :project_not_active} =
               AmbientFlows.update_ambient_flow(ambient_flow, %{
                 "flow_id" => replacement_flow.id
               })
    ...
    test "reorder rejects duplicate, malformed and foreign IDs atomically" do
      ...
      {:ok, first} =
        AmbientFlows.create_ambient_flow(scene.id, %{"flow_id" => first_flow.id})

      {:ok, second} =
        AmbientFlows.create_ambient_flow(scene.id, %{"flow_id" => second_flow.id})

      {:ok, foreign} =
        AmbientFlows.create_ambient_flow(other_scene.id, %{
      ...
      assert {:error, {:invalid_scene_ambient_flow_reorder, _ids}} =
               AmbientFlows.reorder_ambient_flows(scene.id, [second.id, second.id])

      assert positions(SceneAmbientFlow, scene_id: scene.id) == original

      assert {:error, {:invalid_scene_ambient_flow_reorder, _ids}} =
               AmbientFlows.reorder_ambient_flows(scene.id, [second.id, "invalid"])

      assert positions(SceneAmbientFlow, scene_id: scene.id) == original

      assert {:error, {:invalid_scene_ambient_flow_reorder, _ids}} =
               AmbientFlows.reorder_ambient_flows(scene.id, [second.id])

      assert positions(SceneAmbientFlow, scene_id: scene.id) == original

      assert {:error, {:invalid_scene_ambient_flow_reorder, []}} =
               AmbientFlows.reorder_ambient_flows(scene.id, [])

      assert positions(SceneAmbientFlow, scene_id: scene.id) == original

      assert {:error, {:invalid_scene_ambient_flow_reorder, _ids}} =
               AmbientFlows.reorder_ambient_flows(scene.id, [second.id, foreign.id])
    ...
    test "delete re-reads the child and rejects a forged scene owner" do
      ...
      assert {:error, :ambient_flow_not_found} =
               AmbientFlows.delete_ambient_flow(forged_ambient_flow)
  ...
  describe "scene tree reorder integrity" do
    test "rejects a project in trash without changing positions" do
      ...
      assert {:error, :project_not_active} =
               Tree.reorder_scenes(project.id, nil, [second.id, first.id])
    ...
    test "rejects foreign and deleted parents" do
      ...
      assert {:error, {:invalid_project_reference, _context, foreign_parent_id}} =
               Tree.reorder_scenes(project.id, foreign_parent.id, [scene.id])

      assert foreign_parent_id == foreign_parent.id

      assert {:error, {:invalid_project_reference, _context, deleted_parent_id}} =
               Tree.reorder_scenes(project.id, deleted_parent.id, [scene.id])
    ...
    test "rejects duplicate, malformed, foreign-container and trashed scene IDs atomically" do
      ...
      assert {:error, {:invalid_scene_reorder, _ids}} =
               Tree.reorder_scenes(project.id, parent.id, [first.id, first.id])
      ...
      assert {:error, {:invalid_scene_reorder, _ids}} =
               Tree.reorder_scenes(project.id, parent.id, [first.id, "invalid"])

      assert {:error, {:invalid_scene_reorder, _ids}} =
               Tree.reorder_scenes(project.id, parent.id, [first.id, root_scene.id])

      assert {:error, {:invalid_scene_reorder, _ids}} =
               Tree.reorder_scenes(project.id, parent.id, [first.id, trashed.id])
      ...
      assert {:ok, reordered} =
               Tree.reorder_scenes(project.id, Integer.to_string(parent.id), [
                 Integer.to_string(second.id),
                 Integer.to_string(first.id)
               ])
