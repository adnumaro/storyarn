defmodule Storyarn.Scenes.Editor.Commands.PositionsTest do
  ...
  describe "next_position/2" do
    test "returns 0 when no items exist" do
      ...
      assert Positions.next_position(SceneLayer, -1) == 0
    end
    ...
    test "returns max + 1 when items exist" do
      ...
      assert Positions.next_position(SceneLayer, map.id) == 1
    end
  end
  ...
  test "scene lock serializes concurrent zone positions and shortcuts" do
    ...
    zones =
      [attrs, attrs]
      |> Task.async_stream(&Editor.create_zone(scene.id, &1),
        max_concurrency: 2,
        ordered: false,
        timeout: :infinity
      )
