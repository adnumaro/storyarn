defmodule Storyarn.Scenes.Editor.Commands.TreeTest do
  ...
  describe "reorder_scenes/3" do
    test "reorders scenes at root level" do
      ...
      # Reverse the order
      {:ok, reordered} =
        Tree.reorder_scenes(project.id, nil, [scene_c.id, scene_b.id, scene_a.id])
    ...
    test "reorders scenes within a parent" do
      ...
      {:ok, reordered} =
        Tree.reorder_scenes(project.id, parent.id, [child_b.id, child_a.id])
  ...
  describe "move_scene_to_position/3" do
    test "moves scene to a different parent" do
      ...
      {:ok, moved} = Tree.move_scene_to_position(child, parent_b.id, 0)
    ...
    test "moves scene to root level" do
      ...
      {:ok, moved} = Tree.move_scene_to_position(child, nil, 0)
    ...
    test "prevents cyclic parent assignment" do
      ...
      assert {:error, :cyclic_parent} =
               Tree.move_scene_to_position(parent, child.id, 0)
    end
  end
  ...
  describe "next_position/2" do
    test "returns 0 when no scenes exist" do
      %{project: project} = create_project()
      assert Tree.next_position(project.id, nil) == 0
    end
    ...
    test "returns next position after existing scenes" do
      ...
      assert Tree.next_position(project.id, nil) >= 1
    end
  end
  ...
  describe "list_scenes_by_parent/2" do
    test "lists root-level scenes" do
      ...
      scenes = Tree.list_scenes_by_parent(project.id, nil)
    ...
    test "lists child scenes" do
      ...
      children = Tree.list_scenes_by_parent(project.id, parent.id)
