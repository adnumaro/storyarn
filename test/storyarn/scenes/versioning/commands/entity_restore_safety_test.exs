defmodule Storyarn.Scenes.Versioning.Commands.EntityRestoreSafetyTest do
  use ExUnit.Case, async: true

  alias Storyarn.Scenes.Versioning.Commands.EntityRestoreSafety
  alias Storyarn.Scenes.Versioning.EntityVersionRecord, as: EntityVersion
    ...
    test "rejects an explicitly invalid pre-restore snapshot" do
      assert {:error, :invalid_pre_restore_snapshot} =
               EntityRestoreSafety.verify_pre_restore_baseline(
                 "scene",
                 entity(),
                 [restore_action: {:entity_version_restore, "scene"}, pre_restore_snapshot: []],
                 fn _entity -> raise ArgumentError, "must not run" end,
                 :scene_changed_since_pre_restore_snapshot
               )
    end
    ...
    struct!(
      ...
      Keyword.merge(
        [
          ...
          created_by_id: 33,
          version_number: 2,
          storage_key: "entity-versions/scene/11/2.json.gz",
