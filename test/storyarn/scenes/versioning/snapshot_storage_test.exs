defmodule Storyarn.Scenes.Versioning.SnapshotStorageTest do
  ...
  test "rejects persisted size metadata before reading snapshot bytes" do
    snapshot = %{"scene" => %{"name" => "Wrong size"}}
    {key, size_bytes, checksum} = store_snapshot(snapshot)
  ...
  test "rejects a checksum mismatch before starting inflation" do
    snapshot = %{"scene" => %{"name" => "Wrong checksum"}}
    {key, size_bytes, checksum} = store_snapshot(snapshot)
  ...
  test "enforces compressed and incremental uncompressed limits for a highly compressible snapshot" do
  ...
  test "binds both reads to one ETag and detects replacement between checksum and inflation" do
    snapshot = %{"scene" => %{"name" => "Immutable read"}}
    {key, size_bytes, checksum} = store_snapshot(snapshot)
    ...
    assert_receive {:snapshot_stream_read, 1, [etag: "immutable-scene-snapshot"]}
    assert_receive {:snapshot_stream_read, 2, [etag: "immutable-scene-snapshot"]}
    assert Agent.get(read_counter, & &1) == 2
  end

  test "reads default limits from the Scene-owned configuration namespace" do
