defmodule Storyarn.Sheets.Editor.Commands.InheritanceTest do
  ...
  describe "list_health_issues/1" do
    ...
    test "returns no issues for current inherited instances", %{parent: parent, child: child} do
      ...
      assert Inheritance.list_health_issues(child.id) == []
    end
    ...
    test "detects missing and stale inherited instances", %{parent: parent, child: child} do
      ...
      issues = Inheritance.list_health_issues(child.id)
    ...
    test "detects stale inherited table structure", %{parent: parent, child: child} do
      ...
      assert Enum.any?(
               Inheritance.list_health_issues(child.id),
    ...
    test "ignores instances intentionally hidden on the sheet", %{parent: parent, child: child} do
      ...
      assert {:ok, _hidden_sheet} = Inheritance.hide_for_children(child, source.id)
    ...
    test "ignores hidden instances on a sheet with no ancestors too", %{parent: parent, child: child} do
      ...
      project_issues = [root] |> Inheritance.list_project_health_issues() |> Map.fetch!(root.id)

      assert Inheritance.list_health_issues(root.id) == project_issues
  ...
  describe "resolve_inherited_blocks/1" do
    ...
    test "returns inherited blocks from parent", %{parent: parent, child: child} do
      ...
      groups = Inheritance.resolve_inherited_blocks(child.id)
    ...
    test "returns blocks from multiple ancestors", %{
      ...
      groups = Inheritance.resolve_inherited_blocks(grandchild.id)
    ...
    test "filters hidden blocks from grandchild", %{
      ...
      {:ok, _} = Inheritance.hide_for_children(child, block.id)
    ...
    test "returns empty list for root sheet", %{parent: parent} do
      assert Inheritance.resolve_inherited_blocks(parent.id) == []
    end
  ...
  describe "create_inherited_instances/2" do
    ...
    test "skips already-existing instances", %{parent: parent, child: child} do
      ...
      {:ok, count} = Inheritance.create_inherited_instances(block, [child.id])
    ...
    test "returns {:ok, 0} for empty list", %{parent: parent} do
      ...
      {:ok, count} = Inheritance.create_inherited_instances(block, [])
      assert count == 0
    end
    ...
    test "rejects targets in trash or outside the source project", %{
      ...
      assert {:error, {:invalid_inheritance_targets, invalid_ids}} =
               Inheritance.create_inherited_instances(
                 block,
  ...
  describe "sync_definition_change/1" do
    ...
    test "clears value when type changes", %{parent: parent, child: child} do
      ...
      Inheritance.sync_definition_change(updated)
    ...
    test "skips detached instances", %{parent: parent, child: child} do
      ...
      child_blocks = Sheets.list_blocks(child.id)
      instance = Enum.find(child_blocks, &(&1.inherited_from_block_id == block.id))
      {:ok, _} = Inheritance.detach_block(instance)
    ...
    test "is atomic (transaction)", %{parent: parent} do
      ...
      {:ok, count} = Inheritance.sync_definition_change(block)
      assert count >= 2
    end
    ...
    test "updates instances in active and trashed sheets inside the source project", %{
      ...
      assert {:ok, 2} = Inheritance.sync_definition_change(updated_source)
  ...
  describe "reattach_block/1" do
    ...
    test "resets definition from source", %{parent: parent, child: child} do
      ...
      # Detach first
      {:ok, detached} = Inheritance.detach_block(instance)
      assert detached.detached == true
      ...
      {:ok, reattached} = Inheritance.reattach_block(detached)
    ...
    test "returns error when source permanently deleted", %{parent: parent, child: child} do
      ...
      assert {:error, :source_not_found} = Inheritance.reattach_block(detached)
    end
  end
  ...
  describe "hide_for_children/2" do
    ...
    test "is idempotent", %{parent: parent, child: child} do
      ...
      {:ok, first} = Inheritance.hide_for_children(child, block.id)
      {:ok, second} = Inheritance.hide_for_children(first, block.id)
  ...
  describe "unhide_for_children/2" do
    ...
    test "is idempotent", %{parent: parent, child: child} do
      ...
      {:ok, result} = Inheritance.unhide_for_children(child, block.id)
  ...
  describe "delete_inherited_instances/1" do
    ...
    test "soft-deletes all instances", %{parent: parent, child: child, grandchild: grandchild} do
      ...
      {:ok, count} = Inheritance.delete_inherited_instances(block)
    ...
    test "preserves detached instances as local copies", %{
      ...
      assert {:ok, detached} = Inheritance.detach_block(instance)
      assert {:ok, 1} = Inheritance.delete_inherited_instances(block)
    ...
    test "cleans up entity references", %{project: project, parent: parent, child: child} do
      ...
      References.update_block_references(updated_instance)

      backlinks_before = References.get_backlinks("sheet", target_sheet.id)
      assert [_ | _] = backlinks_before

      # Now delete inherited instances
      {:ok, _} = Inheritance.delete_inherited_instances(block)

      # References should be cleaned up
      backlinks_after = References.get_backlinks("sheet", target_sheet.id)
    ...
    test "deletes and cleans hidden IDs in active and trashed sheets inside the source project", %{
      ...
      assert {:ok, 2} = Inheritance.delete_inherited_instances(source)
  ...
  describe "inherit_blocks_for_new_sheet/1" do
    ...
    test "returns {:ok, 0} for root sheets" do
      ...
      assert {:ok, 0} = Inheritance.inherit_blocks_for_new_sheet(root)
    end
  end
  ...
  describe "propagate_to_descendants/2" do
    ...
    test "validates sheet_ids are actual descendants", %{
      ...
      {:ok, _count} =
        Inheritance.propagate_to_descendants(block, [child.id, unrelated.id])
  ...
  describe "restore_block restores inherited instances" do
    ...
    test "restores instances in active and trashed sheets inside the source project", %{
      ...
      assert {:ok, 2} =
               Inheritance.restore_inherited_instances(%{
                 source
