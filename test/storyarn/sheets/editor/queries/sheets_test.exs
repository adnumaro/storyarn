defmodule Storyarn.Sheets.Editor.Queries.SheetsTest do
  ...
  describe "list_reference_options/1" do
    ...
    test "returns empty list for project with no sheets" do
      ...
      assert Sheets.list_reference_options(project.id) == []
    end
  end
  ...
  describe "resolve_variable_values/2" do
    ...
    test "returns empty map for empty refs" do
      ...
      assert Sheets.resolve_variable_values(project.id, []) == %{}
    end

    test "returns empty map when refs don't match any variables" do
      %{project: project} = setup_project()

      result = Sheets.resolve_variable_values(project.id, ["nonexistent.var"])
    ...
    test "resolves multiple simple variable values" do
      ...
      result = Sheets.resolve_variable_values(project.id, ["mc.health", "mc.name"])
    ...
    test "resolves table cell variable from 4-part ref" do
      ...
      result = Sheets.resolve_variable_values(project.id, [ref])
    ...
    test "resolves multiple simple refs from different sheets" do
      ...
      result = Sheets.resolve_variable_values(project.id, ["mc.health", "loc.name"])
  ...
  describe "validate_reference_target/3" do
    ...
    test "returns {:error, :not_found} for non-existent sheet" do
      ...
      assert {:error, :not_found} =
               Sheets.validate_reference_target("sheet", -1, project.id)
    end
    ...
    test "returns {:error, :invalid_type} for unknown type" do
      ...
      assert {:error, :invalid_type} =
               Sheets.validate_reference_target("unknown", 1, project.id)
    end
  end
  ...
  describe "resolve_block_id_by_variable/3" do
    test "resolves block ID by sheet shortcut and variable name" do
      ...
      result = Sheets.resolve_block_id_by_variable(project.id, "mc", "health")
    ...
    test "resolves a shortcutless sheet by its numeric namespace" do
      ...
      assert Sheets.resolve_block_id_by_variable(
               project.id,
    ...
    test "an explicit numeric shortcut deterministically wins over an ID fallback" do
      ...
      assert Sheets.resolve_variable_values(project.id, ["#{namespace}.health"]) == %{
               "#{namespace}.health" => 20
             }
    end
    ...
    test "returns nil for non-existent variable" do
      ...
      assert Sheets.resolve_block_id_by_variable(project.id, "mc", "nonexistent") == nil
    end
    ...
    test "returns nil for deleted sheet" do
      ...
      assert Sheets.resolve_block_id_by_variable(project.id, "mc", "health") == nil
    end
    ...
    test "does not resolve constants or unsupported block types as runtime variables" do
      ...
      assert Sheets.resolve_block_id_by_variable(project.id, "mc", "constant_health") == nil
      assert Sheets.resolve_block_id_by_variable(project.id, "mc", "unsupported_reference") == nil
    end
  end
  ...
  describe "resolve_table_block_id_by_variable/5" do
    test "resolves table block ID by shortcut, table, row, and column" do
      ...
      result =
        Sheets.resolve_table_block_id_by_variable(
          project.id,
          "mc.jaime",
          "stats",
          default_row.slug,
    ...
    test "resolves a shortcutless table sheet by its numeric namespace" do
      ...
      assert Sheets.resolve_table_block_id_by_variable(
               project.id,
               Integer.to_string(sheet.id),
               table.variable_name,
               row.slug,
  ...
  describe "list_sheets_using_asset_as_avatar/2" do
    test "returns sheets using given asset as avatar" do
      ...
      results = Sheets.list_sheets_using_asset_as_avatar(project.id, asset.id)
    ...
    test "returns empty list when no sheets use the asset" do
      ...
      assert Sheets.list_sheets_using_asset_as_avatar(project.id, asset.id) == []
    end
  ...
  describe "list_sheets_using_asset_as_banner/2" do
    test "returns sheets using given asset as banner" do
      ...
      results = Sheets.list_sheets_using_asset_as_banner(project.id, asset.id)
    ...
    test "returns empty list when no sheets use the asset" do
      ...
      assert Sheets.list_sheets_using_asset_as_banner(project.id, asset.id) == []
    end
  end
  ...
  describe "resolve_variable_values/2 edge cases" do
    test "ignores malformed refs without a dot separator" do
      ...
      result = Sheets.resolve_variable_values(project.id, ["nodots", "mc.health"])
  ...
  describe "validate_reference_target/3 flow not found" do
    test "returns {:error, :not_found} for non-existent flow" do
      ...
      assert {:error, :not_found} =
               Sheets.validate_reference_target("flow", -1, project.id)
    end
  end
  ...
  describe "stale node ids" do
    ...
    test "treat a shortcutless sheet numeric namespace as current", context do
      ...
      regular_ref =
        context.plain.id
        |> Sheets.check_stale_flow_node_variable_references(context.project.id)
