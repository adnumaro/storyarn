defmodule Storyarn.Sheets.Health.Integration.DashboardCoverageTest do
  @moduledoc """
  ...
  Both surfaces now go through `Sheets.Health.Rules.Checker.check/1`. What this pins is
  that they also FEED it equivalently: the editor from
  `Sheets.Health.Queries.Snapshots.snapshot/1` (the material the live socket holds), the
  dashboard from `Sheets.Health.Queries.Snapshots.load_project/2` (one batched read of
  ...
  alias Storyarn.Sheets.Health.Queries.Snapshots, as: HealthSnapshots
