defmodule Storyarn.Sheets.LocalizationOwnerTest do
  ...
  test "Sheet writers preserve the localization lifecycle through its owner", %{
    project: project
  } do
    ...
    Repo.update_all(
      from(text in LocalizedText, where: text.id == ^original.id),
      set: [
  ...
  test "concurrent Sheet writes leave one projection matching the committed block", %{
    ...
    assert [%LocalizedText{} = text] = texts
