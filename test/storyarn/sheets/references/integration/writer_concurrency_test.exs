defmodule Storyarn.Sheets.References.Integration.WriterConcurrencyTest do
  ...
  test "flow-node reference rebuild serializes with deleting its owning flow" do
    unboxed_scenario(fn %{project: project} ->
      ...
      assert References.count_backlinks("sheet", target.id) == 1
  ...
  defp rebuild_locked_reference(parent, barrier, project_id, block_id, target_id) do
    Repo.transaction(fn ->
      assert {:ok, _project} =
               References.lock_active_project(project_id)
