defmodule Storyarn.Sheets.References.Queries.VariableUsageTest do
  ...
  alias Storyarn.Projects.References, as: ProjectReferences
  alias Storyarn.Repo
  ...
  test "reads grouped usage and batched staleness from the shared projection", ctx do
    ...
    assert :ok = ProjectReferences.update_flow_node_variable_references(node)
    assert VariableUsage.count_variable_usage(ctx.block.id) == %{"write" => 1}

    assert [%{source_type: "flow_node", stale: false}] =
             VariableUsage.check_stale_references(ctx.block.id, ctx.project.id)
    ...
    assert VariableUsage.count_stale_references(
             [ctx.block.id, -1],
             ctx.project.id
           ) == %{ctx.block.id => 1}
  end
  ...
  test "resolves scene-owned usage without routing the read through Scenes", ctx do
    ...
    usage = VariableUsage.check_stale_references(ctx.block.id, ctx.project.id)
  ...
  test "shortcutless Sheet references remain current for every Scene source" do
    ...
    usage = VariableUsage.check_stale_references(block.id, project.id)
