defmodule Storyarn.Sheets.SceneReadModelTest do
  ...
  test "preserves the legacy zone-only appearances contract without active-Scene filtering", %{
    ...
    assert SceneReadModel.list_sheet_appearances(sheet.id) == [
             %{
               element_type: "zone",
  ...
  test "enriches Scene backlinks and excludes sources in deleted Scenes", %{
    ...
    assert [pin_backlink] = SceneReadModel.pin_backlinks("sheet", sheet.id, project.id)
    ...
    assert [zone_backlink] = SceneReadModel.zone_backlinks("sheet", sheet.id, project.id)
    ...
    assert SceneReadModel.pin_backlinks("sheet", sheet.id, project.id) == []
    assert SceneReadModel.zone_backlinks("sheet", sheet.id, project.id) == []
  end
