defmodule Storyarn.Sheets.VariableNamespaceResolverTest do
  ...
  test "explicit numeric shortcuts take precedence over an ID fallback without planner ambiguity" do
    ...
    assert VariableNamespaceResolver.resolve_sheet_id(project.id, namespace) == explicit_sheet.id

    assert VariableNamespaceResolver.resolve_sheet_ids(project.id, [namespace]) == %{
    ...
    assert VariableNamespaceResolver.resolve_sheet_id(project.id, namespace) == fallback_sheet.id
  end
  ...
  test "ID fallback is exact, active, and project scoped" do
    ...
    assert VariableNamespaceResolver.resolve_sheet_id(project.id, namespace) == sheet.id
    assert VariableNamespaceResolver.resolve_sheet_id(project.id, "0#{namespace}") == nil
    assert VariableNamespaceResolver.resolve_sheet_id(project.id, "") == nil
  end
  ...
  test "runtime and catalog surfaces expose only the authoritative numeric namespace owner" do
    ...
    assert SheetQueries.resolve_variable_values(project.id, [qualified_ref]) == %{
             qualified_ref => 20
           }

    variable_block_ids =
      project.id
      |> SheetQueries.list_project_variables()
      |> Enum.map(& &1.block_id)
    ...
    assert %{items: [%{block_id: block_id}], truncated: false} =
             VariableCatalog.list_definitions(
               project.id,
