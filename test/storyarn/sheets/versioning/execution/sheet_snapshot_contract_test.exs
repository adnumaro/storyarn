defmodule Storyarn.Sheets.Versioning.SheetSnapshotContractTest do
  ...
  describe "restore_snapshot/3" do
    ...
    test "soft-deletes an extra children definition and only its active instances", %{
      ...
      {:ok, detached_instance} = Editor.detach_block(detached_instance)
    ...
    test "syncs only active non-detached instances for children definitions", %{
      ...
      {:ok, detached_instance} =
        Editor.detach_block(detached_instance)
  ...
  defp block_reference_exists?(source_id, target_type, target_id) do
    target_type
    |> References.get_backlinks(target_id)
    |> Enum.any?(&(&1.source_type == "block" and &1.source_id == source_id))
  end
