defmodule StoryarnWeb.PageLive.VariableUsageTest do
  use StoryarnWeb.ConnCase

  import Phoenix.LiveViewTest
  import Storyarn.FlowsFixtures
  import Storyarn.PagesFixtures
  import Storyarn.ProjectsFixtures

  alias Storyarn.Flows.VariableReferenceTracker
  alias Storyarn.Repo

  describe "variable usage in references tab" do
    setup :register_and_log_in_user

    setup %{user: user} do
      project = project_fixture(user) |> Repo.preload(:workspace)
      page = page_fixture(project, %{name: "Player", shortcut: "player"})

      health_block =
        block_fixture(page, %{
          type: "number",
          config: %{"label" => "Health", "placeholder" => "0"}
        })

      flow = flow_fixture(project, %{name: "Main Quest"})

      %{
        project: project,
        page: page,
        health_block: health_block,
        flow: flow
      }
    end

    test "shows variable usage when instruction node writes to variable",
         %{conn: conn, project: project, page: page, health_block: _block, flow: flow} do
      node =
        node_fixture(flow, %{
          type: "instruction",
          data: %{
            "assignments" => [
              %{
                "id" => "assign_1",
                "page" => "player",
                "variable" => "health",
                "operator" => "add",
                "value" => "10",
                "value_type" => "literal"
              }
            ]
          }
        })

      VariableReferenceTracker.update_references(node)

      {:ok, view, _html} =
        live(
          conn,
          ~p"/workspaces/#{project.workspace.slug}/projects/#{project.slug}/pages/#{page.id}"
        )

      # Switch to references tab
      html = render_click(view, "switch_tab", %{"tab" => "references"})

      assert html =~ "Variable Usage"
      assert html =~ "Modified by"
      assert html =~ "Main Quest"
      assert html =~ "instruction"
    end

    test "shows variable usage when condition node reads variable",
         %{conn: conn, project: project, page: page, health_block: _block, flow: flow} do
      node =
        node_fixture(flow, %{
          type: "condition",
          data: %{
            "condition" => %{
              "logic" => "all",
              "rules" => [
                %{
                  "id" => "rule_1",
                  "page" => "player",
                  "variable" => "health",
                  "operator" => "greater_than",
                  "value" => "50"
                }
              ]
            }
          }
        })

      VariableReferenceTracker.update_references(node)

      {:ok, view, _html} =
        live(
          conn,
          ~p"/workspaces/#{project.workspace.slug}/projects/#{project.slug}/pages/#{page.id}"
        )

      html = render_click(view, "switch_tab", %{"tab" => "references"})

      assert html =~ "Variable Usage"
      assert html =~ "Read by"
      assert html =~ "Main Quest"
      assert html =~ "condition"
    end

    test "navigate link includes flow_id and node_id",
         %{conn: conn, project: project, page: page, flow: flow} do
      node =
        node_fixture(flow, %{
          type: "instruction",
          data: %{
            "assignments" => [
              %{
                "id" => "assign_1",
                "page" => "player",
                "variable" => "health",
                "operator" => "set",
                "value" => "100",
                "value_type" => "literal"
              }
            ]
          }
        })

      VariableReferenceTracker.update_references(node)

      {:ok, view, _html} =
        live(
          conn,
          ~p"/workspaces/#{project.workspace.slug}/projects/#{project.slug}/pages/#{page.id}"
        )

      html = render_click(view, "switch_tab", %{"tab" => "references"})

      assert html =~ "flows/#{flow.id}?node=#{node.id}"
    end

    test "variables with no usage do not show section",
         %{conn: conn, project: project, page: page} do
      {:ok, view, _html} =
        live(
          conn,
          ~p"/workspaces/#{project.workspace.slug}/projects/#{project.slug}/pages/#{page.id}"
        )

      html = render_click(view, "switch_tab", %{"tab" => "references"})

      # Section shows but with empty state message
      assert html =~ "No variables on this page are used in any flow yet."
    end

    test "page without variables does not show section at all",
         %{conn: conn, project: project} do
      # Create a page with only a divider (non-variable block)
      page = page_fixture(project, %{name: "Empty Page"})
      block_fixture(page, %{type: "divider", config: %{}})

      {:ok, view, _html} =
        live(
          conn,
          ~p"/workspaces/#{project.workspace.slug}/projects/#{project.slug}/pages/#{page.id}"
        )

      html = render_click(view, "switch_tab", %{"tab" => "references"})

      refute html =~ "Variable Usage"
    end
  end
end
