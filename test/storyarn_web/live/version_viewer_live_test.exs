defmodule StoryarnWeb.VersionViewerLiveTest do
  ...
  setup %{user: user} do
    ...
    {:ok, version} = Versioning.create_version("flow", flow, project.id, user.id, title: "v1")
  ...
  defp from_version(%EntityVersion{id: id}) do
    ...
    from(v in EntityVersion, where: v.id == ^id)
  end
end
