defmodule StoryarnWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, StoryarnWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "redirects to workspace when user is already logged in", %{conn: conn, user: user} do
      conn =
        conn
        |> assign(:current_scope, Scope.for_user(user))
        |> UserAuth.log_in_user(user)
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      StoryarnWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        StoryarnWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
